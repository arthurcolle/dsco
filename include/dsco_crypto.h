...
int dsco_hkdf_sha512(const uint8_t *ikm, size_t ikm_len, const uint8_t *salt, size_t salt_len,
...
int dsco_pwhash_argon2id(const uint8_t *password, size_t pw_len, const uint8_t salt[16],
...
/* Keyed MAC. */
void dsco_keyed_mac(const uint8_t key[32], const uint8_t *msg, size_t msg_len,
...
long dsco_aead_seal(const uint8_t key[DSCO_AEAD_KEY_BYTES],
                    const uint8_t nonce[DSCO_AEAD_NONCE_BYTES], const uint8_t *aad, size_t aad_len,
                    const uint8_t *plaintext, size_t pt_len, uint8_t *ciphertext, size_t ct_cap);
...
long dsco_aead_open(const uint8_t key[DSCO_AEAD_KEY_BYTES],
                    const uint8_t nonce[DSCO_AEAD_NONCE_BYTES], const uint8_t *aad, size_t aad_len,
                    const uint8_t *ciphertext, size_t ct_len, uint8_t *plaintext, size_t pt_cap);
...
long dsco_identity_sign(const uint8_t sk_ed25519[DSCO_ED25519_SK_BYTES],
                        const uint8_t sk_mldsa[DSCO_MLDSA_SK_BYTES], const uint8_t *msg,
                        size_t msg_len, uint8_t sig_out[DSCO_HYBRID_SIG_BYTES]);

/* Hybrid verify. Both halves MUST verify when HAVE_LIBOQS. Returns 0 on
 * success, -1 on failure. */
int dsco_identity_verify(const uint8_t pk_hybrid[DSCO_HYBRID_PK_BYTES], const uint8_t *msg,
...
int dsco_kem_keygen(uint8_t pk_hybrid[DSCO_HYBRID_KEM_PK_BYTES],
                    uint8_t sk_x25519[DSCO_X25519_SK_BYTES], uint8_t sk_mlkem[DSCO_MLKEM_SK_BYTES]);
