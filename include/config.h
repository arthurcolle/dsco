...
static const model_info_t MODEL_REGISTRY[] = {
    ...
    {"moonshotai/kimi-k2.7-code", "moonshotai/kimi-k2.7-code", 262144, 16384, 0.74, 3.50, 0.15, 0,
     1},
    {"moonshotai/kimi-k2.7-code-highspeed", "moonshotai/kimi-k2.7-code-highspeed", 262144, 16384,
     0.45, 8.00, 0.19, 0, 1},
    ...
    {"sakana/fugu-ultra-20260615", "sakana/fugu-ultra-20260615", 1000000, 32768, 0.0, 0.0, 0.0, 0,
     1},
...
static inline bool model_name_is_local_namespace(const char *name) {
    static const char *const local_pfx[] = {
        "ollama/",    "ollama:",    "lmstudio/", "lmstudio:", "mlx/",       "mlx:",
        "local/",     "local:",     "vllm/",     "vllm:",     "llamacpp/",  "llamacpp:",
        "localai/",   "localai:",   "jan/",      "jan:",      "gpt4all/",   "gpt4all:",
        "koboldcpp/", "koboldcpp:", "textgen/",  "textgen:",  "tabby/",     "tabby:",
        "tgi/",       "tgi:",       "sglang/",   "sglang:",   "llamafile/", "llamafile:",
        NULL};
    if (!name)
        return false;
...
static inline bool dsco_effort_is_wire_value(const char *effort) {
    return effort && (strcmp(effort, EFFORT_NONE) == 0 || strcmp(effort, EFFORT_MINIMAL) == 0 ||
                      strcmp(effort, EFFORT_LOW) == 0 || strcmp(effort, EFFORT_MEDIUM) == 0 ||
                      strcmp(effort, EFFORT_HIGH) == 0 || strcmp(effort, EFFORT_XHIGH) == 0);
}
...
#define SYSTEM_PROMPT                                                                                \
    ...
    "DYNAMIC CLARIFICATION: When you need user input to proceed, call AskUserQuestion instead of "   \
    "guessing or emitting a plain-text questionnaire. Use stable session_id values; reopen the "     \
    "same session_id for follow-up questions so prior answers are preserved. On status=chat, "       \
    "answer the user's concern and then reopen/continue the dialog if still needed. On "             \
    "status=no_tty, ask the same questions plainly in chat.\n"                                       \
    "MULTI-EXECUTOR SWARMS: dsco (fork self), claude (Claude Code CLI), codex (OpenAI Codex).\n\n"   \
    "TOKEN EFFICIENCY:\n"                                                                            \
    "- Issue 3+ parallel tool calls per step when gathering information (36% cheaper, 41% "          \
    "faster).\n"                                                                                     \
    "- For external parallelism, you may use bash to launch local dsc or dsco worker processes "     \
    "when swarm/executor tools are not the best fit.\n"                                              \
    "- Large tool results are truncated inline. Full results persist in VFS — use context_recall " \
    "to retrieve.\n"                                                                                 \
    "- Durable artifacts require proof: prefer write_file/append_file; if bash creates files, "      \
    "declare verify_path/verify_paths with optional size/content/hash checks.\n"                     \
...
#define SYSTEM_PROMPT_CHEAP                                                                        \
    ...
    "relevant tools, then load_tools to activate them. Use AskUserQuestion for needed user "       \
    "clarification/follow-up instead of guessing. Loaded tools persist "                           \
    ...
    "- For external parallelism, bash may launch local dsc or dsco workers when that is simpler "  \
    "than loading swarm tools.\n"                                                                  \
    "- Be concise. Prefer action over explanation."
