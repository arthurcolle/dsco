...
char *graphsub_agent_register(const char *agent_id, const char *model,
                              ...
                              const char *hostname, const char *version,
...
char *graphsub_pheromone_deposit(
    const char *agent_id, const char *target_type, /* task|agent|path|resource */
    const char *target_id,
    const char *signal_type, /* progress|attraction|warning|success|help_needed|capacity */
    double intensity,        /* 0.0 - 1.0 */
    int ttl_seconds, const char *metadata_json); /* may be NULL */

/* Query pheromones for a target. Returns response JSON (caller frees). */
char *graphsub_pheromone_query(const char *target_id, const char *signal_type, /* may be NULL */
...
char *graphsub_graph_traverse(const char *start_node, int depth,
...
char *graphsub_memory_consolidate(const char *agent_id, const char *from_ts, /* ISO 8601 */
                                  const char *to_ts);                        /* ISO 8601 */

/* ── Swarm ───────────────────────────────────────────────────────────────── */

/* Register a swarm topology. Returns response JSON (caller frees). */
char *graphsub_swarm_register(
    const char *swarm_id,
...
char *graphsub_tool_result(const char *agent_id, const char *tool_name,
                           ...
                           bool success, const char *executed_on,
                           const char *swarm_id); /* may be NULL */

/* ── Bridge Fleet ────────────────────────────────────────────────────────── */

/* Register bridge fleet peers on GraphSub. Returns response JSON (caller frees). */
char *graphsub_bridge_fleet(
    const char *peers_json); /* JSON array of {name, host, addr, status, agent_id} */
