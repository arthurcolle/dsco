...
typedef struct {
    ...
    /* Escalation budget: switches to a costlier tier consumed this session.
     * Unbounded auto-escalation is how retry loops end up parked on the most
     * expensive model. DSCO_ROUTER_MAX_ESCALATIONS (default 8, 0 = off). */
    int escalations;
    int max_escalations;
...
task_complexity_t router_classify_task(const char *user_msg, int conversation_turns,
                                       int tool_calls_last_turn, int context_token_pct); /* 0-100 */
...
router_decision_t router_decide(router_t *r, const char *current_model,
                                ...
                                double last_latency_ms, int consecutive_failures);
