...
#define DSCO_SET_ERR(code, ...) dsco_err_set((code), __FILE__, __LINE__, __VA_ARGS__)

#define DSCO_WRAP_ERR(code, ...) dsco_err_wrap((code), __FILE__, __LINE__, __VA_ARGS__)

/* Evaluate expr; if false, set error and return false */
#define DSCO_TRY(expr)                                                                             \
    do {                                                                                           \
        if (!(expr)) {                                                                             \
...
#define DSCO_REQUIRE_RET(expr, ret)                                                                \
    do {                                                                                           \
        if (!(expr)) {                                                                             \
            DSCO_SET_ERR(DSCO_ERR_INTERNAL, "invariant violated: %s", #expr);                      \
            return (ret);                                                                          \
