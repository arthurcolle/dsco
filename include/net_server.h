 ...
 * A streaming handler OWNS the connection: it must write the entire HTTP
 * response itself (status line, headers, and body) via netsrv_stream_send.
 * The server does no framing for these routes.  This is what lets
 * /v1/chat/completions emit text/event-stream token deltas. */
typedef struct netsrv_stream netsrv_stream_t;

/* Write raw bytes to the client.  Returns 0 on success, -1 on error. */
...
/* Register a STREAMING handler for METHOD /path.  The handler writes the whole
 * response over the connection via netsrv_stream_send.  Last-registered wins. */
