...
#define SI_RECORD_TURN(n, cost, in_tok, out_tok, ctx_pct, budget_pct)                              \
    self_improve_record_turn(&g_self_improve, (n), (cost), (in_tok), (out_tok), (ctx_pct),         \
                             (budget_pct))

#define SI_RECORD_TURN_USAGE(n, cost, in_tok, out_tok, cr_tok, cw_tok, ctx_pct, budget_pct)        \
    self_improve_record_turn_usage(&g_self_improve, (n), (cost), (in_tok), (out_tok), (cr_tok),    \
                                   (cw_tok), (ctx_pct), (budget_pct))

#define SI_CONSOLIDATE() self_improve_consolidate(&g_self_improve)

#define SI_TURN_RESET() self_improve_turn_reset(&g_self_improve)

#endif /* DSCO_SELF_IMPROVE_H */

/* ── Goal / swarm / strategy hooks (new) ─────────────────────────────── */

void self_improve_record_goal_state(
    self_improve_t *si, const char *goal_id,
