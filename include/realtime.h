...
/* Default cap for DSCO tools exposed to realtime sessions. Voice defaults to
 * the full API request budget, ordered by relevance before the remaining
 * registry entries are appended. */
int realtime_voice_default_max_tools(void);

/* Test/diagnostic helper: return the ordered built-in/plugin tool names that a
 * voice session would expose for `context`, before external MCP tools are appended. */
