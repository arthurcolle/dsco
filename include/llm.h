...
/* Optional global hook fired the instant a client tool_use block finalizes
 * mid-stream (full name+id+input available). Lets the agent speculatively start
 * read-only tools while the model is still generating. Set once at startup. */
...
typedef struct {
    ...
    bool ok;
    bool
        context_overflow; /* provider rejected prompt as too long → reactive compaction can retry */
    double cost_usd; /* authoritative per-turn cost reported by provider (OpenRouter usage.cost); 0
                        = not reported, fall back to token math */
    time_t credit_reset_at; /* provider-supplied epoch seconds when exhausted subscription/rate
                               window reopens */
...
/* Elide older tool_result blocks whose text is byte-identical to a later
 * one (agents re-read files and re-run status commands constantly). The
 * newest copy survives intact; older copies keep their first line
 * (breadcrumb) plus an elision note. Returns the number of blocks stubbed. */
...
/* Fast local estimate, no network round-trip. BPE-shaped heuristic instead
 * of flat chars/4: prose runs ~1 token per word, punctuation and symbols
 * almost always split into their own tokens (dense in code/JSON, where
 * chars/4 undercounts by ~30%), long identifiers fragment every ~6 chars,
 * and non-ASCII runs ~1 token per UTF-8 lead byte (CJK ≈ 1:1). Absolute
 * error is re-anchored every API round by the non_conv_overhead overlay;
 * this fixes the *slope* between anchors so threshold checks track real
 * growth. Slight overcount on symbol-dense text is intentional — compacting
 * a little early is recoverable, blowing the window is not. */
static inline int rough_token_estimate_len(const char *text, int len) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (ch >= 0x80) {
            if ((ch & 0xC0) != 0x80) /* UTF-8 lead byte */
                tokens++;
            word_len = 0;
            ...
            if (word_len == 0)
                tokens++; /* word start */
            else if (word_len % 6 == 0)
                tokens++; /* long identifiers fragment */
            word_len++;
        } else if (ch == ' ' || ch == '\t' || ch == '\n' || ch == '\r') {
            word_len = 0; /* whitespace merges into the next word's token */
        } else {
            tokens++; /* punctuation/symbol — its own token */
            word_len = 0;
        }
    }
...
static inline int rough_token_estimate(const char *text) {
    return text ? rough_token_estimate_len(text, (int)strlen(text)) : 0;
}
