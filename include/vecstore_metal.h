...
bool vm_query_cosine(const float *query, unsigned int dim, const float *corpus, unsigned int n_docs,
                     float *out_scores);

/* Top-k from pre-computed scores (CPU sort — fast for k≤100) */
void vm_topk(const float *scores, unsigned int n, unsigned int k, unsigned int *out_indices,
             float *out_vals);
