...
/* Tool history is intentionally outcome-dense in the native transcript: the
 * call appears immediately, then the same retained row is completed with a
 * bounded result preview. Full results remain in conversation/Chronicle/VFS. */
typedef enum {
    PIXEL_TUI_TOOL_VIEW_RESULTS = 0, /* call + first result line + tail count */
    PIXEL_TUI_TOOL_VIEW_CALLS,       /* call + status/size; hide result text */
    PIXEL_TUI_TOOL_VIEW_FULL,        /* call + bounded multiline result preview */
} pixel_tui_tool_view_t;
...
void pixel_tui_tool_result_preview(const char *result, pixel_tui_tool_view_t view,
                                   char *dst, size_t cap, uint32_t *tail_lines,
...
void pixel_tui_session_tool_end(FILE *out, uint64_t operation_id, const char *name,
                                bool ok, double elapsed_ms, const char *result);
