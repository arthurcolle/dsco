...
static void db_upsert_model(const or_model_t *m, const char *today) {
    ...
    const char *sql =
        "INSERT INTO models(id,name,provider,created,context_length,"
...
static void db_insert_eval(const eval_result_t *e) {
    sqlite3_stmt *st = NULL;
    const char *sql =
        "INSERT INTO evals(model_id,run_date,tier,target_tokens,actual_tokens,"
...
static bool db_already_ran(const char *model_id, const char *today, int tier) {
    sqlite3_stmt *st = NULL;
    const char *sql =
        "SELECT 1 FROM evals WHERE model_id=? AND run_date=? AND tier=? LIMIT 1;";
...
static void parse_model_elem(const char *elem, void *ctx_ptr) {
    ...
    if (name) { snprintf(m->name, sizeof(m->name), "%s", name); free(name); }
    else       { snprintf(m->name, sizeof(m->name), "%s", m->id); }
    ...
    if (pricing) {
        ...
        if (pp) { m->prompt_price = atof(pp); free(pp); }
        if (cp) { m->completion_price = atof(cp); free(cp); }
        free(pricing);
    }
    m->combined_price = m->prompt_price + m->completion_price;

    /* Age filter: skip if older than cutoff */
    if (ctx->cutoff_created > 0 && m->created > 0 &&
        m->created < ctx->cutoff_created)
        return;
...
static int cmp_price_asc(const void *a, const void *b) {
    ...
    if (ma->combined_price < mb->combined_price) return -1;
    if (ma->combined_price > mb->combined_price) return  1;
    return 0;
}

/* Parse models JSON, filter by percentile, mark include=true for middle 70% */
static int parse_and_filter(const char *json, or_model_t *models,
                             int cap, const char *today) {
    parse_ctx_t ctx = { .models = models, .count = 0, .cap = cap,
                        .cutoff_created = 0 };  /* no age cutoff */
...
static eval_result_t run_eval(const char *model_id, const char *api_key,
                               int tier, const char *today) {
    eval_result_t r = {0};
    ...
    snprintf(body, sizeof(body),
        ...
        model_id,
        TIERS[tier].target_tokens,
        TIERS[tier].prompt);

    CURL *curl = curl_easy_init();
    if (!curl) { snprintf(r.error_msg, sizeof(r.error_msg), "curl init"); return r; }

    http_buf_t buf = http_buf_new();
    if (!buf.data) { curl_easy_cleanup(curl); return r; }
    ...
    r.latency_ms = (t1.tv_sec  - t0.tv_sec)  * 1000.0
    ...
    if (http_code != 200) {
        ...
        if (!emsg) {
            ...
            if (eraw) { emsg = json_get_str(eraw, "message"); free(eraw); }
        }
        snprintf(r.error_msg, sizeof(r.error_msg), "HTTP %d: %.180s",
                 (int)http_code, emsg ? emsg : "");
        free(emsg);
        free(buf.data);
        return r;
    }
...
static int cmp_report_rows(const void *a, const void *b) {
    ...
    for (int i = 0; i < NTIERS; i++) {
        if (ra->lat[i] >= 0) { la += ra->lat[i]; ca++; }
        if (rb->lat[i] >= 0) { lb += rb->lat[i]; cb++; }
    }
    if (ca) la /= ca; if (cb) lb /= cb;
...
static const char *REPORT_SQL =
    "SELECT e.model_id, m.provider, m.prompt_price, m.completion_price,"
...
static int load_report_rows(const char *today, report_row_t *rows, int cap) {
    sqlite3_stmt *st = NULL;
    if (sqlite3_prepare_v2(g_db, REPORT_SQL, -1, &st, NULL) != SQLITE_OK) return 0;
    sqlite3_bind_text(st, 1, today, -1, SQLITE_TRANSIENT);
    int nrows = 0;
    while (sqlite3_step(st) == SQLITE_ROW) {
        ...
        for (int i = 0; i < nrows; i++) {
            if (strcmp(rows[i].model_id, mid) == 0) { row = &rows[i]; break; }
        }
        if (!row) {
            if (nrows >= cap) continue;
            row = &rows[nrows++];
...
static void print_report(const char *today, const char *filter) {
    ...
    printf(C_BOLD "  %-42s %-14s  %8s %8s    T1     T2     T3\n" C_RST,
           "Model", "Provider", "$/1Mtok-in", "$/1Mtok-out");
    printf("  %s\n",
           "─────────────────────────────────────────────────────────────────"
           "──────────────────────────────");
    ...
    for (int i = 0; i < nrows; i++) {
        ...
        if (sl) display_id = sl + 1;

        /* Success indicator */
        const char *status_color = r->ok_count == r->total_tiers ? C_GREEN :
                                   r->ok_count > 0               ? C_YELLOW : C_RED;

        printf("    %s%-40s" C_RST " %-14s  %8.2f %8.2f   ",
               status_color,
               display_id,
               r->provider,
    ...
    for (int i = 0; i < nrows; i++) {
        ...
        for (int t = 0; t < NTIERS; t++) {
            if (rows[i].lat[t] >= 0) { total_lat += rows[i].lat[t]; lat_cnt++; }
        }
    }
    printf("  %s\n",
           "─────────────────────────────────────────────────────────────────"
           "──────────────────────────────");
    printf("  " C_BOLD "Total: " C_RST
           C_GREEN "%d passed" C_RST ", "
           C_RED "%d failed" C_RST
           " — avg latency %.0fms\n\n",
           total_ok, total_fail,
           lat_cnt ? total_lat / lat_cnt : 0.0);
}
    ...
    while (tok) {
        ...
        *eq = '\0';
        ...
        for (int i = 0; i < SDIM; i++) {
            if (strcmp(tok, keys[i]) == 0) { w[i] = val; break; }
        }
        tok = strtok(NULL, ",");
    }
    /* Normalize so weights sum to 1 */
    double sum = 0;
    for (int i = 0; i < SDIM; i++) sum += w[i];
    if (sum <= 0) return false;
    for (int i = 0; i < SDIM; i++) w[i] /= sum;
    return true;
}
...
static void bar_str(double rank, int width, char *out, size_t out_len) {
    ...
    for (int i = 0; i < filled && pos + 4 < (int)out_len; i++) {
        ...
        out[pos++] = '\xe2'; out[pos++] = '\x96'; out[pos++] = '\x88';
    }
    for (int i = filled; i < width && pos + 4 < (int)out_len; i++) {
        /* UTF-8 LIGHT SHADE ░ = E2 96 91 */
        out[pos++] = '\xe2'; out[pos++] = '\x96'; out[pos++] = '\x91';
    }
    out[pos] = '\0';
}
...
static void fmt_ctx(int ctx, char *out, size_t n) {
    if (ctx <= 0)             snprintf(out, n, "  ?K");
    else if (ctx >= 1000000)  snprintf(out, n, "%3.0fM", ctx / 1e6);
    else if (ctx >= 1000)     snprintf(out, n, "%3.0fK", ctx / 1e3);
    else                      snprintf(out, n, "%4d", ctx);
}
...
static void print_recommendations(const char *today, const sel_params_t *p) {
    ...
    for (int i = 0; i < nrows; i++) {
        report_row_t *r = &rows[i];
        if (r->ok_count < NTIERS) continue;
        if (p->budget > 0 && r->prompt_price * 1e6 > p->budget) continue;
        if (p->min_ctx > 0 && r->context_length > 0 && r->context_length < p->min_ctx) continue;
        if (p->provider && r->provider[0] && !strstr(r->provider, p->provider)) continue;
        cands[ncands++] = r;
    }
    ...
    for (int d = 0; d < SDIM; d++) {
        ...
        for (int i = 0; i < ncands; i++) {
            ...
            for (int j = 0; j < ncands; j++)
                if (raw[d][j] < raw[d][i]) pos++;
            /* rank: 0 = worst (highest raw), 1 = best (lowest raw) */
            rank[d][i] = ncands > 1
                ? 1.0 - (double)pos / (double)(ncands - 1)
                : 1.0;
        }
    }
    ...
    for (int i = 0; i < ncands - 1; i++) {
        for (int j = i + 1; j < ncands; j++) {
            if (scores[j] > scores[i]) {
                report_row_t *tr = cands[i]; cands[i] = cands[j]; cands[j] = tr;
    ...
    for (int i = 0; i < top; i++) {
        ...
        printf("  %s " C_BOLD C_GREEN "%-38s" C_RST
               "  %s%-10s%s"
               "  %sScore %s%.0f/100%s\n",
               medal, disp,
               C_DIM, r->provider, C_RST,
               C_DIM, C_RST C_CYAN, scores[i], C_RST);

        /* Line 2: pricing + context */
        printf("       %s$%.3f%s / %s$%.3f%s per 1M tok"
               "  ctx %s%s%s",
               C_GREEN, r->prompt_price * 1e6, C_RST,
               C_DIM,   r->completion_price * 1e6, C_RST,
        ...
        for (int d = 0; d < SDIM; d++) {
            if (p->w[d] < 0.01) continue; /* skip near-zero weight dims */
            char bar[64];
            bar_str(dim_scores[d][i], 6, bar, sizeof(bar));
            printf("%s%s%s:%s%s%s  ",
                   C_DIM, bar_labels[d], C_RST,
                   C_CYAN, bar, C_RST);
        }
        printf("\n\n");
    }
    ...
    /* Footer */
    printf("  %s%d candidates · %d passed all tiers%s",
           C_DIM, ncands, ncands, C_RST);
    ...
    if (p->volume_tok_day > 0) {
        ...
        for (int i = 0; i < top; i++) {
            ...
            double day  = in_tok  * r->prompt_price
                        + out_tok * r->completion_price;
            double mo   = day * 30;
            double yr   = day * 365;
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if      (strcmp(argv[i], "--refresh")  == 0) force_refresh = true;
        else if (strcmp(argv[i], "--report")   == 0) report_only = true;
        else if (strcmp(argv[i], "--profiles") == 0) list_profiles = true;
        else if (strcmp(argv[i], "--select")   == 0 && i+1 < argc) select_arg = argv[++i];
        else if (strcmp(argv[i], "--weight")   == 0 && i+1 < argc) weight_arg = argv[++i];
        else if (strcmp(argv[i], "--budget")   == 0 && i+1 < argc) sel.budget = atof(argv[++i]);
        else if (strcmp(argv[i], "--min-ctx")  == 0 && i+1 < argc) sel.min_ctx = atoi(argv[++i]);
        else if (strcmp(argv[i], "--provider") == 0 && i+1 < argc) sel.provider = argv[++i];
        else if (strcmp(argv[i], "--volume")   == 0 && i+1 < argc) sel.volume_tok_day = atol(argv[++i]);
        else if (strcmp(argv[i], "--top")      == 0 && i+1 < argc) sel.top_n = atoi(argv[++i]);
        else if (strcmp(argv[i], "--filter")   == 0 && i+1 < argc) filter = argv[++i];
        else if (strcmp(argv[i], "--db")       == 0 && i+1 < argc) snprintf(db_path, sizeof(db_path), "%s", argv[++i]);
    ...
    if (!db_open(db_path)) { curl_global_cleanup(); return 1; }
    if (!db_init())        { sqlite3_close(g_db); curl_global_cleanup(); return 1; }

    /* ── Step 1: Fetch & store model list ── */
    or_model_t *models = calloc(MAX_MODELS, sizeof(or_model_t));
    if (!models) { fprintf(stderr, "OOM\n"); return 1; }
    int total_models = 0;
    ...
    if (need_fetch) {
        ...
        int included = 0;
        for (int i = 0; i < total_models; i++) if (models[i].include) included++;
    ...
    if (select_arg || weight_arg) {
        ...
        if (!prof && !weight_arg) {
            fprintf(stderr, "error: unknown task profile '%s'\n"
        ...
        if (weight_arg) {
            ...
            if (!select_arg) sel.profile_name = "custom";
        }
    ...
    for (int i = 0; i < total_models; i++) {
        ...
        if (!m->include && !filter_match) continue;
        if (filter && !filter_match) continue;
