...
static void status_write(void) {
    ...
    for (int i = 0; i < snapshot.count; i++) {
        ...
        b.len--; b.data[b.len]=0; jbuf_append(&b,",\"text\":");
        jbuf_append_json_str(&b,w->text);jbuf_appendf(&b,",\"cursor\":%zu,\"anchor\":%zu}",w->editor.cursor,w->editor.anchor);
    }
...
int main(int argc, char **argv) {
    ...
    char result[4096];
    if(!native_windows_bind_buffer(0,"Owned editor","","11111111-1111-4111-8111-111111111111",
        "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","fixture",false,result,sizeof(result))) return 2;
    ...
    if (!line || strcmp(input,"chat-safe")) atomic_fetch_add(&failures,1);
