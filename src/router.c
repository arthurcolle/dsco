...
router_policy_t router_policy_parse(const char *s) {
    ...
    if (strcmp(s, "cost") == 0)
        return ROUTER_POLICY_COST;
    if (strcmp(s, "latency") == 0)
        return ROUTER_POLICY_LATENCY;
    if (strcmp(s, "quality") == 0)
        return ROUTER_POLICY_QUALITY;
    if (strcmp(s, "balanced") == 0)
        return ROUTER_POLICY_BALANCED;
    if (strcmp(s, "adaptive") == 0)
        return ROUTER_POLICY_ADAPTIVE;
    return ROUTER_POLICY_FIXED;
}
...
const char *switch_reason_name(switch_reason_t r) {
    switch (r) {
        case SWITCH_REASON_NONE:
            return "none";
        case SWITCH_REASON_EXPLICIT:
            return "explicit";
        case SWITCH_REASON_COST_BUDGET:
            return "cost_budget";
        case SWITCH_REASON_LATENCY:
            return "latency";
        case SWITCH_REASON_COMPLEXITY_DOWN:
            return "complexity_down";
        case SWITCH_REASON_COMPLEXITY_UP:
            return "complexity_up";
        case SWITCH_REASON_FAILURE:
            return "failure";
        case SWITCH_REASON_CAPABILITY:
            return "capability";
        case SWITCH_REASON_CONTEXT_LIMIT:
...
static const kw_t KEYWORDS_UP[] = {{"analyze", 3},     {"analyse", 3},     {"reason", 3},
                                   {"explain", 2},     {"compare", 2},     {"evaluate", 3},
                                   {"synthesize", 4},  {"synthesise", 4},  {"architecture", 3},
                                   {"design", 2},      {"implement", 2},   {"refactor", 2},
                                   {"debug", 2},       {"diagnose", 3},    {"security", 3},
                                   {"optimize", 2},    {"optimise", 2},    {"research", 3},
                                   {"prove", 4},       {"derive", 4},      {"theorem", 4},
                                   {"algorithm", 3},   {"complex", 2},     {"distributed", 3},
                                   {"concurrency", 3}, {"performance", 2}, {"trade-off", 2},
                                   {"tradeoff", 2},    {"review", 2},      {"audit", 3},
                                   {"codebase", 2},    {"entire", 2},      {NULL, 0}};

static const kw_t KEYWORDS_DOWN[] = {
    {"translate", -2}, {"format", -2},  {"convert", -1}, {"list", -1},   {"summarize", -1},
    {"summarise", -1}, {"spell", -2},   {"grammar", -2}, {"hello", -3},  {"hi", -3},
    {"thanks", -3},    {"thank", -2},   {"yes", -3},     {"no", -3},     {"ok", -3},
    {"sure", -2},      {"what is", -1}, {"what's", -1},  {"who is", -2}, {"define", -1},
    {NULL, 0}};
...
static int keyword_score(const char *msg) {
    ...
    for (int k = 0; KEYWORDS_UP[k].word; k++)
        if (strstr(lower, KEYWORDS_UP[k].word))
            score += KEYWORDS_UP[k].score;
    for (int k = 0; KEYWORDS_DOWN[k].word; k++)
        if (strstr(lower, KEYWORDS_DOWN[k].word))
            score += KEYWORDS_DOWN[k].score;
    return score;
}

task_complexity_t router_classify_task(const char *user_msg, int conversation_turns,
                                       int tool_calls_last_turn, int context_token_pct) {
    int score = 0;
    ...
    /* Tool usage pressure */
    if (tool_calls_last_turn > 5)
        score += 3;
    else if (tool_calls_last_turn > 2)
        score += 2;
    else if (tool_calls_last_turn > 0)
        score += 1;

    /* Conversation depth */
    if (conversation_turns > 20)
        score += 2;
    else if (conversation_turns > 8)
        score += 1;

    /* Context pressure */
    if (context_token_pct > 70)
        score += 3;
    else if (context_token_pct > 50)
        score += 2;
    else if (context_token_pct > 30)
        score += 1;
...
int model_tier(const char *model_id) {
    ...
    if (strstr(model_id, "fable"))
        return 4;
    if (strstr(model_id, "opus"))
        return 4;
    if (strstr(model_id, "gpt-5.5-pro"))
        return 4;
    if (strstr(model_id, "gpt-5.5"))
        return 4;
    if (strstr(model_id, "gpt-5.4-pro"))
        return 4;
    if (strstr(model_id, "gpt-5.4"))
        return 4;
    if (strstr(model_id, "gpt-5.3"))
        return 4;
    if (strstr(model_id, "gpt-5.2-pro"))
        return 4;
    if (strstr(model_id, "gpt-5-pro"))
        return 4;
    if (strstr(model_id, "o1-pro"))
        return 4;
    if (strstr(model_id, "o3-pro"))
        return 4;
    if (strstr(model_id, "grok-4.20"))
        return 4;
    if (strstr(model_id, "grok-4") && !strstr(model_id, "fast"))
        return 4;
    if (strstr(model_id, "kimi-k2.7-code"))
        return 4; /* matches both kimi-k2.7-code and kimi-k2.7-code-highspeed */
    if (strstr(model_id, "kimi-k2.5"))
        return 4;
    if (strstr(model_id, "qwen3-max-thinking"))
        return 4;
    if (strstr(model_id, "gemini-3.1-deep-think"))
        return 4;
    if (strstr(model_id, "gemini-3.1-pro"))
        return 4;
    if (strstr(model_id, "gemini-3-pro"))
        return 4;
    if (strstr(model_id, "nova-premier"))
        return 4;
    if (strstr(model_id, "deepseek-r1"))
        return 4;
    if (strstr(model_id, "deepseek-r2"))
        return 4;
    if (strstr(model_id, "deepseek-v4"))
        return 4;
    if (strstr(model_id, "fugu-ultra"))
        return 4;
    if (strstr(model_id, "glm-5"))
        return 4;

    /* ── Complex tier (3) — strong general, $1–10/M output ───────────── */
    if (strstr(model_id, "sonnet"))
        return 3;
    if (strstr(model_id, "gpt-5.2"))
        return 3; /* non-pro 5.2 */
    if (strstr(model_id, "gpt-5.1"))
        return 3;
    ...
    if (strstr(model_id, "gpt-4o") && !strstr(model_id, "mini"))
        return 3;
    if (strstr(model_id, "o4-mini"))
        return 3;
    if (strstr(model_id, "o3"))
        return 3;
    if (strstr(model_id, "o1"))
        return 3;
    if (strstr(model_id, "gemini-2.5-pro"))
        return 3;
    if (strstr(model_id, "mistral-large"))
        return 3;
    if (strstr(model_id, "devstral"))
        return 3;
    if (strstr(model_id, "deepseek-v3"))
        return 3;
    if (strstr(model_id, "deepseek-chat"))
        return 3;
    if (strstr(model_id, "composer"))
        return 3;
    if (strstr(model_id, "fugu"))
        return 3;
    if (strstr(model_id, "glm-4.7"))
        return 3;
    if (strstr(model_id, "kimi-k2"))
        return 3;
    if (strstr(model_id, "qwen3.5-397b"))
        return 3;
    if (strstr(model_id, "qwen3.5-122b"))
        return 3;
    if (strstr(model_id, "qwen3.5-plus"))
        return 3;
    if (strstr(model_id, "llama-4"))
        return 3;
    if (strstr(model_id, "llama-3.3-70b"))
        return 3;
    if (strstr(model_id, "hermes-4"))
        return 3;
    if (strstr(model_id, "command-a"))
        return 3;
    if (strstr(model_id, "command-r"))
        return 3;
    if (strstr(model_id, "sonar-pro"))
        return 3;
    if (strstr(model_id, "palmyra"))
        return 3;
    if (strstr(model_id, "seed-2.0"))
        return 3;
    if (strstr(model_id, "maestro"))
        return 3;
    if (strstr(model_id, "ernie-4.5"))
        return 3;
    if (strstr(model_id, "aion-2"))
        return 3;
    if (strstr(model_id, "mercury"))
        return 3;

    /* ── Medium tier (2) — good utility, <$1/M output ────────────────── */
    if (strstr(model_id, "gpt-5-mini"))
        return 2;
    if (strstr(model_id, "gpt-4.1-mini"))
        return 2;
    if (strstr(model_id, "gpt-4o-mini"))
        return 2;
    if (strstr(model_id, "mistral-medium"))
        return 2;
    if (strstr(model_id, "mistral-small"))
        return 2;
    if (strstr(model_id, "codestral"))
        return 2;
    if (strstr(model_id, "gemini-2.5-flash"))
        return 2;
    if (strstr(model_id, "gemini-3-flash"))
        return 2;
    if (strstr(model_id, "gemini-3.1-flash"))
        return 2;
    if (strstr(model_id, "qwen3.5"))
        return 2;
    if (strstr(model_id, "qwen3-coder"))
        return 2;
    if (strstr(model_id, "minimax"))
        return 2;
    if (strstr(model_id, "step-3.5"))
        return 2;
    if (strstr(model_id, "seed-1.6"))
        return 2;
    if (strstr(model_id, "nova-2"))
        return 2;
    if (strstr(model_id, "mimo"))
        return 2;
    if (strstr(model_id, "kat-coder"))
        return 2;
    if (strstr(model_id, "sonar"))
        return 2;

    /* ── Simple tier (1) — fast/cheap, <$0.5/M output ────────────────── */
    if (strstr(model_id, "haiku"))
        return 1;
    if (strstr(model_id, "gpt-5-nano"))
        return 1;
    if (strstr(model_id, "gpt-4.1-nano"))
        return 1;
    if (strstr(model_id, "gpt-oss"))
        return 1;
    if (strstr(model_id, "llama-3.1-8b"))
        return 1;
    if (strstr(model_id, "ministral"))
        return 1;
    if (strstr(model_id, "glm-4.7-flash"))
        return 1;
    if (strstr(model_id, "nemotron-nano"))
        return 1;
    if (strstr(model_id, "seed-2.0-mini"))
        return 1;
...
router_model_stat_t *router_get_stats(router_t *r, const char *model_id) {
    ...
    if (r->stats_count >= ROUTER_STATS_MAX)
        return NULL;
...
void router_record_turn(router_t *r, const char *model_id, int input_tokens, int output_tokens,
                        double latency_ms, double cost_usd, double tokens_per_sec, bool success) {
 ...
 */
static double score_model_for_policy(router_t *r, const model_info_t *mi, router_policy_t policy,
    ...
    double ema_cost =
        st && st->ema_cost_per_turn > 0 ? st->ema_cost_per_turn : mi->input_price * 1000 / 1e6;
    ...
    double norm_quality =
        1.0 - ((double)model_tier(mi->model_id) - 1.0) / 3.0; /* invert: low tier = high cost */
    ...
    switch (policy) {
        ...
        case ROUTER_POLICY_ADAPTIVE:
            return (double)r->weight_cost * norm_cost + (double)r->weight_latency * norm_latency +
...
router_decision_t router_decide(router_t *r, const char *current_model,
                                task_complexity_t complexity, double session_cost_so_far,
    ...
    if (r->policy == ROUTER_POLICY_FIXED) {
        snprintf(d.rationale, sizeof(d.rationale), "policy=fixed; staying on %s",
    ...
    if (consecutive_failures >= 2) {
        ...
        if (stronger && strcmp(stronger, current_model) != 0) {
            ...
            d.confidence = 0.80f;
            snprintf(d.rationale, sizeof(d.rationale),
                     "%d consecutive failures on %s; escalating to %s", consecutive_failures,
                     current_model ? current_model : "?", stronger);
            pthread_mutex_unlock(&r->lock);
            return d;
        }
    }
    ...
    if (best_mi) {
        ...
        d.estimated_latency_ms = bst && bst->ema_latency_ms > 0 ? bst->ema_latency_ms : 3000.0;
    }
    ...
    if (differs) {
        ...
        if (best_tier > current_tier) {
            ...
            snprintf(d.rationale, sizeof(d.rationale),
                     ...
                     task_complexity_name(complexity), min_tier, current_model, best_id,
            ...
            snprintf(d.rationale, sizeof(d.rationale),
                     ...
                     task_complexity_name(complexity), current_model, best_id,
                     router_policy_name(r->policy), best_score);
        }
    } else {
        snprintf(d.rationale, sizeof(d.rationale),
                 "%s already optimal for task=%s (policy=%s score=%.3f)", current_model,
                 task_complexity_name(complexity), router_policy_name(r->policy), best_score);
    }
...
const char *router_fastest_model(task_complexity_t min_complexity) {
    ...
    for (int i = 0; MODEL_REGISTRY[i].alias; i++) {
        const model_info_t *mi = &MODEL_REGISTRY[i];
        if (model_tier(mi->model_id) < min_tier)
            continue;
        double price = mi->input_price + mi->output_price;
        if (price < best_price) {
            best_price = price;
...
int router_history_to_json(router_t *r, char *buf, size_t len) {
    ...
    for (int i = 0; i < r->history_count; i++) {
        ...
        jbuf_appendf(&b,
                     ...
                     h->model_id, switch_reason_name(h->reason),
                     task_complexity_name(h->complexity), h->cost_usd, h->latency_ms,
