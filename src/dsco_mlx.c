...
typedef mlx_array_t (*fn_array_new_data)(const void *data, const int *shape, int dim, int dtype);
...
typedef mlx_array_t (*fn_op_softmax)(mlx_array_t a, const int *axes, int n, int precise,
                                     mlx_stream_t s);
typedef mlx_array_t (*fn_op_argmax)(mlx_array_t a, int axis, int keepdims, mlx_stream_t s);
typedef mlx_array_t (*fn_op_norm)(mlx_array_t a, const int *axes, int n, int keepdims,
                                  mlx_stream_t s);
typedef mlx_array_t (*fn_op_divide)(mlx_array_t a, mlx_array_t b, mlx_stream_t s);
typedef mlx_array_t (*fn_op_transpose)(mlx_array_t a, const int *axes, int n, mlx_stream_t s);
...
int dsco_mlx_init(void) {
    ...
    if (!g_mlx.array_new_data || !g_mlx.array_data_f32 || !g_mlx.array_free || !g_mlx.array_eval ||
...
int dsco_mlx_matmul(const float *A, int m, int n, const float *B, int k, float *C) {
    ...
    if (out)
        memcpy(C, out, sizeof(float) * (size_t)m * (size_t)k);
    g_mlx.array_free(a);
    g_mlx.array_free(b);
    g_mlx.array_free(c);
...
int dsco_mlx_softmax(float *x, int batch, int n) {
    ...
    if (out)
        memcpy(x, out, sizeof(float) * (size_t)batch * (size_t)n);
    g_mlx.array_free(a);
    g_mlx.array_free(r);
    return out ? DSCO_MLX_OK : DSCO_MLX_ERR;
}
...
int dsco_mlx_argmax(const float *x, int batch, int n, int *idx) {
    if (!g_available || !g_mlx.argmax)
        return DSCO_MLX_UNAVAILABLE;
    if (!x || !idx || batch <= 0 || n <= 0)
        return DSCO_MLX_ERR;
    int shape[2] = {batch, n};
    mlx_stream_t s = g_mlx.default_stream(0);
    mlx_array_t a = g_mlx.array_new_data(x, shape, 2, MLX_DTYPE_FLOAT32);
...
int dsco_mlx_l2norm_rows(float *x, int n, int dim) {
    if (!g_available || !g_mlx.norm || !g_mlx.divide)
        return DSCO_MLX_UNAVAILABLE;
    if (!x || n <= 0 || dim <= 0)
        return DSCO_MLX_ERR;
    int shape[2] = {n, dim};
    ...
    const float *out = g_mlx.array_data_f32(r);
    if (out)
        memcpy(x, out, sizeof(float) * (size_t)n * (size_t)dim);
    g_mlx.array_free(a);
    g_mlx.array_free(nn);
    g_mlx.array_free(r);
    return out ? DSCO_MLX_OK : DSCO_MLX_ERR;
...
int dsco_mlx_cosine_batch(const float *q, int dim, const float *cands, int n, float *out) {
    ...
    if (res)
        memcpy(out, res, sizeof(float) * (size_t)n);
    g_mlx.array_free(r);
    free(Cbuf);
    free(qbuf);
