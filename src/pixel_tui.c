...
static void put_pixel(px_canvas_t *c, int x, int y, px_color_t color, double alpha) {
    ...
    for (int yy = 0; yy < scale && py + yy < c->pixel_height; yy++) {
        px_color_t *row = c->pixels +
            (size_t)(py + yy) * (size_t)c->pixel_width + (size_t)px;
...
static void fill_rect(px_canvas_t *c, int x, int y, int w, int h,
    ...
    if (px1 > c->pixel_width) px1 = c->pixel_width;
    if (py1 > c->pixel_height) py1 = c->pixel_height;
    for (int yy = py0; yy < py1; yy++) {
        px_color_t *row = c->pixels +
            (size_t)yy * (size_t)c->pixel_width + (size_t)px0;
...
static pixel_fx_surface_t fx_surface(px_canvas_t *c) {
    ...
    pixel_fx_surface_init(&s, (uint8_t *)c->pixels,
...
static void fill_rounded(px_canvas_t *c, int x, int y, int w, int h,
    ...
    int scale = c->backing_scale > 0 ? c->backing_scale : 1;
    pixel_fx_fill_rounded(&s, x * scale, y * scale, w * scale, h * scale,
                          radius * scale, fx_color(color), alpha);
}
...
static void stroke_rounded(px_canvas_t *c, int x, int y, int w, int h,
    ...
    pixel_fx_surface_t s = fx_surface(c);
    int scale = c->backing_scale > 0 ? c->backing_scale : 1;
    pixel_fx_stroke_rounded(&s, x * scale, y * scale, w * scale, h * scale,
                            radius * scale, scale, fx_color(color), alpha);
}
...
static void draw_panel(px_canvas_t *c, int x, int y, int w, int h, int radius,
    ...
    pixel_fx_surface_t s = fx_surface(c);
    int scale = c->backing_scale > 0 ? c->backing_scale : 1;
    pixel_fx_shadow(&s, x * scale, y * scale, w * scale, h * scale,
                    radius * scale, 14 * scale, 3 * scale,
                    (pixel_fx_rgb_t){0, 0, 0}, 0.42);
    pixel_fx_fill_rounded(&s, x * scale, y * scale, w * scale, h * scale,
                          radius * scale, fx_color(fill), fill_alpha);
    pixel_fx_stroke_rounded(&s, x * scale, y * scale, w * scale, h * scale,
                            radius * scale, scale,
                            fx_color(C_DIM), 0.26);
}
...
static void canvas_background(px_canvas_t *c, uint32_t seed) {
    ...
    for (int y = 0; y < c->pixel_height; y++) {
        double v = (double)y /
                   (double)(c->pixel_height > 1 ? c->pixel_height - 1 : 1);
...
static int draw_text(px_canvas_t *c, int x, int y, int scale, const char *text,
    ...
    int native_advance = font_compat_draw_rgb(
        ...
        text_point_size(scale) * (float)backing, false,
...
static px_canvas_t *canvas_acquire_device(int width, int height,
    ...
    if (width > INT32_MAX / backing_scale || height > INT32_MAX / backing_scale)
        return NULL;
    if (pixel_width < width * backing_scale ||
        pixel_height < height * backing_scale)
        return NULL;
    if ((size_t)pixel_width > SIZE_MAX / sizeof(px_color_t) /
                              (size_t)pixel_height)
        return NULL;
...
static px_canvas_t *canvas_acquire_scaled(int width, int height,
                                          int backing_scale) {
    if (backing_scale < 1 || backing_scale > 4 ||
        width > INT32_MAX / backing_scale ||
        height > INT32_MAX / backing_scale)
        return NULL;
    return canvas_acquire_device(width, height, width * backing_scale,
                                 height * backing_scale, backing_scale);
}
...
static void draw_session_rail(px_canvas_t *c, int x, int y, int w, int h,
    ...
    draw_meter(c, inner_x, yy, inner_w,
               ...
               (g_session.context_percent >= 60.0 ? C_AMBER : C_GREEN));
    yy += 17;
    snprintf(value, sizeof(value), "%d / %d", g_session.input_tokens,
             g_session.output_tokens);
    ...
    if (g_session.budget_limit_usd > 0.0) {
        ...
        snprintf(value, sizeof(value), "%.0f%% / %s", g_session.budget_percent,
                 runway);
        draw_key_value(c, inner_x, yy, inner_w, "BUDGET", value,
                       g_session.budget_percent >= 95.0 ? C_RED :
...
static pixel_visual_line_t *visual_lines_acquire(int cap) {
    ...
    if (s_visual_line_cap < cap) {
        pixel_visual_line_t *grown = realloc(
            s_visual_lines, (size_t)cap * sizeof(*s_visual_lines));
...
static float rich_style_size(rich_style_t style, int level) {
    if (style == RICH_STYLE_HEADING) return level <= 2 ? 13.5f : 12.5f;
    if (style == RICH_STYLE_MATH_DISPLAY) return 12.5f;
...
static int draw_rich_run(px_canvas_t *c, int x, int y, int max_width,
    ...
    int advance = font_compat_draw_rgb_styled(
        (uint8_t *)c->pixels, c->pixel_width, c->pixel_height,
        c->pixel_width * (int)sizeof(px_color_t), x * backing, y * backing,
        max_width * backing, run->text,
        rich_style_size(run->style, run->level) * (float)backing, bold, italic,
...
static void draw_session_transcript_lines(px_canvas_t *c, int x, int y,
    ...
    int role_w = 62;
    int full_role_w = font_compat_measure_utf8("  THINK",
                                               body_size, false);
    if (full_role_w > 0 && full_role_w + 8 > role_w)
        role_w = full_role_w + 8;
    if (role_w > 72) role_w = 72;
    if (role_w > w / 5) role_w = w / 5;
    ...
    for (int i = first; i < last; i++, yy += line_h) {
        ...
        if (lines[i].first) {
            ...
            font_compat_draw_rgb_styled(
                ...
                (x + 7 + slide) * backing, yy * backing,
                (role_w - 9) * backing, role_label,
...
static int session_input_columns(int width) {
    int alphabet_w = font_compat_measure_utf8(
        "abcdefghijklmnopqrstuvwxyz", text_point_size(1), false);
...
static native_ui_composer_layout_t session_input_layout(int width, int max_rows) {
    ...
    return native_ui_composer_layout(g_session.input, g_session.input_cursor,
                                     columns, max_rows);
}
...
static void draw_session_input(px_canvas_t *c, int x, int y, int w, int h) {
    ...
    for (int i = 0; i < layout.row_count; i++) {
        ...
        if (n >= sizeof(row_text)) n = sizeof(row_text) - 1;
        memcpy(row_text, g_session.input + start, n);
        row_text[n] = '\0';
        draw_text(c, x, baseline + i * line_h, scale, row_text,
                  C_TEXT, 0.96, w - 8);
    }
    ...
    if (g_session.input_active && cursor_visible_row >= 0 &&
        ...
        if (cursor < row.byte_start) cursor = row.byte_start;
        if (cursor > row.byte_end) cursor = row.byte_end;
        size_t prefix_len = cursor - row.byte_start;
        if (prefix_len >= sizeof(row_text)) prefix_len = sizeof(row_text) - 1;
        ...
        if (advance < 0) advance = layout.cursor_column * 6;
        int cursor_x = x + advance;
        if (cursor_x > x + w - 3) cursor_x = x + w - 3;
        fill_rect(c, cursor_x, baseline + cursor_visible_row * line_h,
                  2, line_h, session_animated_accent(g_session.state), 0.88);
    }
...
static px_color_t notice_color(pixel_tui_notice_level_t level) {
    switch (level) {
    case PIXEL_TUI_NOTICE_SUCCESS: return C_GREEN;
    case PIXEL_TUI_NOTICE_WARNING: return C_AMBER;
    case PIXEL_TUI_NOTICE_ERROR: return C_RED;
    case PIXEL_TUI_NOTICE_ACTIVITY: return C_VIOLET;
...
static bool session_has_live_notice(void) {
    ...
    for (int i = 0; i < PIXEL_NOTICE_CAP; i++)
        if (session_notice_alive(&g_session.notices[i], now)) return true;
    return false;
}
...
static void draw_session_notices(px_canvas_t *c, int x, int bottom, int w) {
    ...
    for (int i = 0; i < PIXEL_NOTICE_CAP; i++) {
        ...
        double alpha = age > PIXEL_NOTICE_TTL_S - 1.0
                           ? PIXEL_NOTICE_TTL_S - age : 1.0;
        if (alpha < 0.0) alpha = 0.0;
        fill_rounded(c, x + w - toast_w, yy, toast_w, 23, 5,
                     C_PANEL_ALT, 0.94 * alpha);
        fill_rounded(c, x + w - toast_w + 2, yy + 3, 2, 17, 1,
                     color, 0.86 * alpha);
        draw_text_ellipsis(c, x + w - toast_w + 10, yy + 5, 1,
                           notice->text, color, 0.90 * alpha, toast_w - 18);
    }
}
...
static void draw_session_composer_menu(px_canvas_t *c, int x, int bottom, int w) {
    ...
    if (menu->kind == PIXEL_TUI_MENU_NONE || menu->count < 1 || w < 120) return;
    int line_h = 20;
    ...
    const char *title = menu->kind == PIXEL_TUI_MENU_IMAGES
                            ? "IMAGE PICKER / TAB ATTACHES"
    ...
    for (int i = 0; i < menu->count; i++) {
        ...
        if (selected)
            fill_rounded(c, x + 5, yy - 2, panel_w - 10, line_h - 1, 4,
                         C_CYAN, 0.13);
        px_color_t label_color = item->disabled ? C_DIM :
                                 (selected ? C_CYAN : C_TEXT);
        draw_text(c, x + 11, yy + 2, 1, selected ? "›" : " ",
                  label_color, 0.90, 10);
        int label_w = panel_w >= 470 ? panel_w * 2 / 5 : panel_w - 34;
        draw_text_ellipsis(c, x + 24, yy + 2, 1, item->label,
                           label_color, selected ? 0.96 : 0.80, label_w);
        if (panel_w >= 470 && item->detail[0])
            draw_text_ellipsis(c, x + 34 + label_w, yy + 2, 1, item->detail,
                               C_DIM, 0.68, panel_w - label_w - 46);
    }
}

static px_color_t modal_color(pixel_tui_modal_kind_t kind) {
    switch (kind) {
    case PIXEL_TUI_MODAL_PERMISSION: return C_AMBER;
    case PIXEL_TUI_MODAL_QUESTION: return C_CYAN;
    case PIXEL_TUI_MODAL_MENU: return C_VIOLET;
...
static void draw_session_modal(px_canvas_t *c) {
    ...
    if (panel_w < 280) panel_w = c->width - 16;
    int line_h = 30;
    int max_visible = (c->height - 150) / line_h;
    if (max_visible < 2) max_visible = 2;
    if (max_visible > modal->count) max_visible = modal->count;
    int top = 0;
    ...
    if (modal->footer[0]) panel_h += 24;
    if (panel_h > c->height - 24) panel_h = c->height - 24;
    ...
    draw_text_ellipsis(c, x + 18, y + 15, 2,
                       modal->title[0] ? modal->title : "DSCO",
                       C_TEXT, 0.96, panel_w - 36);
    if (modal->subtitle[0])
        draw_text_ellipsis(c, x + 18, y + 39, 1, modal->subtitle,
                           C_DIM, 0.78, panel_w - 36);
    draw_line(c, x + 18, y + 61, x + panel_w - 18, y + 61,
              C_DIM, 0.24);
    ...
    for (int i = 0; i < max_visible; i++) {
        ...
        if (selected)
            fill_rounded(c, x + 12, yy - 3, panel_w - 24, line_h - 2, 5,
                         accent, 0.15);
        px_color_t color = item->disabled ? C_DIM : (selected ? accent : C_TEXT);
        draw_text(c, x + 21, yy + 4, 1, selected ? "›" : " ",
                  color, 0.92, 12);
        int label_w = panel_w >= 520 ? panel_w * 2 / 5 : panel_w - 58;
        draw_text_ellipsis(c, x + 38, yy + 4, 1, item->label,
                           color, selected ? 0.98 : 0.82, label_w);
        if (panel_w >= 520 && item->detail[0])
            draw_text_ellipsis(c, x + 48 + label_w, yy + 4, 1,
                               item->detail, C_DIM, 0.70,
                               panel_w - label_w - 72);
    }
    if (top > 0)
        draw_text(c, x + panel_w - 50, list_y - 1, 1, "▲ MORE", C_DIM, 0.66, 42);
    if (top + max_visible < modal->count)
        draw_text(c, x + panel_w - 50, list_y + max_visible * line_h - 12,
                  1, "▼ MORE", C_DIM, 0.66, 42);
    if (modal->footer[0])
        draw_text_ellipsis(c, x + 18, y + panel_h - 22, 1, modal->footer,
                           C_DIM, 0.68, panel_w - 36);
}
...
static native_ui_agent_state_t masthead_agent_state(pixel_tui_state_t state) {
    switch (state) {
    case PIXEL_TUI_REASONING: return NATIVE_UI_AGENT_REASONING;
    case PIXEL_TUI_EXECUTING: return NATIVE_UI_AGENT_EXECUTING;
    case PIXEL_TUI_RESPONDING: return NATIVE_UI_AGENT_RESPONDING;
    case PIXEL_TUI_IDLE:
    default: return NATIVE_UI_AGENT_IDLE;
    }
}

static bool draw_session_masthead(px_canvas_t *canvas,
...
static bool draw_session_composer(px_canvas_t *canvas,
                                  native_ui_rect_t frame,
                                  pixel_tui_state_t state,
                                  px_color_t accent,
                                  double accent_energy);

static px_canvas_t *render_session_frame(int width, int height, int backing_scale,
                                         int pixel_width, int pixel_height,
                                         const char *model,
                                         pixel_tui_state_t state) {
    px_canvas_t *c = canvas_acquire_device(width, height,
    ...
    if (state != PIXEL_TUI_IDLE) {
        ...
        pixel_fx_gradient_radial(&fx, device_px(c, width / 2), 0,
                                 device_px(c, width / 2), glow, 2,
    ...
    bool transcript_focus = state == PIXEL_TUI_REASONING ||
    ...
    if (max_deck_h < deck_h) max_deck_h = deck_h;
    ...
    if (transcript_h < 0) transcript_h = 0;
    ...
    native_ui_rect_t masthead_frame = {
        outer, 4, width - outer * 2, header_h - 4
    };
    if (!draw_session_masthead(c, masthead_frame, model, state, !wide, accent)) {
        ...
        draw_panel(c, masthead_frame.x, masthead_frame.y,
                   masthead_frame.width, masthead_frame.height,
                   9, C_PANEL, 0.94);
        draw_text_ellipsis(c, masthead_frame.x + inner, masthead_frame.y + 12,
                           1, "DSCO / AGENT WORKSPACE", C_TEXT, 0.92,
    ...
    native_ui_rect_t composer_frame = {
        outer, deck_y, width - outer * 2, deck_h - 6
    };
    double composer_energy = 0.58 + active_pulse * 0.14 + queue_breath * 0.22;
    if (!draw_session_composer(c, composer_frame, state, accent,
                               composer_energy)) {
        draw_panel(c, composer_frame.x, composer_frame.y,
                   composer_frame.width, composer_frame.height,
                   9, C_PANEL_ALT, 0.90);
        draw_text(c, composer_frame.x + inner, composer_frame.y + 4,
                  1, "COMPOSER", C_TEXT, 0.72,
                  composer_frame.width - inner * 2);
        draw_session_input(c, composer_frame.x + inner,
                           composer_frame.y + 18,
                           composer_frame.width - inner * 2,
                           composer_frame.height - 24);
    }
...
static size_t compositor_retained_bytes(void) {
    ...
    if ((size_t)s_visual_line_cap <= SIZE_MAX / sizeof(*s_visual_lines)) {
        ...
        if (visual_bytes <= SIZE_MAX - total) total += visual_bytes;
        else total = SIZE_MAX;
    }
    (void)pthread_mutex_lock(&g_canvas_pool_mutex);
    for (int i = 0; i < CANVAS_POOL_SLOTS; i++) {
...
bool pixel_tui_write_plan_view_ppm(const char *path, int plan_id, int width,
    ...
    bool ok = fwrite(c->pixels, sizeof(px_color_t),
                     (size_t)c->pixel_width * (size_t)c->pixel_height, f) ==
              (size_t)c->pixel_width * (size_t)c->pixel_height;
    if (fclose(f) != 0) ok = false;
    free_canvas(c);
    return ok;
}
...
bool pixel_tui_write_session_ppm(const char *path, int width, int height,
                                 const char *model, pixel_tui_state_t state) {
    if (!path || !*path || width < 320 || height < 180 ||
        state < PIXEL_TUI_IDLE || state > PIXEL_TUI_RESPONDING)
        return false;
    session_lock();
...
static void fixture_density_metrics(int width, int height,
                                    pixel_tui_density_metrics_t *metrics) {
    if (!metrics) return;
    memset(metrics, 0, sizeof(*metrics));
    native_ui_agent_shell_layout_t shell = native_ui_agent_shell_layout(width, height);
    bool transcript_focus = g_session.state == PIXEL_TUI_REASONING ||
    ...
    int max_deck_h = height - header_h - 54;
    if (max_deck_h < deck_h) max_deck_h = deck_h;
    if (deck_h + deck_extra > max_deck_h) deck_extra = max_deck_h - deck_h;
    int transcript_h = shell.transcript.height - deck_extra;
    if (transcript_h < 0) transcript_h = 0;
    int transcript_w = shell.transcript.width;
    if (transcript_focus && shell.shows_inspector)
...
bool pixel_tui_write_fixture_ppm(const char *path, int width, int height,
    ...
    if (fixture->input) {
        ...
        g_session.input_cursor = fixture->input_cursor < input_len
                                     ? fixture->input_cursor : input_len;
    }
    g_session.input_active = fixture->input_active;
    int count = fixture->message_count;
    if (count > PIXEL_MESSAGE_CAP) count = PIXEL_MESSAGE_CAP;
    for (int i = 0; i < count; i++) {
        ...
        if (source->text)
            plain_text_copy(message->text, sizeof(message->text), source->text);
    ...
    bool ok = canvas && canvas_write_ppm(path, canvas);
    if (canvas) fixture_density_metrics(width, height, metrics);
    free_canvas(canvas);
    memset(&g_session, 0, sizeof(g_session));
    visual_cache_invalidate();
    session_unlock();
...
static bool send_kitty_pixels(FILE *out, const char *control, const px_canvas_t *c,
    ...
    size_t raw_len = (size_t)c->pixel_width * (size_t)c->pixel_height *
                     sizeof(px_color_t);
    ...
    return kitty_graphics_send_pixels_ex(out, control, c->pixels, raw_len,
                                         &options, stats);
}
...
static void perf_finish_frame(pixel_tui_frame_sample_t *sample,
                              double frame_started_ms) {
    if (!sample || !pixel_tui_perf_enabled()) return;
    size_t retained = compositor_retained_bytes();
    if (retained > sample->retained_bytes) sample->retained_bytes = retained;
    sample->frame_ms = monotonic_s() * 1000.0 - frame_started_ms;
    if (sample->frame_ms < 0.0) sample->frame_ms = 0.0;
...
static void session_render_geometry(int cols, int rows, int pixel_width, int pixel_height,
    ...
    *width = w;
    *height = h;
    if (backing_scale) *backing_scale = viewport.backing_scale;
    ...
    if (surface_width)
        *surface_width = pixel_width > 0 && w == viewport.logical_width ?
                         pixel_width : w * viewport.backing_scale;
    if (surface_height)
        *surface_height = pixel_height > 0 && h == viewport.logical_height ?
                          pixel_height : h * viewport.backing_scale;
}
...
static bool session_upload_state(FILE *out, const char *model, int width, int height,
                                 int backing_scale, int surface_width,
                                 int surface_height,
    ...
    px_canvas_t *canvas = render_session_frame(width, height, backing_scale,
    ...
    if (sent && out_frame && out_frame_size) {
        size_t frame_size = (size_t)canvas->pixel_width *
                            (size_t)canvas->pixel_height * sizeof(px_color_t);
        ...
        if (baseline) {
            ...
            *out_frame_size = frame_size;
            size_t retained = compositor_retained_bytes();
            sample.retained_bytes = frame_size <= SIZE_MAX - retained
                                        ? retained + frame_size : SIZE_MAX;
        }
    }
...
static int session_collect_damage(const px_canvas_t *canvas,
    ...
    for (int ty = 0; ty < canvas->pixel_height; ty += SESSION_DAMAGE_TILE) {
        int th = canvas->pixel_height - ty < SESSION_DAMAGE_TILE
                     ? canvas->pixel_height - ty : SESSION_DAMAGE_TILE;
        for (int tx = 0; tx < canvas->pixel_width; tx += SESSION_DAMAGE_TILE) {
            int tw = canvas->pixel_width - tx < SESSION_DAMAGE_TILE
                         ? canvas->pixel_width - tx : SESSION_DAMAGE_TILE;
            bool dirty = false;
    ...
    double coverage = (double)dirty_area /
                      ((double)canvas->pixel_width *
                       (double)canvas->pixel_height);
...
static bool session_send_patch(FILE *out, const px_canvas_t *canvas,
    ...
    bool sent = kitty_graphics_send_pixels_ex(out, control, region, bytes,
                                              &options, stats);
    free(region);
    return sent;
}
...
static void session_store_frame(const px_canvas_t *canvas, uint32_t image_id) {
    size_t need = (size_t)canvas->pixel_width *
                  (size_t)canvas->pixel_height * 3;
...
static bool session_repaint(FILE *out, bool force) {
    ...
    px_canvas_t *canvas = render_session_frame(g_session.width, g_session.height,
                                               g_session.backing_scale,
                                               g_session.surface_width,
                                               g_session.surface_height,
    ...
    if (g_session.patch_enabled && current_id != 0 &&
        ...
        if (count > 0) {
            if (perf) upload_started_ms = monotonic_s() * 1000.0;
            bool sent = true;
            for (int i = 0; i < count && sent; i++) {
                ...
                sent = session_send_patch(out, canvas, current_id, rects[i],
                                          perf ? &stats : NULL);
                if (perf) {
                    size_t region_bytes = (size_t)rects[i].w *
                                          (size_t)rects[i].h * 3U;
    ...
    snprintf(control, sizeof(control), "a=t,t=d,f=24,s=%d,v=%d,i=%u,q=2,o=z",
             canvas->pixel_width, canvas->pixel_height, next_id);
...
static bool session_refresh_geometry_locked(FILE *out, bool reanchor_unchanged) {
    ...
    session_render_geometry(cols, rows, pixel_width, pixel_height,
                            &width, &height, &backing_scale,
                            &surface_width, &surface_height);
    if (cols == g_session.cols && rows == g_session.rows &&
        width == g_session.width && height == g_session.height &&
        backing_scale == g_session.backing_scale &&
        surface_width == g_session.surface_width &&
        surface_height == g_session.surface_height) {
        if (reanchor_unchanged) {
            session_place_current(out);
            fflush(out);
        }
...
static void *session_animation_thread_main(void *arg) {
    ...
    while (g_session.active && !g_session.animation_stop) {
        ...
        if (g_session.terminal_suspended) {
            ...
            double remaining = session_repaint_interval_s() -
                (monotonic_s() - g_session.last_paint_s);
            int pending_delay = remaining > 0.0 ?
                (int)ceil(remaining * 1000.0) : 1;
            if (pending_delay < delay_ms) delay_ms = pending_delay;
        }
...
bool pixel_tui_session_begin(FILE *out, const char *model) {
    ...
    bool ok = session_upload_state(out, model, width, height, backing_scale,
                                   surface_width, surface_height,
                                   generation,
                                   PIXEL_TUI_IDLE, &image_ids[PIXEL_TUI_IDLE],
                                   &initial_frame, &initial_frame_size);
    if (!ok) {
        free(initial_frame);
    ...
    /* Thirty hertz keeps token arrival visually immediate while still
     * coalescing provider deltas. The loop parks when neither motion nor a
     * semantic repaint is pending. */
    int animation_interval_ms = 33;
...
static bool capture_is_progress_tick(const char *line, size_t len) {
    ...
    if (len > 1 && line[len - 1] == ')') len--; /* "(12.3s)" elapsed suffix */
...
bool pixel_capture_parser_feed(pixel_capture_parser_t *p, const char *bytes,
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (p->state == CAPTURE_ESC) {
            if (ch == '[') p->state = CAPTURE_CSI;
            else if (ch == ']' || ch == '_' || ch == 'P' || ch == '^')
                p->state = CAPTURE_STRING;
            else p->state = CAPTURE_TEXT;
            continue;
        }
        ...
        if (p->state == CAPTURE_STRING) {
            if (ch == '\a') p->state = CAPTURE_TEXT;
            else if (ch == 0x1b) p->state = CAPTURE_STRING_ESC;
            continue;
        }
        ...
        if (ch == 0x1b) {
            ...
            if (ch == '\t') {
                do {
                    if (p->len + 1 < sizeof(p->line)) p->line[p->len++] = ' ';
                } while (p->len % 4 != 0);
                continue;
            }
...
static void *session_capture_thread_main(void *arg) {
    ...
    for (;;) {
        ...
        dirty |= pixel_capture_parser_feed(&parser, input, (size_t)got,
                                           session_capture_publish_cb, NULL);
...
void pixel_tui_session_set_composer(FILE *out, const char *text, size_t cursor,
    ...
    memset(&g_session.composer_menu, 0, sizeof(g_session.composer_menu));
    if (menu_kind != PIXEL_TUI_MENU_NONE && items && item_count > 0) {
        ...
        g_session.composer_menu.kind = menu_kind;
        g_session.composer_menu.count = item_count;
        if (selected < 0) selected = 0;
        if (selected >= item_count) selected = item_count - 1;
        g_session.composer_menu.selected = selected;
        for (int i = 0; i < item_count; i++) {
            pixel_composer_item_t *dst = &g_session.composer_menu.items[i];
...
void pixel_tui_session_set_input(FILE *out, const char *text, size_t cursor, bool active) {
    pixel_tui_session_set_composer(out, text, cursor, active,
...
void pixel_tui_session_set_model(FILE *out, const char *model, const char *slot_name) {
    ...
    out = session_output(out);
    if (model) snprintf(g_session.model, sizeof(g_session.model), "%s", model);
    if (slot_name)
...
void pixel_tui_session_set_budget(FILE *out, double limit_usd, double burn_rate,
    ...
    if (percent < 0.0) percent = 0.0;
    if (percent > 999.0) percent = 999.0;
...
bool pixel_tui_session_set_theme(FILE *out, const char *name) {
    if (!px_theme_find(name)) return false;
...
void pixel_tui_session_show_modal(FILE *out, pixel_tui_modal_kind_t kind,
    ...
    modal->active = true;
    modal->kind = kind >= PIXEL_TUI_MODAL_INFO && kind <= PIXEL_TUI_MODAL_MENU
                      ? kind : PIXEL_TUI_MODAL_INFO;
    ...
    if (item_count > PIXEL_MODAL_ITEM_CAP) item_count = PIXEL_MODAL_ITEM_CAP;
    modal->count = item_count;
    if (selected < 0) selected = 0;
    if (item_count > 0 && selected >= item_count) selected = item_count - 1;
...
int pixel_tui_render_plan_view(FILE *out, int plan_id, pixel_plan_view_t view) {
    ...
    snprintf(control, sizeof(control),
             ...
             c->pixel_width, c->pixel_height, image_id,
             placement_cols, occupied_rows);
...
static native_ui_rect_t scene_host_rect(const scene_pixel_host_t *host,
                                        native_ui_rect_t rect) {
...
static void scene_host_push_clip(void *surface, native_ui_rect_t rect) {
    ...
    (void)pixel_fx_clip_push(&host->fx, rect.x * scale, rect.y * scale,
...
static void scene_host_fill_rect(void *surface, native_ui_rect_t rect,
    ...
    if (raised && radius > 0)
        pixel_fx_shadow(&host->fx, rect.x * scale, rect.y * scale,
                        rect.width * scale, rect.height * scale,
                        radius * scale, 12 * scale, 3 * scale,
                        (pixel_fx_rgb_t){0, 0, 0}, 0.38 * alpha);
    pixel_fx_fill_rounded(&host->fx, rect.x * scale, rect.y * scale,
                          rect.width * scale, rect.height * scale,
                          radius * scale,
                          fx_color(scene_host_color(color)), alpha);
}

static void scene_host_stroke_rect(void *surface, native_ui_rect_t rect,
                                   px_backend_color_t color, uint8_t opacity,
                                   uint8_t width, uint8_t radius) {
    scene_pixel_host_t *host = surface;
    rect = scene_host_rect(host, rect);
    int scale = host->canvas->backing_scale;
    pixel_fx_stroke_rounded(&host->fx, rect.x * scale, rect.y * scale,
                            rect.width * scale, rect.height * scale,
                            radius * scale,
                            (width > 0 ? width : 1) * scale,
                            fx_color(scene_host_color(color)),
                            (double)opacity / 255.0);
}

static void scene_host_draw_text(void *surface, native_ui_rect_t rect,
    ...
    draw_text_ellipsis(host->canvas, rect.x, y, scale, text,
                       scene_host_color(color), (double)opacity / 255.0,
                       rect.width);
}

static void scene_host_draw_icon(void *surface, native_ui_rect_t rect,
                                 const char *name, px_backend_color_t color,
                                 uint8_t opacity) {
    scene_pixel_host_t *host = surface;
    (void)name;
    rect = scene_host_rect(host, rect);
    ...
    fill_circle(host->canvas, cx, cy, radius, resolved,
                (double)opacity / 300.0);
    draw_circle_ring(host->canvas, cx, cy, radius + 2, 1, resolved,
                     (double)opacity / 640.0);
}

static void scene_host_draw_line(void *surface, int x0, int y0, int x1, int y1,
                                 px_backend_color_t color, uint8_t opacity) {
    scene_pixel_host_t *host = surface;
    draw_line(host->canvas, x0 + host->origin_x, y0 + host->origin_y,
              x1 + host->origin_x, y1 + host->origin_y,
              scene_host_color(color), (double)opacity / 255.0);
}
...
static void scene_host_fill_circle(void *surface, int cx, int cy, int radius,
                                   px_backend_color_t color, uint8_t opacity) {
    scene_pixel_host_t *host = surface;
    fill_circle(host->canvas, cx + host->origin_x, cy + host->origin_y,
                radius, scene_host_color(color), (double)opacity / 255.0);
}
...
static void scene_host_draw_custom(void *surface, const native_ui_node_t *node,
    ...
    if (node && node->key == NATIVE_MASTHEAD_KEY_SOUL) {
        ...
        draw_dsco_soul(host->canvas, rect.x + rect.width / 2,
                       rect.y + rect.height / 2, radius, host->state);
...
static void scene_pixel_backend_init(px_backend_t *backend,
                                     scene_pixel_host_t *host,
                                     px_canvas_t *canvas, int origin_x,
                                     int origin_y, pixel_tui_state_t state,
                                     px_color_t accent) {
...
static bool draw_session_masthead(px_canvas_t *canvas,
                                  ...
                                  bool show_compact_metrics,
                                  px_color_t accent) {
    if (!canvas || frame.width < 160 || frame.height < 44) return false;
    int next = s_masthead_scene_valid ? 1 - s_masthead_scene_index : 0;
    native_ui_scene_t *scene = &s_masthead_scenes[next];
    const native_ui_scene_t *previous = s_masthead_scene_valid
        ? &s_masthead_scenes[s_masthead_scene_index] : NULL;
    const char *title = canvas->width < 700 ? "DSCO / WORKSPACE"
    ...
    scene_pixel_backend_init(&backend, &host, canvas, frame.x, frame.y,
                             state, accent);
    ...
    backend.palette.colors[NATIVE_UI_COLOR_SURFACE_RAISED] =
        scene_backend_color(C_PANEL);
...
static bool draw_session_composer(px_canvas_t *canvas,
                                  ...
                                  px_color_t accent,
                                  double accent_energy) {
    if (!canvas || frame.width < 160 || frame.height < 56) return false;
    int next = s_composer_scene_valid ? 1 - s_composer_scene_index : 0;
    native_ui_scene_t *scene = &s_composer_scenes[next];
    const native_ui_scene_t *previous = s_composer_scene_valid
        ? &s_composer_scenes[s_composer_scene_index] : NULL;
    char clock_label[16] = {0};
    if (g_session.show_clock) {
        time_t wall = time(NULL);
        struct tm tm_now;
        if (localtime_r(&wall, &tm_now))
            snprintf(clock_label, sizeof(clock_label), "%02d:%02d",
...
static px_canvas_t *render_scene_frame(const char *scene_json, int width,
    ...
    scene_pixel_host_t host;
    px_backend_t backend;
    scene_pixel_backend_init(&backend, &host, c, 0, 0,
                             PIXEL_TUI_IDLE, C_CYAN);
