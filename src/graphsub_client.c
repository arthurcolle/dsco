...
bool graphsub_is_available(void) {
    ...
    char cmd[256];
    snprintf(cmd, sizeof(cmd), "curl -sk --max-time 2 http://%s:%u/health >/dev/null 2>&1", s_host,
             s_port);
    return system(cmd) == 0;
#endif
}
...
#else
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "curl -sk --max-time 10 http://%s:%u%s 2>/dev/null", s_host, s_port,
             path);
    FILE *fp = popen(cmd, "r");
    if (!fp)
        return NULL;
...
char *graphsub_agent_register(const char *agent_id, const char *model,
                              const char *capabilities_json, const char *tools_json,
                              const char *hostname, const char *version, const char *bridge_peer) {
    char body[4096];
    int n = snprintf(body, sizeof(body),
                     ...
                     capabilities_json ? capabilities_json : "[]", tools_json ? tools_json : "[]",
                     hostname ? hostname : "", version ? version : "", s_tenant,
                     bridge_peer ? ",\"bridge_peer\":\"" : "", bridge_peer ? bridge_peer : "",
                     bridge_peer ? "\"" : "");
...
char *graphsub_agent_heartbeat(const char *agent_id, const char *status, double load,
                               int memory_used_mb, int active_agents, int uptime_seconds) {
    char body[512];
    snprintf(body, sizeof(body),
             ...
             "\"active_agents\":%d,"
             "\"uptime_seconds\":%d}",
             agent_id ? agent_id : "", status ? status : "idle", load, memory_used_mb,
             active_agents, uptime_seconds);
...
char *graphsub_pheromone_deposit(const char *agent_id, const char *target_type,
                                 const char *target_id, const char *signal_type, double intensity,
                                 int ttl_seconds, const char *metadata_json) {
    char body[1024];
    snprintf(body, sizeof(body),
             "{\"agent_id\":\"%s\","
             "\"target_type\":\"%s\","
             ...
             "\"ttl_seconds\":%d"
             "%s%s%s}",
             agent_id ? agent_id : "", target_type ? target_type : "task",
             target_id ? target_id : "", signal_type ? signal_type : "progress", intensity,
             ttl_seconds, metadata_json ? ",\"metadata\":" : "", metadata_json ? metadata_json : "",
             metadata_json ? "" : "");
    return graphsub_post("/pheromone/deposit", body);
}

char *graphsub_pheromone_query(const char *target_id, const char *signal_type,
    ...
    if (signal_type) {
        ...
        snprintf(path, sizeof(path), "/pheromone/query?target_id=%s&min_intensity=%.2f",
...
char *graphsub_pheromone_sweep(const char *target_type, const char *signal_type) {
    char body[256];
    snprintf(body, sizeof(body), "{\"target_type\":%s,\"signal_type\":%s}",
             target_type ? target_type : "null", signal_type ? signal_type : "null");
    return graphsub_post("/pheromone/sweep", body);
}

/* ── Graph Operations ───────────────────────────────────────────────────── */

char *graphsub_graph_traverse(const char *start_node, int depth, const char *edge_filter_json,
                              int limit) {
    char body[1024];
    snprintf(body, sizeof(body),
             "{\"start\":\"%s\","
             ...
             "%s%s%s"
             "\"limit\":%d}",
             start_node ? start_node : "", depth > 0 ? depth : 3,
             edge_filter_json ? "\"edge_filter\":" : "", edge_filter_json ? edge_filter_json : "",
             edge_filter_json ? "," : "", limit > 0 ? limit : 100);
...
char *graphsub_memory_sync(const char *agent_id, const char *episodes_json, const char *type) {
    ...
    fprintf(mem,
            ...
            "\"type\":\"%s\","
            "\"episodes\":%s}",
            agent_id ? agent_id : "", type ? type : "episodic",
...
char *graphsub_memory_consolidate(const char *agent_id, const char *from_ts, const char *to_ts) {
    ...
    snprintf(body, sizeof(body),
             ...
             agent_id ? agent_id : "", from_ts ? from_ts : "", to_ts ? to_ts : "");
    return graphsub_post("/memory/consolidate", body);
}

/* ── Swarm ───────────────────────────────────────────────────────────────── */

char *graphsub_swarm_register(const char *swarm_id, const char *topology, const char *agents_json,
    ...
    fprintf(mem,
            ...
            "\"agents\":%s,"
            "\"task\":\"%s\"}",
            swarm_id ? swarm_id : "", topology ? topology : "fanout",
            agents_json ? agents_json : "[]", task_desc ? task_desc : "");
...
char *graphsub_tool_result(const char *agent_id, const char *tool_name,
                           const char *tool_params_json, const char *result_json, int duration_ms,
                           bool success, const char *executed_on, const char *swarm_id) {
    char *body = NULL;
    size_t body_len = 0;
    FILE *mem = open_memstream(&body, &body_len);
    if (!mem)
    ...
    fprintf(mem,
            ...
            tool_params_json ? tool_params_json : "{}", result_json ? result_json : "{}",
            duration_ms, success ? "true" : "false", executed_on ? executed_on : "",
            swarm_id ? ",\"swarm_id\":\"" : "", swarm_id ? swarm_id : "", swarm_id ? "\"" : "");
...
char *graphsub_bridge_fleet(const char *peers_json) {
    char body[4096];
    snprintf(body, sizeof(body), "{\"peers\":%s}", peers_json ? peers_json : "[]");
