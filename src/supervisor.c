...
bool supervisor_resolve_hotswap_exec(const char *current_bin, const char *env_bin, char *exec_path,
                                     size_t exec_path_len, bool *explicit_hotswap) {
    if (!exec_path || exec_path_len == 0)
        return false;
    exec_path[0] = '\0';
    if (explicit_hotswap)
        *explicit_hotswap = false;

    if (env_bin && env_bin[0]) {
        if (access(env_bin, X_OK) != 0)
            return false;
        int n = snprintf(exec_path, exec_path_len, "%s", env_bin);
        if (n <= 0 || (size_t)n >= exec_path_len) {
            exec_path[0] = '\0';
            return false;
        }
        if (explicit_hotswap)
            *explicit_hotswap = true;
        return true;
    }

    if (!current_bin || !current_bin[0])
        return false;

    const char *slash = strrchr(current_bin, '/');
    if (!slash)
        return false;

    char staged[PATH_MAX];
    size_t dir_len = (size_t)(slash - current_bin);
    if (dir_len + sizeof("/dsco-new") > sizeof(staged))
        return false;
    memcpy(staged, current_bin, dir_len);
    memcpy(staged + dir_len, "/dsco-new", sizeof("/dsco-new"));

    if (access(staged, X_OK) != 0)
        return false;

    int n = snprintf(exec_path, exec_path_len, "%s", staged);
    if (n <= 0 || (size_t)n >= exec_path_len) {
        exec_path[0] = '\0';
        return false;
    }
...
static void supervisor_dir(char *dir, size_t len) {
    ...
    if (home && home[0])
        snprintf(dir, len, "%s/.dsco", home);
    else
        snprintf(dir, len, "/tmp/.dsco");
...
static void append_child_metric(const char *path, pid_t child, double uptime, uint64_t rss,
                                uint64_t peak_rss, int pressure) {
    FILE *f = fopen(path, "a");
    if (!f)
        return;
    fprintf(f,
            ...
            (unsigned long long)(rss >> 20), (unsigned long long)(peak_rss >> 20), pressure);
    fclose(f);
}
...
static uint64_t system_mem_bytes(void) {
    ...
    if (pages > 0 && psz > 0)
        mem = (uint64_t)pages * (uint64_t)psz;
#endif
    return mem;
}
...
static void fleet_remove(void) {
    ...
    buf[0] = '\0';
    if (jfd >= 0) {
        ssize_t n = read(jfd, buf, sizeof(buf) - 1);
        if (n > 0)
            buf[n] = '\0';
        close(jfd);
    }
    char mykey[32];
    snprintf(mykey, sizeof(mykey), "\"sup%d\"", (int)getpid());
    ...
    if (buf[0] == '{') {
        ...
        while (*p && op < (int)sizeof(out) - 256) {
            ...
            if (!ob || !cb)
                break;
            const char *tsp = strstr(ob, "\"ts\":");
            long long ts = 0;
            if (tsp)
                ts = strtoll(tsp + 6, NULL, 10);
            if (ts > 0 && (long long)time(NULL) - ts > 60) {
                p = cb + 1;
                if (*p == ',')
                    p++;
                continue;
            ...
            first = false;
            int span = (int)(cb - q) + 1;
            if (op + span < (int)sizeof(out) - 2) {
                memcpy(out + op, q, span);
                op += span;
            }
            p = cb + 1;
            if (*p == ',')
...
static fleet_totals_t fleet_read_totals(void) {
    ...
    int fd = open(DSCO_FLEET_JSON_PATH, O_RDONLY);
    if (fd < 0)
        return t;
    char buf[16384];
    buf[0] = '\0';
    ...
    while (*p) {
        ...
        if (age <= 60) {
            ...
            if (em && em < cb)
                t.ewma_mb += (uint64_t)strtoull(em + 10, NULL, 10);
            if (pm && pm < cb)
                t.peak_mb += (uint64_t)strtoull(pm + 10, NULL, 10);
            t.sessions++;
        }
        p = cb + 1;
    }
...
static void fleet_update(pid_t child, uint64_t rss_bytes, uint64_t ewma_mb, uint64_t peak_mb,
                         uint64_t budget_mb) {
    int lfd = open(DSCO_FLEET_LOCK_PATH, O_RDWR | O_CREAT, 0600);
    if (lfd < 0)
        return;
...
static uint64_t fleet_compute_budget(uint64_t our_rss_bytes, uint64_t our_ewma_mb,
                                     uint64_t our_peak_mb, uint64_t fleet_deriv_mb,
    ...
    uint64_t fleet_demand = (ft.ewma_mb * (DSCO_PEAK_WEIGHT_DEN - DSCO_PEAK_WEIGHT_NUM) +
                             ft.peak_mb * DSCO_PEAK_WEIGHT_NUM) /
                            DSCO_PEAK_WEIGHT_DEN;
    ...
    /* Shrink hysteresis */
    if (shrink_hold_ticks && pool > DSCO_POOL_BASE_MB && fleet_demand + DSCO_POOL_STEP_MB < pool) {
    ...
    uint64_t our_demand = (our_ewma_mb * (DSCO_PEAK_WEIGHT_DEN - DSCO_PEAK_WEIGHT_NUM) +
                           our_peak_mb * DSCO_PEAK_WEIGHT_NUM) /
                          DSCO_PEAK_WEIGHT_DEN;
    uint64_t fd_total = fleet_demand > 0 ? fleet_demand : 1;

    uint64_t budget_mb = our_demand > 0 ? (pool * our_demand) / fd_total
                                        : pool / (uint64_t)(ft.sessions > 0 ? ft.sessions : 1);

    if (budget_mb < DSCO_SESSION_MIN_BUDGET_MB)
        budget_mb = DSCO_SESSION_MIN_BUDGET_MB;
    return budget_mb << 20;
}
...
static crash_class_t classify_exit(int status) {
    if (WIFEXITED(status)) {
        ...
        if (code == 0)
            return EXIT_CLEAN;
        if (code == DSCO_EXIT_USER_REQUESTED)
            return EXIT_USER_REQUESTED;
        if (code == DSCO_EXIT_CONFIG)
            return EXIT_CONFIG;
...
static void write_incident_json(FILE *f, const incident_report_t *r, const char *incident_path) {
    ...
    fprintf(f,
            ...
            "  \"child_cmdline\":\"%s\",\n",
            (long long)time(NULL), (int)getpid(), (int)r->child_pid, crash_class_name(r->cls),
            signal_name(r->sig), r->sig, r->exit_code, action_esc, r->restart_count,
            r->next_delay_ms, r->uptime, (unsigned long long)(r->peak_rss >> 20),
            (unsigned long long)(r->mem_hard >> 20), (unsigned long long)(r->mem_soft >> 20),
            r->pressure, r->poll_ms, r->preempted ? "true" : "false",
            r->tracer_reaped ? "true" : "false",
            getenv("DSCO_RESUME_AFTER_CRASH") ? "true" : "false",
            getenv("DSCO_MEM_PRESSURE") ? "true" : "false", have_hb_pid ? hb_pid : -1, hb_age_s,
            (have_hb_pid && hb_pid == (long long)r->child_pid) ? "true" : "false", cmd_esc);
    ...
    fprintf(f,
            ...
            incident_esc, hb_path_esc, runtime_esc, supervisor_esc, metrics_esc, bt_path);

    fprintf(f, "  \"last_heartbeat\":");
    if (have_hb && json_object_like(last_hb))
        fputs(last_hb, f);
    else
        fprintf(f, "\"%s\"", hb_esc);
    fprintf(f, ",\n");
    ...
    fprintf(f,
            ...
            "}\n",
            have_crash ? "true" : "false", have_bt ? "true" : "false", crash_esc, bt_esc);
}

static bool write_incident_report(const incident_report_t *r, char *out_path, size_t out_path_len) {
    char dir[512], incident_dir[640];
    supervisor_dir(dir, sizeof(dir));
    snprintf(incident_dir, sizeof(incident_dir), "%s/incidents", dir);
    if (mkdir(incident_dir, 0700) != 0 && errno != EEXIST)
        return false;
    ...
    char path[PATH_MAX];
    snprintf(path, sizeof(path), "%s/incident-%s-pid%d.json", incident_dir, stamp,
             (int)r->child_pid);
...
static void capture_diagnostics(pid_t child_pid, crash_class_t cls, int sig) {
    ...
    if (f) {
        ...
        fprintf(stderr, "[supervisor] no /tmp/dsco_crash.log for pid=%d (%s/%s).\n", (int)child_pid,
                crash_class_name(cls), signal_name(sig));
        if (cls == EXIT_OOM_KILL || sig == SIGKILL) {
            fprintf(stderr, "[supervisor] SIGKILL cannot be caught by the child; "
...
static bool common_shell_name(const char *name) {
    ...
    lower[i] = '\0';
    return strcmp(lower, "sh") == 0 || strcmp(lower, "bash") == 0 || strcmp(lower, "zsh") == 0 ||
           strcmp(lower, "fish") == 0 || strcmp(lower, "tcsh") == 0 || strcmp(lower, "csh") == 0 ||
           strcmp(lower, "ksh") == 0 || strcmp(lower, "dash") == 0 || strcmp(lower, "nu") == 0 ||
           strcmp(lower, "elvish") == 0 || strcmp(lower, "pwsh") == 0;
}
...
static void restore_tty_before_shell(void) {
    ...
    static const char reset[] = "\033[?1049l\033[?1047l\033[?47l" /* leave alt screens */
...
int supervisor_run(int child_argc, char **child_argv) {
    ...
    int backoff_max_ms =
        dsco_env_int("DSCO_SUPERVISE_BACKOFF_MAX_MS", DEFAULT_BACKOFF_MAX_MS, 100, 300000);

    /* ── Active memory watchdog tunables ──────────────────────────────── */
    int poll_ms = dsco_env_int("DSCO_SUPERVISE_POLL_MS", DEFAULT_POLL_MS, 20, 5000);
    int term_grace_ms =
        dsco_env_int("DSCO_SUPERVISE_TERM_GRACE_MS", DEFAULT_TERM_GRACE_MS, 100, 60000);
    ...
    format_cmdline(child_argc, child_argv, child_cmdline, sizeof(child_cmdline));
    {
        ...
        fprintf(stderr,
                ...
                (unsigned long long)(mem_soft_bytes >> 20), (unsigned long long)(sys >> 20),
                poll_ms);
        supervisor_log("event=start child_cmdline=%s mem_budget_mb=%llu soft_mb=%llu ram_mb=%llu "
                       "poll_ms=%d metrics_s=%d",
    ...
    enum {
        ...
        RT_PERMANENT,
        RT_TEMPORARY
    } restart_type = (rt && strcmp(rt, "permanent") == 0)   ? RT_PERMANENT
                     : (rt && strcmp(rt, "temporary") == 0) ? RT_TEMPORARY
                                                            : RT_TRANSIENT;
    ...
    for (;;) {
        ...
        if (child == 0) {
             ...
             *
             * Do not rename the staged binary over child_argv[0] here. Other
             * supervised sessions may already have a tamper watcher on the
             * target path; replacing it makes healthy peers exit with
             * "[TAMPER] binary deleted, renamed, or revoked". A committed
             * upgrade belongs in make/install, not in a crash-restart child. */
            {
                bool hotswap_explicit = false;
                char hotswap_path[PATH_MAX] = {0};

                if (supervisor_resolve_hotswap_exec(child_argv[0], getenv("DSCO_HOTSWAP_BIN"),
                                                    hotswap_path, sizeof(hotswap_path),
                                                    &hotswap_explicit)) {
                    fprintf(stderr, "[supervisor] hotswap: exec staged %s (target remains %s)\n",
                            hotswap_path, child_argv[0]);
                    supervisor_log("event=hotswap_exec staged=%s target=%s explicit=%d",
                                   hotswap_path, child_argv[0], hotswap_explicit ? 1 : 0);
        ...
        supervisor_log("event=child_start child_pid=%d generation=%d", (int)child,
                       restart_count + 1);
        ...
        for (;;) {
            ...
            mem_hard_bytes = fleet_compute_budget(rss, rss_ewma_mb, rss_peak_mb, fleet_deriv,
                                                  &shrink_hold_ticks);
            ...
            if (rss > 0 && rss >= mem_hard_bytes) {
                ...
                if (pressure < DSCO_PREEMPT_PRESSURE_LEVEL) {
                    if (!soft_warned) {
                        ...
                        fprintf(stderr,
                                ...
                                DSCO_PREEMPT_PRESSURE_LEVEL);
                        supervisor_log(
                            "event=mem_grow child_pid=%d rss_mb=%llu budget_mb=%llu pressure=%d",
                            (int)child, (unsigned long long)(rss >> 20),
                            (unsigned long long)(mem_hard_bytes >> 20), pressure);
                    }
                    sleep_ms(poll_ms);
                ...
                supervisor_log(
                    "event=mem_preempt child_pid=%d rss_mb=%llu budget_mb=%llu pressure=%d",
                    (int)child, (unsigned long long)(rss >> 20),
                    (unsigned long long)(mem_hard_bytes >> 20), pressure);
                preempted = true;
            ...
            if (rss > 0 && rss >= mem_soft_bytes && !soft_warned) {
                ...
                fprintf(stderr,
                        ...
                        (unsigned long long)(rss >> 20), (unsigned long long)(mem_soft_bytes >> 20),
                        (unsigned long long)(mem_hard_bytes >> 20));
                supervisor_log(
                    "event=mem_soft child_pid=%d rss_mb=%llu soft_mb=%llu budget_mb=%llu",
                    (int)child, (unsigned long long)(rss >> 20),
        ...
        if (cls == EXIT_CONFIG) {
            ...
            supervisor_log("event=child_exit child_pid=%d class=%s exit_code=%d restart=never "
                           "peak_rss_mb=%llu",
        ...
        if (cls == EXIT_USER_REQUESTED) {
            if (restart_count > 0)
                fprintf(stderr, "[supervisor] child exited by user request after %d restart(s).\n",
                        restart_count);
            supervisor_log(
                "event=child_exit child_pid=%d class=%s restart=user_exit peak_rss_mb=%llu",
                (int)child, crash_class_name(cls), (unsigned long long)(peak_rss >> 20));
            fleet_remove();
        ...
        if (cls == EXIT_CLEAN) {
            if (restart_type != RT_PERMANENT) {
                if (restart_count > 0)
                    fprintf(stderr, "[supervisor] child exited cleanly after %d restart(s).\n",
                            restart_count);
                supervisor_log(
                    "event=child_exit child_pid=%d class=%s uptime_s=%.1f peak_rss_mb=%llu",
                    (int)child, crash_class_name(cls), now_monotonic() - last_start_time,
                    (unsigned long long)(peak_rss >> 20));
                fleet_remove();
                return 0;
            }
            ...
            fprintf(stderr,
                    "[supervisor] child exited cleanly but restart=permanent — relaunching.\n");
            supervisor_log(
                "event=child_exit child_pid=%d class=%s restart=permanent peak_rss_mb=%llu",
                (int)child, crash_class_name(cls), (unsigned long long)(peak_rss >> 20));
        }
        ...
        if (restart_type == RT_TEMPORARY) {
            ...
            supervisor_log("event=child_exit child_pid=%d class=%s signal=%s exit_code=%d "
                           "restart=temporary peak_rss_mb=%llu",
        ...
        fprintf(stderr, "\n[supervisor] child pid=%d died: %s", (int)child, crash_class_name(cls));
        if (sig > 0)
            fprintf(stderr, " (%s/%d)", signal_name(sig), sig);
        fprintf(stderr, " after %.1fs\n", uptime);
        supervisor_log(
            "event=child_exit child_pid=%d class=%s signal=%s signal_num=%d exit_code=%d "
            "uptime_s=%.1f peak_rss_mb=%llu preempted=%d tracer_reaped=%d pressure=%d",
            (int)child, crash_class_name(cls), signal_name(sig), sig,
            WIFEXITED(status) ? WEXITSTATUS(status) : -1, uptime,
            (unsigned long long)(peak_rss >> 20), preempted ? 1 : 0, tracer_reaped ? 1 : 0,
            system_mem_pressure());
        ...
        if (uptime >= (double)stable_s) {
            if (restart_count > 0) {
                fprintf(stderr,
                        "[supervisor] child was stable for %.0fs — resetting crash counter.\n",
                        uptime);
            }
            restart_count = 0;
            first_crash_time = 0;
        ...
        if (restart_count > max_restarts && window_elapsed < (double)window_s) {
            ...
            supervisor_log(
                "event=crash_loop restarts=%d window_s=%.0f limit=%d last_class=%s last_signal=%s",
                restart_count, window_elapsed, max_restarts, crash_class_name(cls),
                signal_name(sig));
        ...
        fprintf(stderr, "[supervisor] restarting in %dms (attempt %d)...\n", current_backoff,
                restart_count + 1);
        supervisor_log("event=restart delay_ms=%d next_attempt=%d", current_backoff,
                       restart_count + 1);
        sleep_ms(current_backoff);
        current_backoff *= 2;
        if (current_backoff > backoff_max_ms)
            current_backoff = backoff_max_ms;
    }
}
