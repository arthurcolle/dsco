...
bool supervisor_resolve_hotswap_exec(const char *current_exec, const char *explicit_hotswap,
    ...
    if (!out_exec || out_exec_len == 0)
        return false;
    out_exec[0] = '\0';

    if (explicit_hotswap && explicit_hotswap[0] && access(explicit_hotswap, X_OK) == 0) {
        snprintf(out_exec, out_exec_len, "%s", explicit_hotswap);
        return true;
    }

    if (!current_exec || !current_exec[0])
        return false;
    const char *slash = strrchr(current_exec, '/');
    if (!slash)
        return false;

    char staged[PATH_MAX];
    size_t dir_len = (size_t)(slash - current_exec);
    if (dir_len + sizeof("/dsco-new") > sizeof(staged))
        return false;
    memcpy(staged, current_exec, dir_len);
    memcpy(staged + dir_len, "/dsco-new", sizeof("/dsco-new"));
    if (access(staged, X_OK) != 0)
        return false;

    snprintf(out_exec, out_exec_len, "%s", staged);
    return true;
}
...
int supervisor_run(int child_argc, char **child_argv) {
    ...
    for (;;) {
        ...
        if (child == 0) {
             ...
             * The child execs the staged file directly. Replacing child_argv[0]
             * while other sessions are running can trip their tamper monitors;
             * durable replacement belongs to make/install, not supervision. */
            {
                const char *hotswap_env = getenv("DSCO_HOTSWAP_BIN");
                char hotswap_path[PATH_MAX] = {0};

                if (supervisor_resolve_hotswap_exec(child_argv[0], hotswap_env, hotswap_path,
                                                    sizeof(hotswap_path))) {
                    fprintf(stderr, "[supervisor] hotswap: exec staged %s (target remains %s)\n",
                            hotswap_path, child_argv[0]);
                    supervisor_log("event=hotswap_exec staged=%s target=%s", hotswap_path,
                                   child_argv[0]);
