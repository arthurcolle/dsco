...
typedef struct { const char *key; int idx; uint8_t kind; } or_hentry_t;
...
const model_info_t *openrouter_cache_lookup(const char *slug) {
    ...
    if (idx >= 0) return &cat->models[idx].info;
...
static void on_model(const char *elem, void *ctx) {
    ...
    if (pr) {
        ...
        s = json_get_str(pr, "input_cache_write"); if (s) { cache_w = strtod(s, NULL) * 1e6; free(s); }
        free(pr);
    }
    ...
    if (canonical && strcmp(canonical, id) == 0) { free(canonical); canonical = NULL; }
    ...
    if (b->count == b->cap) {
        ...
        if (!nm) { free(id); free(name); free(canonical); return; }
        b->models = nm;
        b->cap = ncap;
    }
...
static or_catalog_t *catalog_from_json(const char *json) {
    ...
    if (cat->buckets) {
        for (int i = 0; i < cat->count; i++) {
            ...
            if (m->canonical) index_insert(cat, m->canonical, i, 0);
            if (m->norm)      index_insert(cat, m->norm, i, 1);
        }
    } else {
