...
static int repeated_prefix_count(const char *s) {
    ...
    for (int period = 2; period <= max_period; period++) {
        ...
        int pos = period;
        while (pos + period <= n) {
            if (memcmp(s, s + pos, (size_t)period) != 0)
                break;
            reps++;
            pos += period;
        }
...
static bool frame_is_path_like(const char *s) {
    ...
    if (non_path_chars * 10 > core_len)
        return false;
    if (slash_count * 100 < core_len * 15)
        return false;
    return true;
}
...
static int diagnostic_fd(void) {
    ...
    if (err->active && err->mirror_fd >= 0)
        return err->mirror_fd;
    og_stream_t *out = &g_og.streams[0];
    if (out->active && out->mirror_fd >= 0)
        return out->mirror_fd;
...
static void output_guard_trip(const og_stream_t *s, const char *reason) {
    ...
    int n =
        snprintf(msg, sizeof(msg),
                 ...
                 s && s->name ? s->name : "unknown", reason ? reason : "repeat flood",
                 g_og.repeat_limit, s ? s->repeat_count : 0, s ? s->repeat_bytes : 0,
                 s ? s->total_bytes : 0, (s && s->last_preview[0]) ? s->last_preview : "(empty)");
    if (n > 0)
        write_all_fd(fd, msg, (size_t)n);
...
static void finalize_frame(og_stream_t *s, char delim) {
    ...
    if (track_repeats) {
        ...
        if (s->repeat_count > g_og.repeat_limit && s->repeat_bytes >= g_og.repeat_min_bytes) {
     ...
     */
    size_t motif_min_bytes =
        g_og.motif_min_bytes > 0 ? g_og.motif_min_bytes : OG_DEFAULT_REPEAT_MIN_BYTES;
    if (motif_min_bytes < g_og.repeat_min_bytes)
        motif_min_bytes = g_og.repeat_min_bytes;
    if (motif_min_bytes < 256)
        motif_min_bytes = 256;
...
bool output_guard_init(void) {
    ...
    if (enabled_env && (strcmp(enabled_env, "0") == 0 || strcasecmp(enabled_env, "false") == 0 ||
