...
__attribute__((weak)) int dsco_mux_spawn_worker(dsco_project_t *p,
                                                const char *api_key);
...
size_t dsco_ring_write(dsco_ring_t *r, const char *data, size_t len) {
    if (!r || !r->buf || r->cap == 0 || !data || len == 0) return 0;
    pthread_mutex_lock(&r->mu);
    if (len >= r->cap) {
        /* keep only tail */
    ...
    if (first >= len) {
        ...
        r->head += len;
        if (r->head == r->cap) { r->head = 0; r->wrapped = true; }
    } else {
    ...
    if (r->wrapped) r->tail = r->head; /* oldest byte moves with head */
    r->total_written += len;
    pthread_mutex_unlock(&r->mu);
    return len;
}
...
size_t dsco_ring_snapshot(dsco_ring_t *r, char *out, size_t out_cap) {
    ...
    if (!r->wrapped) {
        ...
        size_t total = a + b;
        size_t skip = 0;
        if (total >= out_cap) skip = total - (out_cap - 1);
        n = 0;
        if (skip < a) {
...
const char *dsco_project_registry_root(void) {
    if (s_registry_root[0] == '\0') {
        snprintf(s_registry_root, sizeof(s_registry_root),
                 "%s/.dsco/projects", home_dir());
    }
...
static int read_toml(const char *id, dsco_project_t *p) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        unquote(v);
        if      (!strcmp(k, "id"))         snprintf(p->id.id, sizeof(p->id.id), "%s", v);
        else if (!strcmp(k, "name"))       snprintf(p->id.name, sizeof(p->id.name), "%s", v);
        else if (!strcmp(k, "root"))       snprintf(p->id.root, sizeof(p->id.root), "%s", v);
        else if (!strcmp(k, "workspace"))  snprintf(p->id.workspace, sizeof(p->id.workspace), "%s", v);
        else if (!strcmp(k, "model"))      snprintf(p->model, sizeof(p->model), "%s", v);
        else if (!strcmp(k, "created_at")) p->created_at = (int64_t)strtoll(v, NULL, 10);
        else if (!strcmp(k, "started_at")) p->started_at = (int64_t)strtoll(v, NULL, 10);
        else if (!strcmp(k, "token_cap"))  p->budget.token_cap = (uint64_t)strtoull(v, NULL, 10);
        else if (!strcmp(k, "cents_cap"))  p->budget.cents_cap = (uint64_t)strtoull(v, NULL, 10);
        else if (!strcmp(k, "tokens_used")) p->budget.tokens_used = (uint64_t)strtoull(v, NULL, 10);
        else if (!strcmp(k, "cents_spent")) p->budget.cents_spent = (uint64_t)strtoull(v, NULL, 10);
        else if (!strcmp(k, "allow_global_fs")) p->policy.allow_global_fs = !strcmp(v, "true");
        else if (!strcmp(k, "allow_network"))   p->policy.allow_network   = !strcmp(v, "true");
        else if (!strcmp(k, "allow_shell"))     p->policy.allow_shell     = !strcmp(v, "true");
    }
    fclose(f);
    return 0;
}
...
static const char *index_path(void) {
    ...
    if (!p[0]) snprintf(p, sizeof(p), "%s/registry.idx", dsco_project_registry_root());
    return p;
}

static int index_upsert(const dsco_project_t *p, bool archived) {
    if (dsco_project_registry_ensure() != 0) return -1;
    ...
    if (in) {
        ...
        while (fgets(line, sizeof(line), in)) {
            if (!strncmp(line, p->id.id, strlen(p->id.id)) &&
                line[strlen(p->id.id)] == '\t') {
                replaced = true;
...
int dsco_project_create(const char *root, const char *name, dsco_project_t **out) {
    ...
    if (root[0] == '/') {
        ...
        char cwd[PATH_MAX];
        if (!getcwd(cwd, sizeof(cwd))) { free(p); return -1; }
...
static int load_from_id(const char *id, dsco_project_t **out) {
    dsco_project_t *p = (dsco_project_t *)calloc(1, sizeof(*p));
    if (!p) return -1;
    if (read_toml(id, p) != 0) { free(p); return -1; }
    p->state = DSCO_PROJECT_IDLE;
    p->worker_pid = -1;
    p->worker_in_fd = -1;
    p->worker_out_fd = -1;
...
int dsco_project_open(const char *key, dsco_project_t **out) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        char *name = t1 + 1;
        char *t2 = strchr(name, '\t'); if (!t2) continue; *t2 = '\0';
        char *root = t2 + 1;
        char *t3 = strchr(root, '\t'); if (t3) *t3 = '\0';
...
int dsco_project_archive(const char *id) {
    ...
    if (load_from_id(id, &p) != 0) return -1;
...
void dsco_project_free(dsco_project_t *p) {
    ...
    if (p->worker_in_fd  >= 0) close(p->worker_in_fd);
    if (p->worker_out_fd >= 0) close(p->worker_out_fd);
    free(p);
}

int dsco_project_close(dsco_project_t *p) {
    if (!p) return -1;
    if (p->worker_pid > 0) dsco_project_kill(p);
    dsco_project_save(p);
    dsco_project_free(p);
    return 0;
}
...
int dsco_project_list(dsco_project_summary_t *out, int max, bool include_archived) {
    ...
    while (n < max && fgets(line, sizeof(line), f)) {
        ...
        char *t2 = strchr(name, '\t'); if (!t2) continue; *t2 = '\0';
        char *root = t2 + 1;
        char *t3 = strchr(root, '\t'); if (!t3) continue; *t3 = '\0';
        char *flags = t3 + 1;
        char *t4 = strchr(flags, '\t'); if (t4) *t4 = '\0';
        bool archived = strstr(flags, "archived") != NULL;
        if (archived && !include_archived) continue;
...
int dsco_project_start(dsco_project_t *p, const char *api_key) {
    if (!p) return -1;
    if (p->state == DSCO_PROJECT_RUNNING) return 0;
...
int dsco_project_pause(dsco_project_t *p) {
    if (!p || p->worker_pid <= 0) return -1;
    if (kill(p->worker_pid, SIGSTOP) != 0) return -1;
...
int dsco_project_resume(dsco_project_t *p) {
    if (!p || p->worker_pid <= 0) return -1;
    if (kill(p->worker_pid, SIGCONT) != 0) return -1;
    p->state = DSCO_PROJECT_RUNNING;
    p->epoch++;
    return 0;
}
...
int dsco_project_send_input(dsco_project_t *p, const char *line) {
    if (!p || p->worker_in_fd < 0 || !line) return -1;
    size_t n = strlen(line);
    if (write(p->worker_in_fd, line, n) < 0) return -1;
    if (n == 0 || line[n - 1] != '\n')
...
int dsco_project_drain_output(dsco_project_t *p) {
    if (!p || p->worker_out_fd < 0) return 0;
    char buf[4096];
    int total = 0;
    for (;;) {
        ...
        if (n > 0) {
            ...
            total += (int)n;
            if (n < (ssize_t)sizeof(buf)) break;  /* drained for now */
        } else if (n == 0) {
            /* EOF — worker exited */
            p->state = DSCO_PROJECT_DEAD;
            p->epoch++;
...
double dsco_project_activity_bps(dsco_project_t *p) {
    if (!p) return 0.0;
    int64_t now = now_ms();
    int64_t dt_ms = (p->activity_last_sample_ms > 0)
                    ? (now - p->activity_last_sample_ms) : 100;
    if (dt_ms <= 0) dt_ms = 1;
    p->activity_last_sample_ms = now;

    unsigned long long bytes = atomic_exchange_explicit(
        &p->activity_bytes, 0ULL, memory_order_acq_rel);
...
const char *dsco_project_state_name(dsco_project_state_t s) {
    switch (s) {
        case DSCO_PROJECT_IDLE:        return "IDLE";
        case DSCO_PROJECT_RUNNING:     return "RUNNING";
        case DSCO_PROJECT_PAUSED:      return "PAUSED";
        case DSCO_PROJECT_QUARANTINED: return "QUARANTINED";
        case DSCO_PROJECT_CLOSED:      return "CLOSED";
        case DSCO_PROJECT_DEAD:        return "DEAD";
    }
    return "?";
}
