...
static memory_tier_t tier_from_ttl(int ttl) {
    if (ttl == 0) return MEM_SEMANTIC;
    if (ttl <= SESSION_TTL_WORKING) return MEM_WORKING;
    return MEM_EPISODIC;
}
...
static memory_tier_t tier_from_name(const char *name) {
    if (!name) return MEM_EPISODIC;
    if (strcmp(name, "working")  == 0) return MEM_WORKING;
    if (strcmp(name, "semantic") == 0) return MEM_SEMANTIC;
    return MEM_EPISODIC;
}
...
static void serialize_db(const session_db_t *db, jbuf_t *out) {
    ...
    for (int i = 0; i < db->kv_count; i++) {
        ...
        jbuf_appendf(out,
            ...
            ",\"access_count\":%d}",
            tier_name(kv->tier), kv->ttl_seconds,
            kv->created_at, kv->accessed_at, kv->access_count);
    }
    ...
    for (int i = 0; i < db->record_count; i++) {
        ...
        jbuf_appendf(out,
            ...
            tier_name(rec->tier), rec->created_at,
            rec->accessed_at, rec->access_count);
    }
    jbuf_append(out, "]}");
}
...
typedef struct { session_db_t *db; } parse_ctx_t;

static void parse_kv_entry(const char *elem, void *ctx) {
    session_db_t *db = ((parse_ctx_t *)ctx)->db;
    if (db->kv_count >= SESSION_MAX_KV) return;

    /* Find inactive slot or extend array */
    int slot = -1;
    for (int i = 0; i < db->kv_count; i++) {
        if (!db->kv[i].active) { slot = i; break; }
    }
    if (slot < 0) slot = db->kv_count++;
    ...
    if (!key || !val) { free(key); free(val); free(tier); db->kv_count--; return; }
    ...
    free(key); free(val); free(tier);
}

static void parse_session_entry(const char *elem, void *ctx) {
    session_db_t *db = ((parse_ctx_t *)ctx)->db;
    if (db->record_count >= SESSION_MAX_RECORDS) return;

    int slot = -1;
    for (int i = 0; i < db->record_count; i++) {
        if (!db->records[i].active) { slot = i; break; }
    }
    if (slot < 0) slot = db->record_count++;
    ...
    char *tier = json_get_str(elem, "tier");
    if (!id || !task) { free(id); free(task); free(kv); free(tier); db->record_count--; return; }
    ...
    free(id); free(task); free(kv); free(tier);
}

static int load_db(session_db_t *db) {
    FILE *f = fopen(db->db_path, "rb");
    if (!f) return 0; /* no file yet — fresh DB */
...
int session_remember(session_db_t *db, const char *key,
    ...
    if (db->kv_count >= SESSION_MAX_KV)
        session_evict_expired(db);
    if (db->kv_count >= SESSION_MAX_KV) return -1;
...
const char *session_recall(session_db_t *db, const char *key) {
    if (!db || !db->initialized || !key) return NULL;
...
static int tokenize_words(const char *text, char words[][32], int max_words) {
    ...
    while (*p && n < max_words) {
        ...
        if (!dup) snprintf(words[n++], 32, "%s", word);
    }
    return n;
}
...
int session_lookup_similar(session_db_t *db, const char *task,
    ...
    if (!db || !db->initialized || !task || !out || k <= 0) return 0;
    ...
    for (int i = 0; i < db->record_count; i++) {
        const session_record_t *rec = &db->records[i];
        if (!rec->active) continue;
        if (strcmp(rec->id, db->current_session_id) == 0) continue;
...
int session_promote(session_db_t *db) {
    if (!db || !db->initialized) return 0;

    int promoted = 0;
    for (int i = 0; i < db->kv_count; i++) {
        session_kv_t *kv = &db->kv[i];
        if (!kv->active || kv->tier == MEM_SEMANTIC) continue;

        if (kv->access_count < SESSION_PROMOTE_THRESHOLD) continue;
...
int session_persist(session_db_t *db) {
    ...
    if (!f) { jbuf_free(&buf); return -1; }

    size_t written = fwrite(buf.data, 1, buf.len, f);
    fclose(f);
    jbuf_free(&buf);

    if (written == 0) { remove(tmp_path); return -1; }
...
int session_evict_expired(session_db_t *db) {
    ...
    for (int i = 0; i < db->kv_count; i++) {
        ...
        if (!kv->active) continue;
        if (kv->ttl_seconds > 0 &&
            (t - kv->created_at) > (double)kv->ttl_seconds) {
            kv->active = false;
            evicted++;
            db->dirty = true;
        }
...
int session_status_json(const session_db_t *db, char *buf, size_t len) {
    ...
    for (int i = 0; i < db->kv_count; i++) {
        if (!db->kv[i].active) continue;
        kv_active++;
        if ((int)db->kv[i].tier < MEM_TIER_COUNT)
            by_tier[(int)db->kv[i].tier]++;
    }
    for (int i = 0; i < db->record_count; i++) {
        if (db->records[i].active) rec_active++;
    }
    ...
    return snprintf(buf, len,
        ...
        kv_active, rec_active,
        by_tier[MEM_WORKING], by_tier[MEM_EPISODIC], by_tier[MEM_SEMANTIC],
