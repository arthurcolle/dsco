...
static const char *s_pinned[] = {
    "bash", "read_file", "discover_tools", "context_recall", NULL
};
...
static orch_domain_t classify_tool(const char *name) {
    /* Git */
    if (strncmp(name, "git_", 4) == 0)                   return ORCH_DOMAIN_GIT;

    /* Trading execution */
    if (strncmp(name, "kalshi_", 7) == 0 ||
        strncmp(name, "polymarket_", 11) == 0 ||
        strncmp(name, "arb_", 4) == 0)                   return ORCH_DOMAIN_TRADING;

    /* Market data */
    if (strncmp(name, "prediction_", 11) == 0 ||
        strncmp(name, "market_", 7) == 0)                return ORCH_DOMAIN_MARKET;

    /* Wings / governance / immune */
    if (strncmp(name, "pheromone_", 10) == 0 ||
        strncmp(name, "ooda_", 5) == 0 ||
        strncmp(name, "killswitch_", 11) == 0 ||
        strncmp(name, "governance_", 11) == 0 ||
        strncmp(name, "memory_", 7) == 0 ||
        strncmp(name, "talons_", 7) == 0 ||
        strncmp(name, "legion_", 7) == 0)               return ORCH_DOMAIN_WINGS;

    /* Web / network */
    if (strcmp(name, "web_extract")    == 0 ||
        strcmp(name, "http_request")   == 0 ||
        strcmp(name, "json_api")       == 0 ||
        strcmp(name, "curl_raw")       == 0 ||
        strcmp(name, "dns_lookup")     == 0 ||
        strcmp(name, "port_scan")      == 0 ||
        strcmp(name, "port_check")     == 0 ||
        strcmp(name, "ping")           == 0 ||
        strcmp(name, "web_search")     == 0 ||
        strcmp(name, "web_screenshot") == 0 ||
        strncmp(name, "browser_", 8)   == 0 ||
        strncmp(name, "research_", 9)  == 0)            return ORCH_DOMAIN_WEB;

    /* Code / execution */
    if (strcmp(name, "compile")        == 0 ||
        strcmp(name, "run_command")    == 0 ||
        strcmp(name, "run_background") == 0 ||
        strcmp(name, "sandbox_run")    == 0 ||
        strncmp(name, "code_", 5)      == 0 ||
        strcmp(name, "debug_trace")    == 0)            return ORCH_DOMAIN_CODE;

    /* System */
    if (strcmp(name, "ps")             == 0 ||
        strcmp(name, "kill_process")   == 0 ||
        strncmp(name, "env_", 4)       == 0 ||
        strcmp(name, "sysinfo")        == 0 ||
        strcmp(name, "disk_usage")     == 0 ||
        strcmp(name, "which")          == 0 ||
        strcmp(name, "cwd")            == 0 ||
        strcmp(name, "whoami")         == 0 ||
        strcmp(name, "uptime")         == 0 ||
        strcmp(name, "open_url")       == 0 ||
        strncmp(name, "clipboard_", 10) == 0 ||
        strcmp(name, "sleep_ms")       == 0)            return ORCH_DOMAIN_SYSTEM;
    ...
    if (strncmp(name, "file_", 5)      == 0 ||
        ...
        strcmp(name, "grep_files")     == 0 ||
        strcmp(name, "list_dir")       == 0 ||
        strcmp(name, "find_and_replace") == 0 ||
        strcmp(name, "diff")           == 0 ||
        strcmp(name, "patch")          == 0 ||
        strcmp(name, "copy_file")      == 0 ||
        strcmp(name, "move_file")      == 0 ||
        strcmp(name, "delete_file")    == 0)            return ORCH_DOMAIN_FILE;

    /* Text processing */
    if (strcmp(name, "sed")            == 0 ||
        strcmp(name, "awk")            == 0 ||
        strcmp(name, "sort_uniq")      == 0 ||
        strcmp(name, "jq")             == 0 ||
        strcmp(name, "tar")            == 0 ||
        strcmp(name, "zip")            == 0 ||
        strncmp(name, "csv_", 4)       == 0 ||
        strcmp(name, "regex_match")    == 0 ||
        strcmp(name, "json_format")    == 0 ||
        strcmp(name, "sed_transform")  == 0 ||
        strcmp(name, "awk_process")    == 0 ||
        strncmp(name, "url_", 4)       == 0 ||
        strcmp(name, "semver_compare") == 0 ||
        strcmp(name, "cron_parse")     == 0 ||
        strcmp(name, "mapbox_geocode") == 0)            return ORCH_DOMAIN_TEXT;
...
static bool orch_chat_filter(const char *name, const char *group_hint) {
    (void)group_hint;
    return strcmp(name, "dispatch_agent")    == 0 ||
           strcmp(name, "dispatch_topology") == 0 ||
           strcmp(name, "dispatch_tools")    == 0 ||
           strcmp(name, "list_domains")      == 0;
}
...
static bool orch_worker_filter(const char *name, const char *group_hint) {
    (void)group_hint;
    if (g_worker_domain == ORCH_DOMAIN_GENERAL) return true;
    for (int i = 0; s_pinned[i]; i++)
        if (strcmp(name, s_pinned[i]) == 0) return true;
    ...
    for (int turn = 0; turn < ORCH_WORKER_MAX_TURNS; turn++) {
        ...
        if (session.context_window > 0)
            session.tool_budget_ratio = 1.0f -
                (float)session.total_input_tokens / (float)session.context_window;
...
static char *run_virtual_tool(const char *task, const char *tool_name) {
    ...
    const char *key = provider_resolve_request_api_key(
        provider_route_for_model("claude-haiku-4-5-20251001", api_key, NULL),
        api_key);
    if (!key || !key[0]) key = api_key;
...
static char *dispatch_virtual_tools_cb(const char *name, const char *input_json, void *ctx) {
    ...
    while (*p) {
        ...
        if (tool && task && tool[0] && task[0]) {
            ...
            if (call_count > 0) jbuf_append(&result, ",");
...
static char *list_domains_cb(const char *name, const char *input_json, void *ctx) {
    (void)name; (void)input_json; (void)ctx;
    ...
    jbuf_appendf(&b,
        ...
        "  %-16s  %s\n"
        "  %-16s  %s\n",
        "clinic",        "Intake→Plan→Implement→Validate→Judge (code tasks)",
        "research",      "Plan→Gather(scouts)→Analyze→Synthesize (info gathering)",
        "code_review",   "Lint→Logic→Security→Verdict (review tasks)",
        "sentinel",      "Triage(H)→Analyze(S)→Decide(O) (quick decisions)",
        "switchboard",   "Classify→Route to specialists→Integrate (routing)",
        "critic_loop",   "Generate→Critique→Refine, repeat 3x (quality)",
        "tournament",    "N candidates compete, judge picks (best-of-N)",
        "starburst",     "Coordinator fans out to parallel workers→merge");
...
static orch_domain_t domain_from_str(const char *s) {
    if (!s || !s[0]) return ORCH_DOMAIN_GENERAL;
    for (int i = 0; i < ORCH_DOMAIN_COUNT; i++)
...
static char *dispatch_agent_cb(const char *name, const char *input_json, void *ctx) {
    ...
    if (!task || !task[0]) {
        free(domain_str); free(task); free(model_alias);
...
static char *dispatch_topology_cb(const char *name, const char *input_json, void *ctx) {
    ...
    bool have_plan = topology_plan_build(
        topo_name, auto_mode || (!topo_name || !topo_name[0]),
        task, &plan);
    ...
    fprintf(stderr, "  \033[36m[orch]\033[0m topology → %s (%d nodes, %s%s)\n",
            ...
            plan.is_dynamic ? "dynamic" : "static",
            plan.rationale[0] ? ", " : "");
    if (plan.rationale[0])
    ...
    bool ok = topology_plan_run(&plan, api_key, g_worker_model,
                                 task, result_buf, ORCH_RESULT_BUF, &stats);
    ...
    fprintf(stderr, "  \033[36m[orch]\033[0m topology done: %d iterations, "
            "%d nodes, %d agents, ~$%.4f\n",
            stats.iterations, stats.nodes_executed,
            stats.agents_spawned, stats.est_cost_usd);
...
static const char s_dispatch_schema[] =
    ...
    "\"properties\":{"
      ...
      "\"task\":{"
        "\"type\":\"string\","
        "\"description\":\"Complete, self-contained task description. Include ALL context the worker needs — it has no access to our conversation history.\""
      "},"
      "\"model\":{"
        "\"type\":\"string\","
...
static const char s_topology_schema[] =
    ...
    "\"properties\":{"
      ...
      "\"auto\":{"
        "\"type\":\"boolean\","
        "\"description\":\"Auto-select best topology for the task (default: true if no topology specified).\""
      "}"
    "},"
    "\"required\":[\"task\"]"
    "}";
...
static const char s_vtool_schema[] =
    ...
    "\"properties\":{"
      "\"calls\":{"
        "\"type\":\"array\","
        "\"description\":\"Array of tool invocations. Each is a Haiku micro-agent with exactly 1 tool.\","
        "\"items\":{"
          "\"type\":\"object\","
          "\"properties\":{"
            "\"tool\":{\"type\":\"string\",\"description\":\"Exact tool name to invoke (e.g. read_file, git_status, compile)\"},"
            "\"task\":{\"type\":\"string\",\"description\":\"Natural language description of what to do with this tool.\"}"
          "},"
          "\"required\":[\"tool\",\"task\"]"
        "}"
      "}"
...
static const char s_list_domains_schema[] =
...
void agent_run_orchestrated(const char *api_key,
    ...
    tools_register_external(
        ...
        s_vtool_schema,
        dispatch_virtual_tools_cb, NULL);

    tools_register_external(
        "list_domains",
        "List available worker domains, topology patterns, models, and pricing.",
        s_list_domains_schema,
        list_domains_cb, NULL);
