...
const char *toolmgmt_base_url(void) {
    if (s_url_override[0]) return s_url_override;
...
const char *toolmgmt_token(void) {
    if (s_tok_override[0]) return s_tok_override;
...
void toolmgmt_set_base_url(const char *url) {
    if (url) snprintf(s_url_override, sizeof(s_url_override), "%s", url);
}
void toolmgmt_set_token(const char *token) {
    if (token) snprintf(s_tok_override, sizeof(s_tok_override), "%s", token);
}

static long tm_timeout_secs(void) {
    const char *e = getenv("TOOLS_API_TIMEOUT");
    if (e && e[0]) { long v = atol(e); if (v > 0) return v; }
    return 60;
}
...
static void tm_curl_global_init(void) { curl_global_init(CURL_GLOBAL_DEFAULT); }

/* ── HTTP ──────────────────────────────────────────────────────────────── */

typedef struct { char *data; size_t len, cap; } tm_buf_t;
...
static long tm_request_once(const char *method, const char *path,
    ...
    buf.cap = 4096;
    if (!buf.data) { curl_easy_cleanup(c); return -1; }
    buf.data[0] = '\0';
    ...
    if (out) *out = buf.data; else free(buf.data);
    return code;
}
...
char *toolmgmt_list_tools(int limit) {
    ...
    snprintf(path, sizeof(path),
             "/api/v1/tools?limit=%d", limit > 0 ? limit : 1000);
    char *out = NULL;
    long st = toolmgmt_request("GET", path, NULL, &out);
    if (st >= 200 && st < 300) return out;
    free(out);
    return NULL;
}
...
int toolmgmt_parallel(tm_call_t *calls, int n, int max_concurrency) {
    ...
    if (!th) { /* fall back to serial */ tm_worker(&pool); }
    else {
        int spawned = 0;
        for (int i = 0; i < max_concurrency; i++)
            if (pthread_create(&th[i], NULL, tm_worker, &pool) == 0) spawned++;
        if (spawned == 0) tm_worker(&pool);  /* none spawned: run inline */
    ...
    int ok = 0;
    for (int i = 0; i < n; i++)
        if (calls[i].status >= 200 && calls[i].status < 300) ok++;
    return ok;
}
...
static void tm_reg_cb(const char *el, void *ctx) {
    ...
    tools_register_external(name, desc ? desc : "", schema.data, tm_external_cb, NULL);
    rc->count++;
...
static void kv_to_json(jbuf_t *b, char **pairs, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        *eq = '\0';
        ...
        jbuf_append(b, ":");
        if (is_json_scalar(val)) jbuf_append(b, val);
...
static int tm_cli_list(int argc, char **argv) {
    ...
    for (int i = 0; i < argc; i++)
        if (strcmp(argv[i], "--limit") == 0 && i + 1 < argc) limit = atoi(argv[i + 1]);
    char path[128];
    snprintf(path, sizeof(path),
             "/api/v1/tools?limit=%d", limit > 0 ? limit : 1000);
    char *body = NULL;
...
static int tm_cli_run(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--timeout-ms") == 0 && i + 1 < argc) { timeout_ms = atoi(argv[++i]); continue; }
        if (kvn < 256) kv[kvn++] = argv[i];
    }
    jbuf_t args;
    jbuf_init(&args, 256);
    ...
    jbuf_free(&args);
    if (r) { printf("%s\n", r); free(r); }
    else fprintf(stderr, "tools run: request to %s failed\n", toolmgmt_base_url());
...
static int tm_cli_batch(int argc, char **argv) {
    ...
    while (i < argc && ncalls < 64) {
        ...
        if (i < argc && strcmp(argv[i], "--") == 0) i++;  /* skip separator */
...
static int tm_cli_plan(int argc, char **argv) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        else if (strcmp(argv[i], "--max-steps") == 0 && i + 1 < argc) max_steps = atoi(argv[++i]);
        else if (!query) query = argv[i];
    }
    if (!query) { fprintf(stderr, "usage: dsco tools plan \"<query>\" [--intent X] [--max-steps N]\n"); return 2; }
...
int toolmgmt_cli(int argc, char **argv) {
    ...
    for (int i = 2; i < argc && rn < 256; i++) {
        if (strcmp(argv[i], "--tm-url") == 0 && i + 1 < argc) { toolmgmt_set_base_url(argv[++i]); continue; }
        if (strcmp(argv[i], "--tm-token") == 0 && i + 1 < argc) { toolmgmt_set_token(argv[++i]); continue; }
        rest[rn++] = argv[i];
    }
    ...
    if (strcmp(rest[0], "register") == 0) {
        ...
        if (c < 0) { fprintf(stderr, "register: catalog fetch from %s failed\n", toolmgmt_base_url()); return 1; }
        printf("registered %d remote tools as dsco external tools\n", c);
        return 0;
    }
    if (strcmp(rest[0], "-h") == 0 || strcmp(rest[0], "--help") == 0) { tm_cli_usage(); return 0; }
