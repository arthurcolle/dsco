...
#define CAP_OPENAI_COMPAT                                                                          \
    (PROVIDER_CAP_TOOLS | PROVIDER_CAP_MULTITURN | PROVIDER_CAP_STREAMING | PROVIDER_CAP_JSON |    \
...
#define CAP_ANTHROPIC                                                                              \
    (PROVIDER_CAP_TOOLS | PROVIDER_CAP_MULTITURN | PROVIDER_CAP_STREAMING | PROVIDER_CAP_VISION |  \
     PROVIDER_CAP_VISION_TOOL_MESSAGES | PROVIDER_CAP_REASONING | PROVIDER_CAP_JSON |              \
     PROVIDER_CAP_PROMPT_CACHE | PROVIDER_CAP_HEALTH_CHECK)
...
static const provider_profile_t PROVIDER_PROFILES[] = {
    ...
    },
    {
        ...
        .description = "Z.AI/GLM Coding Plan OpenAI-compatible API.",
        .api_mode = PROVIDER_API_CHAT_COMPLETIONS,
        .auth_type = PROVIDER_AUTH_API_KEY,
        .transport = PROVIDER_TRANSPORT_OPENAI_CHAT,
        .base_url = "https://api.z.ai/api/coding/paas/v4",
        .transport_base_url = "https://api.z.ai/api/coding/paas/v4",
        .env_vars = {"GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY", "ZAI_CODING_PLAN_API_KEY",
    ...
    },
    {
        ...
        .description = "Sakana Fugu multi-agent orchestration system (OpenAI-compatible Chat "
                       "Completions; Codex profile uses Responses API).",
        .api_mode = PROVIDER_API_CHAT_COMPLETIONS,
        .auth_type = PROVIDER_AUTH_API_KEY,
        .transport = PROVIDER_TRANSPORT_OPENAI_CHAT,
    ...
    },
    {
        ...
        .caps = PROVIDER_CAP_TOOLS | PROVIDER_CAP_MULTITURN | PROVIDER_CAP_STREAMING |
                PROVIDER_CAP_VISION | PROVIDER_CAP_JSON,
    },
    {
        .name = "copilot",
...
bool provider_profile_has_env_var(const provider_profile_t *profile, const char *env_var) {
    if (!profile || !env_var || !env_var[0])
        return false;
    for (int i = 0; i < PROVIDER_PROFILE_MAX_ENV_VARS && profile->env_vars[i]; i++) {
        if (strcmp(profile->env_vars[i], env_var) == 0)
            return true;
    }
    return false;
}
...
const provider_profile_t *provider_profile_find(const char *name_or_alias) {
    ...
    for (size_t i = 0; i < provider_profile_count(); i++) {
        ...
        if (strcmp(profile->name, name_or_alias) == 0)
            return profile;
        if (provider_profile_has_alias(profile, name_or_alias))
            return profile;
    }
    return NULL;
}
...
const char *provider_api_mode_name(provider_api_mode_t mode) {
    switch (mode) {
        case PROVIDER_API_CHAT_COMPLETIONS:
            return "chat_completions";
        case PROVIDER_API_ANTHROPIC_MESSAGES:
            return "anthropic_messages";
        case PROVIDER_API_CODEX_RESPONSES:
            return "codex_responses";
        case PROVIDER_API_BEDROCK_CONVERSE:
            return "bedrock_converse";
        case PROVIDER_API_ACP:
            return "acp";
        case PROVIDER_API_EXTERNAL_PROCESS:
...
const char *provider_auth_type_name(provider_auth_type_t auth_type) {
    switch (auth_type) {
        case PROVIDER_AUTH_API_KEY:
            return "api_key";
        case PROVIDER_AUTH_OAUTH_DEVICE_CODE:
            return "oauth_device_code";
        case PROVIDER_AUTH_OAUTH_EXTERNAL:
            return "oauth_external";
        case PROVIDER_AUTH_COPILOT:
            return "copilot";
        case PROVIDER_AUTH_AWS_SDK:
            return "aws_sdk";
        case PROVIDER_AUTH_EXTERNAL_PROCESS:
            return "external_process";
        case PROVIDER_AUTH_NONE:
            return "none";
    }
    return "unknown";
}
...
const char *provider_transport_kind_name(provider_transport_kind_t transport) {
    switch (transport) {
        case PROVIDER_TRANSPORT_NONE:
            return "none";
        case PROVIDER_TRANSPORT_OPENAI_CHAT:
            return "openai_chat";
        case PROVIDER_TRANSPORT_ANTHROPIC_MESSAGES:
            return "anthropic_messages";
        case PROVIDER_TRANSPORT_CODEX_RESPONSES:
            return "codex_responses";
        case PROVIDER_TRANSPORT_BEDROCK_CONVERSE:
            return "bedrock_converse";
        case PROVIDER_TRANSPORT_ACP:
            return "acp";
        case PROVIDER_TRANSPORT_EXTERNAL_PROCESS:
            return "external_process";
    }
    return "unknown";
}
