...
bool dsco_image_downscale_jpeg(const char *in_path, int max_dim,
    ...
    if (!pix) return false;                 /* undecodable → caller falls back */
    ...
    if (max_dim > 0 && longest > max_dim) {
        ...
        nw = (int)((double)w * s); if (nw < 1) nw = 1;
        nh = (int)((double)h * s); if (nh < 1) nh = 1;
    }
    ...
    if (nw != w || nh != h) {
        ...
        stbir_pixel_layout layout = (comp == 1) ? STBIR_1CHANNEL
                                  : (comp == 3) ? STBIR_RGB
                                                : STBIR_RGBA;
        unsigned char *r = stbir_resize_uint8_srgb(pix, w, h, 0,
                                                   resized, nw, nh, 0, layout);
        if (!r) { free(resized); stbi_image_free(pix); return false; }
        resized_owned = true;
    }
