...
static void entry_path(char *buf, size_t len, uint64_t task_hash) {
    snprintf(buf, len, "%s/.dsco/cache/plan_%016llx.json",
             home(), (unsigned long long)task_hash);
}
...
static char *normalize(const char *task) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (c == ' ' || c == '\t' || c == '\n' || c == '\r') {
            if (!last_sp && ni > 0) { out[ni++] = ' '; last_sp = true; }
        } else {
            out[ni++] = c; last_sp = false;
        }
    }
    out[ni] = '\0';
    return out;
...
static int build_ngrams(const char *task, uint32_t **out) {
    *out = NULL;
    ...
    for (int i = 1; i < max_g; i++) {
        uint32_t k = g[i]; int j = i - 1;
        while (j >= 0 && g[j] > k) { g[j+1] = g[j]; j--; }
        g[j+1] = k;
    }
    *out = g;
    return max_g;
...
static int lru_victim(void) {
    ...
    for (int i = 0; i < PLAN_CACHE_MAX; i++) {
        if (!s_cache.entries[i].occupied) { return i; } /* empty slot preferred */
        plan_cache_entry_t *e = &s_cache.entries[i];
        if (e->last_used < oldest ||
            (e->last_used == oldest && e->hit_count < min_hits)) {
            oldest   = e->last_used;
...
static void load_locked(void) {
    ...
    while (fgets(line, sizeof(line), f)) {
        if (line[0] != '{') continue;
        if (s_cache.count >= PLAN_CACHE_MAX) break;

        int slot = lru_victim();
        if (slot < 0) break;
        if (s_cache.entries[slot].occupied) evict_entry(slot);
        ...
        if (jsget_num(line, "fit",  &dval)) e->fit_score = (float)dval;
        if (jsget_num(line, "hits", &dval)) e->hit_count = (int)dval;
        if (jsget_num(line, "lu",   &dval)) e->last_used = (time_t)dval;
        if (jsget_num(line, "cr",   &dval)) e->created   = (time_t)dval;
        if (jsget_num(line, "th",   &dval)) {} /* hash also in file; we recompute */
        ...
        if (!e->task_text[0] || !e->topology_name[0]) continue;
...
static void persist_locked(void) {
    ...
    for (int i = 0; i < PLAN_CACHE_MAX; i++) {
        ...
        fprintf(f,
            ...
            (unsigned long long)e->task_hash,
            esc_tx, esc_topo, esc_rat,
            e->fit_score, e->hit_count,
            (long long)e->last_used, (long long)e->created);
    }
    fclose(f);
    rename(tmp, path);
}
...
static void persist_plan_json(const plan_cache_entry_t *e) {
    if (!e || !e->plan_json || !e->task_hash) return;
    ensure_cache_dir();
    char path[512];
    entry_path(path, sizeof(path), e->task_hash);
    FILE *f = fopen(path, "w");
...
static char *load_plan_json(uint64_t task_hash) {
    ...
    long sz = ftell(f);
    fseek(f, 0, SEEK_SET);
    if (sz <= 0 || sz > 4 * 1024 * 1024) { fclose(f); return NULL; } /* 4 MB cap */
...
int plan_cache_stats_json(char *buf, size_t buflen) {
    ...
    for (int i = 0; i < PLAN_CACHE_MAX; i++) {
        if (!s_cache.entries[i].occupied) continue;
        total++;
        total_hits += s_cache.entries[i].hit_count;
        if (s_cache.entries[i].plan_json) with_json++;
    }
    ...
    int n = snprintf(buf, buflen,
        ...
        total, total_hits, with_json,
        PLAN_CACHE_MAX, PLAN_CACHE_MIN_SIM, hit_rate);
...
float plan_similarity_score(const char *task_a, const char *task_b) {
    if (!task_a || !task_b) return 0.0f;
    if (strcmp(task_a, task_b) == 0) return 1.0f;
...
static int extract_entities(const char *text, char ents[][32], int max) {
    ...
    while (*p && count < max) {
        ...
        bool all_up = true;
        for (int i = 0; i < ti; i++) {
            if (islower((unsigned char)tok[i])) { all_up = false; break; }
        }
        /* Must have at least one alpha char */
        bool has_alpha = false;
        for (int i = 0; i < ti; i++) {
            if (isalpha((unsigned char)tok[i])) { has_alpha = true; break; }
        }
        if (all_up && has_alpha) {
            /* Deduplicate */
            bool dup = false;
            for (int d = 0; d < count; d++) {
                if (strcmp(ents[d], tok) == 0) { dup = true; break; }
            }
            if (!dup) strncpy(ents[count++], tok, 31);
        }
    }
    return count;
}
...
static char *str_replace_all(const char *src, const char *from, const char *to) {
    if (!src || !from || !*from) return strdup(src ? src : "");
...
char *plan_cache_adapt(const plan_cache_entry_t *entry, const char *new_task) {
    ...
    for (int i = 0; i < n_pairs; i++) {
        if (strcmp(old_ents[i], new_ents[i]) == 0) continue;
