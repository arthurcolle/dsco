...
#define dsco_cpuidex(leaf, subleaf, a, b, c, d)                                                    \
    do {                                                                                           \
        int __r[4];                                                                                \
        __cpuidex(__r, (int)(leaf), (int)(subleaf));                                               \
        (a) = __r[0];                                                                              \
        (b) = __r[1];                                                                              \
        (c) = __r[2];                                                                              \
        (d) = __r[3];                                                                              \
...
#define dsco_cpuidex(leaf, subleaf, a, b, c, d)                                                    \
    __cpuid_count((unsigned)(leaf), (unsigned)(subleaf), (unsigned &)(a), (unsigned &)(b),         \
...
static bool fp_nonempty(const char *s) {
    return s && s[0] && strcmp(s, "None") && strcmp(s, "n/a");
}
...
static void collect_cpu_x86(dsco_fingerprint_t *fp) {
    ...
    if (!strcmp(vendor, "GenuineIntel"))
        fp_trim(fp->cpu_vendor, sizeof(fp->cpu_vendor), "Intel");
    else if (!strcmp(vendor, "AuthenticAMD"))
        fp_trim(fp->cpu_vendor, sizeof(fp->cpu_vendor), "AMD");
    else if (!strcmp(vendor, "HygonGenuine"))
        fp_trim(fp->cpu_vendor, sizeof(fp->cpu_vendor), "Hygon");
    ...
    if (max_leaf >= 1) {
        ...
        snprintf(fp->cpu_stepping_str, sizeof(fp->cpu_stepping_str), "0x%02x", fp->cpuid_stepping);
    ...
    if (a >= 0x80000004) {
        ...
        for (int i = 0; i < 3; i++) {
            uint32_t r[4];
            dsco_cpuidex(0x80000002 + i, 0, r[0], r[1], r[2], r[3]);
    ...
    /* Hypervisor detection: leaf 0x40000000 */
    {
        ...
        if (hvstr[0]) {
            ...
            if (strstr(hvstr, "VMwareVMware"))
                fp->hypervisor = DSCO_HV_VMWARE;
            else if (strstr(hvstr, "VBoxVBox"))
                fp->hypervisor = DSCO_HV_VIRTUALBOX;
            else if (strstr(hvstr, "Microsoft"))
                fp->hypervisor = DSCO_HV_HYPERV;
            else if (strstr(hvstr, "KVMKVMKVM"))
                fp->hypervisor = DSCO_HV_KVM;
            else if (strstr(hvstr, "prl hyperv"))
                fp->hypervisor = DSCO_HV_PARALLELS;
            else if (strstr(hvstr, "XenVMMXen"))
                fp->hypervisor = DSCO_HV_XEN;
            else if (strstr(hvstr, "bhyve"))
                fp->hypervisor = DSCO_HV_BHYVE;
            else
                fp->hypervisor = DSCO_HV_UNKNOWN;
        }
    }
    ...
    fp->mem_total_bytes = (uint64_t)sysconf(_SC_PHYS_PAGES) * (uint64_t)sysconf(_SC_PAGESIZE);
#endif
}
#endif /* x86 */
...
static bool mac_sysctl_int(const char *n, int *v) {
    size_t s = sizeof(int);
    return sysctlbyname(n, v, &s, NULL, 0) == 0;
}
...
static bool mac_sysctl_str(const char *n, char *o, size_t c) {
    size_t s = c;
    if (sysctlbyname(n, o, &s, NULL, 0) != 0)
        return false;
    o[c - 1] = '\0';
    return true;
}
...
static bool mac_iokit_str(io_registry_entry_t e, const char *key, char *out, size_t cap) {
    out[0] = '\0';
...
static void mac_collect_firmware(dsco_fingerprint_t *fp) {
    io_registry_entry_t e = mac_ioplatform();
    if (e != MACH_PORT_NULL) {
        mac_iokit_str(e, "system-firmware-version", fp->firmware_version,
                      sizeof(fp->firmware_version));
    ...
    if (m && IOServiceGetMatchingServices(kIOMainPortDefault, m, &iter) == KERN_SUCCESS) {
        ...
        if (s) {
            mac_iokit_str(s, "smc-version", fp->smc_version, sizeof(fp->smc_version));
            IOObjectRelease(s);
        }
...
static void mac_collect_battery(dsco_fingerprint_t *fp) {
    ...
    CFTypeRef v =
        IORegistryEntryCreateCFProperty(bat, CFSTR("ExternalConnected"), kCFAllocatorDefault, 0);
    if (v) {
...
try_metal:
    ...
    fp_popen_str("system_profiler SPDisplaysDataType 2>/dev/null | awk '/Chipset "
                 "Model:/{sub(/.*Chipset Model: /,\"\"); print; exit}'",
...
static void mac_collect_network(dsco_fingerprint_t *fp) {
    ...
    for (ifa = ifap; ifa; ifa = ifa->ifa_next) {
        ...
        if ((m[0] & 0x01) || (!m[0] && !m[1] && !m[2]))
            continue;

        char mac[18];
        snprintf(mac, sizeof(mac), "%02x:%02x:%02x:%02x:%02x:%02x", m[0], m[1], m[2], m[3], m[4],
                 m[5]);

        if (strcmp(ifa->ifa_name, "en0") == 0) {
            if (!fp->wifi_mac[0])
                snprintf(fp->wifi_mac, sizeof(fp->wifi_mac), "%s", mac);
            if (!fp->primary_mac[0]) {
...
static void mac_collect_storage(dsco_fingerprint_t *fp) {
    ...
    while ((p = strstr(p, "\"device_name\"")) != NULL) {
        ...
        if (len > 0 && strncmp(vs, "Disk Image", len) && strncmp(vs, "disk image", len)) {
            if (len >= sizeof(fp->storage_model))
                len = sizeof(fp->storage_model) - 1;
    ...
    if ((p = strstr(p, "\"protocol\"")) != NULL) {
        ...
        if (vs) {
            ...
            if (ve) {
                size_t len = (size_t)(ve - vs);
                if (len >= sizeof(fp->storage_protocol))
                    len = sizeof(fp->storage_protocol) - 1;
    ...
    while ((p = strstr(p, "\"volume_uuid\"")) != NULL) {
        ...
        size_t len = (size_t)(ve - vs);
        if (len == 36) {
            if (len >= sizeof(fp->primary_volume_uuid))
                len = sizeof(fp->primary_volume_uuid) - 1;
...
static void mac_collect_silicon(dsco_fingerprint_t *fp) {
    ...
    snprintf(fp->cpu_vendor, sizeof(fp->cpu_vendor), "%s",
             ...
             : strstr(fp->cpu_brand, "Intel") ? "Intel"
                                              : "Unknown");
    ...
    mac_sysctl_u64("hw.memsize", &fp->mem_total_bytes);
    {
        ...
        {
            uint64_t _v = 0;
            if (mac_sysctl_u64("hw.l1dcachesize", &_v))
                fp->cache.l1d_bytes = (uint32_t)_v;
        }
        {
            uint64_t _v = 0;
            if (mac_sysctl_u64("hw.l1icachesize", &_v))
                fp->cache.l1i_bytes = (uint32_t)_v;
        }
        {
            uint64_t _v = 0;
            if (mac_sysctl_u64("hw.l2cachesize", &_v))
                fp->cache.l2_bytes = (uint32_t)_v;
        }
        {
            uint64_t _v = 0;
            if (mac_sysctl_u64("hw.cachelinesize", &_v))
                fp->cache.cache_line_bytes = (uint32_t)_v;
        }
        ...
        /* ARM feature flags */
        if (mac_sysctl_int("hw.optional.arm.FEAT_BF16", &v))
            fp->features.bf16 = (v != 0);
        if (mac_sysctl_int("hw.optional.arm.FEAT_FP16", &v))
            fp->features.fp16 = (v != 0);
        if (mac_sysctl_int("hw.optional.arm.FEAT_I8MM", &v))
            fp->features.i8mm = (v != 0);
        if (mac_sysctl_int("hw.optional.arm.FEAT_SHA512", &v))
            fp->features.sha512 = (v != 0);
        if (mac_sysctl_int("hw.optional.arm.FEAT_SHA3", &v))
            fp->features.sha3 = (v != 0);
        if (mac_sysctl_int("hw.optional.arm.FEAT_LSE", &v))
            fp->features.lse = (v != 0);
        if (mac_sysctl_int("hw.optional.arm.FEAT_LSE2", &v))
            fp->features.lse2 = (v != 0);
        if (mac_sysctl_int("hw.optional.arm.FEAT_RDM", &v))
            fp->features.dotprod = (v != 0);
        if (mac_sysctl_int("hw.optional.arm.FEAT_LRCPC", &v))
            fp->features.lrcpc = (v != 0);
        if (mac_sysctl_int("hw.optional.arm.FEAT_LRCPC2", &v))
            fp->features.lrcpc |= (v != 0);
        if (mac_sysctl_int("hw.optional.arm.FEAT_SVE", &v))
            fp->features.sve = (v != 0);
        if (mac_sysctl_int("hw.optional.arm.FEAT_SVE2", &v))
            fp->features.sve2 = (v != 0);
        if (mac_sysctl_int("hw.optional.amx_version", &v))
            fp->features.amx = (v != 0);
        if (mac_sysctl_int("hw.optional.arm.FEAT_PAUTHQARMA5", &v))
            fp->features.pauth = (v != 0);
...
static void linux_collect_cpu(dsco_fingerprint_t *fp) {
    ...
    if (!fp->cpu_vendor[0])
        linux_cpuinfo("CPU implementer", fp->cpu_vendor, sizeof(fp->cpu_vendor));
    ...
    /* Try to get physical from /sys */
    {
        ...
        if (d) {
            ...
            while ((de = readdir(d))) {
                if (strncmp(de->d_name, "cpu", 3) != 0 || !isdigit(de->d_name[3]))
                    continue;
                char p[128];
                snprintf(p, sizeof(p), "/sys/devices/system/cpu/%s/topology/core_id", de->d_name);
                char val[8] = {0};
                fp_read_file(p, val, sizeof(val));
                if (val[0]) {
                    int id = atoi(val);
                    if (id < 2048 && !seen[id]) {
                        seen[id] = 1;
...
static void linux_collect_identity(dsco_fingerprint_t *fp) {
    ...
    linux_sysfs("/sys/class/dmi/id/board_serial", fp->baseboard_serial,
                sizeof(fp->baseboard_serial));
    linux_sysfs("/sys/class/dmi/id/chassis_serial", fp->chassis_serial, sizeof(fp->chassis_serial));
    linux_sysfs("/sys/class/dmi/id/bios_date", fp->bios_release_date,
                sizeof(fp->bios_release_date));
...
static void linux_collect_firmware(dsco_fingerprint_t *fp) {
    ...
    /* Secure boot */
    char sb[8] = {0};
    linux_sysfs("/sys/firmware/efi/efivars/SecureBoot-8be4df61-93ca-11d2-aa0d-00e098032b8c", sb,
                sizeof(sb));
...
static void linux_collect_storage(dsco_fingerprint_t *fp) {
    ...
    for (int i = 0; preferred[i]; i++) {
        ...
        if (access(p, F_OK) == 0) {
            snprintf(base, sizeof(base), "%s", preferred[i]);
            break;
        }
    }
    if (!base[0])
    ...
    for (char *c = ser; *c; c++)
        if (!isspace((unsigned char)*c) || c > ser)
    ...
    fp_popen_str("blkid -s UUID -o value / 2>/dev/null | head -1", fp->primary_volume_uuid,
                 sizeof(fp->primary_volume_uuid));
}
...
static void linux_collect_battery(dsco_fingerprint_t *fp) {
    ...
    while ((de = readdir(d))) {
        if (strncmp(de->d_name, "BAT", 3) == 0 || strncmp(de->d_name, "battery", 7) == 0) {
            snprintf(bat, sizeof(bat), "%s", de->d_name);
            break;
        }
    }
    closedir(d);
    ...
    char v[16] = {0};
    if (fp_read_file(p, v, sizeof(v)))
        fp->battery_cycle_count = (uint32_t)atoi(v);
...
static void linux_collect_hypervisor(dsco_fingerprint_t *fp) {
    ...
    if (strstr(prod, "VMware")) {
        fp->is_virtual = true;
        fp->hypervisor = DSCO_HV_VMWARE;
    } else if (strstr(prod, "VirtualBox")) {
        fp->is_virtual = true;
        fp->hypervisor = DSCO_HV_VIRTUALBOX;
    } else if (strstr(prod, "KVM")) {
        fp->is_virtual = true;
        fp->hypervisor = DSCO_HV_KVM;
    } else if (strstr(prod, "HVM")) {
        fp->is_virtual = true;
        fp->hypervisor = DSCO_HV_XEN;
    } else if (strstr(prod, "QEMU")) {
        fp->is_virtual = true;
        fp->hypervisor = DSCO_HV_QEMU;
    }
...
#if defined(DSCO_FP_ARCH_X64) || defined(DSCO_FP_ARCH_X86)
    {
        ...
        dsco_cpuid(1, a, b, c, d);
        if ((c >> 31) & 1)
            fp->is_virtual = true; /* hypervisor present bit */
        if (fp->is_virtual) {
            dsco_cpuid(0x40000000, a, b, c, d);
            char hvs[13] = {0};
            memcpy(hvs + 0, &b, 4);
    ...
    if (fp->tpm_present) {
        ...
        if (!tver[0])
            linux_sysfs("/sys/class/tpm/tpm0/device/description", tver, sizeof(tver));
        snprintf(fp->tpm_version, sizeof(fp->tpm_version), "2.0"); /* default modern */
        linux_sysfs("/sys/class/tpm/tpm0/device/manufacturer", fp->tpm_manufacturer,
                    sizeof(fp->tpm_manufacturer));
    }
}
...
static bool win_reg_str(HKEY root, const wchar_t *subkey, const wchar_t *name, char *out,
                        size_t cap) {
    out[0] = '\0';
    HKEY hk;
    ...
    RegCloseKey(hk);
    if (ok)
        WideCharToMultiByte(CP_UTF8, 0, buf, -1, out, (int)cap, NULL, NULL);
    return ok;
}
...
static void win_collect_identity(dsco_fingerprint_t *fp) {
    ...
    win_reg_str(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Cryptography", L"MachineGuid",
                fp->windows_machine_guid, sizeof(fp->windows_machine_guid));

    /* ProductId */
    win_reg_str(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion",
                L"ProductId", fp->windows_product_id, sizeof(fp->windows_product_id));

    /* Hardware Profile GUID */
    win_reg_str(HKEY_LOCAL_MACHINE,
                L"SYSTEM\\CurrentControlSet\\Control\\IDConfigDB\\Hardware Profiles\\0001",
                L"HwProfileGuid", fp->windows_hwprofile_guid, sizeof(fp->windows_hwprofile_guid));

    /* InstallDate as proxy */
    DWORD idate = 0;
    if (win_reg_dword(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion",
                      L"InstallDate", &idate))
        snprintf(fp->windows_install_date, sizeof(fp->windows_install_date), "%lu",
                 (unsigned long)idate);
    ...
    win_reg_str(HKEY_LOCAL_MACHINE, L"HARDWARE\\DESCRIPTION\\System\\BIOS", L"BaseBoardProduct",
                fp->machine_model, sizeof(fp->machine_model));
    win_reg_str(HKEY_LOCAL_MACHINE, L"HARDWARE\\DESCRIPTION\\System\\BIOS", L"SystemProductName",
...
static void win_collect_cpu(dsco_fingerprint_t *fp) {
    ...
    win_reg_str(HKEY_LOCAL_MACHINE, L"HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0",
                L"ProcessorNameString", fp->cpu_brand, sizeof(fp->cpu_brand));
    win_reg_str(HKEY_LOCAL_MACHINE, L"HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0",
                L"VendorIdentifier", fp->cpu_vendor, sizeof(fp->cpu_vendor));
                ...
                if (e->Relationship == RelationCache) {
                    ...
                    switch (cr->Level) {
                        case 1:
                            ...
                            break;
                        case 2:
                            fp->cache.l2_bytes = cr->CacheSize;
                            break;
                        case 3:
                            fp->cache.l3_bytes = cr->CacheSize;
                            break;
                    }
                }
                p += e->Size;
...
static void win_collect_network(dsco_fingerprint_t *fp) {
    ...
    if (GetAdaptersAddresses(
        ...
        for (IP_ADAPTER_ADDRESSES *a = buf; a; a = a->Next) {
            ...
            if (!m[0] && !m[1] && !m[2])
                continue;
            snprintf(fp->primary_mac, sizeof(fp->primary_mac), "%02x:%02x:%02x:%02x:%02x:%02x",
                     m[0], m[1], m[2], m[3], m[4], m[5]);
            WideCharToMultiByte(CP_UTF8, 0, a->FriendlyName, -1, fp->iface_name,
                                sizeof(fp->iface_name), NULL, NULL);
            fp->mac_addr_assign_type = 0;
            break;
        }
    }
...
static void win_collect_storage(dsco_fingerprint_t *fp) {
    ...
    if (GetVolumeInformationW(L"C:\\", volname, MAX_PATH, &serial_no, &maxlen, &flags, fsname,
                              MAX_PATH)) {
        snprintf(fp->primary_volume_uuid, sizeof(fp->primary_volume_uuid), "%08lX",
                 (unsigned long)serial_no);
        WideCharToMultiByte(CP_UTF8, 0, fsname, -1, fp->storage_protocol,
                            sizeof(fp->storage_protocol), NULL, NULL);
    }
    /* Disk model via DeviceIoControl — IOCTL_STORAGE_QUERY_PROPERTY */
    HANDLE h = CreateFileW(L"\\\\.\\PhysicalDrive0", 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                           OPEN_EXISTING, 0, NULL);
    if (h != INVALID_HANDLE_VALUE) {
        struct {
            STORAGE_PROPERTY_QUERY q;
        } req = {.q = {StorageDeviceProperty, PropertyStandardQuery}};
        BYTE resp[512] = {0};
        DWORD ret;
        if (DeviceIoControl(h, IOCTL_STORAGE_QUERY_PROPERTY, &req, sizeof(req), resp, sizeof(resp),
                            &ret, NULL)) {
...
static void win_collect_firmware(dsco_fingerprint_t *fp) {
    win_reg_str(HKEY_LOCAL_MACHINE, L"HARDWARE\\DESCRIPTION\\System\\BIOS", L"BIOSVersion",
                fp->bios_version, sizeof(fp->bios_version));
    win_reg_str(HKEY_LOCAL_MACHINE, L"HARDWARE\\DESCRIPTION\\System\\BIOS", L"BIOSReleaseDate",
                fp->bios_release_date, sizeof(fp->bios_release_date));
    win_reg_str(HKEY_LOCAL_MACHINE, L"HARDWARE\\DESCRIPTION\\System\\BIOS", L"BIOSVendor",
    ...
    /* UEFI vs legacy */
    {
        ...
        if (fn && fn(&ft))
            fp->uefi_mode = (ft == FirmwareTypeUefi);
    }
    /* Secure boot */
    DWORD sb = 0;
    if (win_reg_dword(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Control\\SecureBoot\\State",
...
static void win_collect_hypervisor(dsco_fingerprint_t *fp) {
    ...
    if ((c >> 31) & 1) {
        ...
        memcpy(hvs + 8, &d, 4);
        fp_trim(fp->hypervisor_vendor, sizeof(fp->hypervisor_vendor), hvs);
        if (strstr(hvs, "VMwareVMware"))
            fp->hypervisor = DSCO_HV_VMWARE;
        else if (strstr(hvs, "VBoxVBox"))
            fp->hypervisor = DSCO_HV_VIRTUALBOX;
        else if (strstr(hvs, "Microsoft"))
            fp->hypervisor = DSCO_HV_HYPERV;
        else if (strstr(hvs, "KVMKVMKVM"))
            fp->hypervisor = DSCO_HV_KVM;
        else if (strstr(hvs, "prl hyperv"))
            fp->hypervisor = DSCO_HV_PARALLELS;
        else
            fp->hypervisor = DSCO_HV_UNKNOWN;
    }
    ...
    /* TPM */
    {
        ...
        PFN_TISI fn = (PFN_TISI)GetProcAddress(GetModuleHandleW(L"tbs.dll"), "Tbsi_Is_TPM_Present");
        if (!fn) {
            HMODULE h = LoadLibraryW(L"tbs.dll");
            if (h)
                fn = (PFN_TISI)GetProcAddress(h, "Tbsi_Is_TPM_Present");
        }
        if (fn)
            fp->tpm_present = fn();
        if (fp->tpm_present)
...
static void fp_collect_posix_os(dsco_fingerprint_t *fp) {
    ...
    if (uname(&u) == 0) {
        ...
        for (char *p = fp->os_name; *p; p++)
            *p = (char)tolower((unsigned char)*p);
    }
    /* username */
    struct passwd *pw = getpwuid(getuid());
    if (pw)
        fp_trim(fp->username, sizeof(fp->username), pw->pw_name);
}
#endif
...
static void fp_update_signals(dsco_fingerprint_t *fp) {
    ...
    if (fp_nonempty(fp->serial_number))
        s |= DSCO_FPS_SERIAL;
    if (fp_nonempty(fp->platform_uuid) || fp_nonempty(fp->machine_id))
        s |= DSCO_FPS_UUID;
    if (fp_nonempty(fp->board_id))
        s |= DSCO_FPS_BOARD_ID;
    if (fp_nonempty(fp->mlb) || fp_nonempty(fp->baseboard_serial))
        s |= DSCO_FPS_MLB;
    if (fp_nonempty(fp->efi_guid) || fp_nonempty(fp->windows_machine_guid))
        s |= DSCO_FPS_EFI_GUID;
    if (fp_nonempty(fp->cpu_brand))
        s |= DSCO_FPS_CPU;
    if (fp_nonempty(fp->machine_model))
        s |= DSCO_FPS_MODEL;
    if (fp_nonempty(fp->primary_mac))
        s |= DSCO_FPS_MAC;
    if (fp_nonempty(fp->storage_model) || fp_nonempty(fp->storage_serial))
        s |= DSCO_FPS_STORAGE;
    if (fp_nonempty(fp->battery_serial))
        s |= DSCO_FPS_BATTERY;
    if (fp->tpm_present)
        s |= DSCO_FPS_TPM;
    if (fp_nonempty(fp->gpu_model) || fp->gpu_core_count > 0)
        s |= DSCO_FPS_GPU;
    if (fp->is_virtual)
        s |= DSCO_FPS_HYPERVISOR;
    if (fp_nonempty(fp->bios_version) || fp_nonempty(fp->firmware_version))
        s |= DSCO_FPS_BIOS;
    if (fp_nonempty(fp->primary_volume_uuid))
        s |= DSCO_FPS_VOLUME_UUID;
...
#define HA(F)                                                                                      \
    ...
    HA(efi_guid);
    HA(machine_id);
    HA(windows_machine_guid);
    /* TPM EK hash if available */
    if (fp_nonempty(fp->tpm_ekpub_hash)) {
        HA(tpm_ekpub_hash);
    }
#undef HA
    fp->tier_a_count = ta;
...
#define HB(F)                                                                                      \
    ...
    HB(storage_serial);
    HB(battery_serial);
    HB(windows_product_id);
    HB(windows_install_date);
#undef HB
    fp->tier_b_count = tb;
    sha256_final(&s, h);
    ...
    char bstr[24];
    snprintf(bstr, sizeof(bstr), "%lld", (long long)fp->boot_time);
    sha256_init(&s);
...
int dsco_fingerprint_refresh(void) {
    ...
    g_fp.mem_total_bytes = (uint64_t)sysconf(_SC_PHYS_PAGES) * (uint64_t)sysconf(_SC_PAGESIZE);
...
const char *dsco_hv_name(dsco_hv_type_t hv) {
    switch (hv) {
        ...
        case DSCO_HV_XEN:
            return "Xen";
        case DSCO_HV_APPLE_HV:
            return "Apple Hypervisor";
        case DSCO_HV_PARALLELS:
            return "Parallels";
        case DSCO_HV_BHYVE:
            return "bhyve";
        case DSCO_HV_UNKNOWN:
            return "unknown";
        default:
            return "none";
    }
...
#define _JI(K, V)                                                                                  \
    do {                                                                                           \
        int _n = snprintf(p, end - p, "%s\"%s\":%lld", first ? "" : ",", K, (long long)(V));       \
        if (_n < 0 || _n >= end - p)                                                               \
            goto done;                                                                             \
        p += _n;                                                                                   \
        first = false;                                                                             \
...
#define _JU(K, V)                                                                                  \
    do {                                                                                           \
        int _n =                                                                                   \
            snprintf(p, end - p, "%s\"%s\":%llu", first ? "" : ",", K, (unsigned long long)(V));   \
        if (_n < 0 || _n >= end - p)                                                               \
            goto done;                                                                             \
        p += _n;                                                                                   \
        first = false;                                                                             \
...
#define _JB(K, V)                                                                                  \
    do {                                                                                           \
        int _n = snprintf(p, end - p, "%s\"%s\":%s", first ? "" : ",", K, (V) ? "true" : "false"); \
        if (_n < 0 || _n >= end - p)                                                               \
            goto done;                                                                             \
        p += _n;                                                                                   \
        first = false;                                                                             \
...
#define _JF(K, V)                                                                                  \
    do {                                                                                           \
        int _n = snprintf(p, end - p, "%s\"%s\":%.2f", first ? "" : ",", K, (double)(V));          \
        if (_n < 0 || _n >= end - p)                                                               \
            goto done;                                                                             \
        p += _n;                                                                                   \
        first = false;                                                                             \
...
#define _SEP(S)                                                                                    \
    do {                                                                                           \
        int _n = snprintf(p, end - p, "%s", S);                                                    \
        if (_n < 0 || _n >= end - p)                                                               \
            goto done;                                                                             \
        p += _n;                                                                                   \
    } while (0)

size_t dsco_fingerprint_to_json(const dsco_fingerprint_t *fp, bool include_pii, bool include_all,
                                char *out, size_t out_cap) {
    if (!fp || !out || out_cap < 16)
        return 0;
...
#define _F(NAME, COND)                                                                             \
    do {                                                                                           \
        if (COND) {                                                                                \
            int _n = snprintf(p, end - p, "%s\"%s\"", ff ? "" : ",", NAME);                        \
            if (_n < 0 || _n >= end - p)                                                           \
                goto done;                                                                         \
            p += _n;                                                                               \
            ff = false;                                                                            \
...
#define _JU(K, V)                                                                                  \
    do {                                                                                           \
        int _n =                                                                                   \
            snprintf(p, end - p, "%s\"%s\":%llu", _cf ? "" : ",", K, (unsigned long long)(V));     \
        if (_n < 0 || _n >= end - p)                                                               \
            goto done;                                                                             \
        p += _n;                                                                                   \
        _cf = false;                                                                               \
        ...
        if (fp->cache.perf_l2_bytes)
            _JU("perf_l2", fp->cache.perf_l2_bytes);
        if (fp->cache.eff_l2_bytes)
            _JU("eff_l2", fp->cache.eff_l2_bytes);
        _SEP("}");
#undef _JU
#define _JU(K, V)                                                                                  \
    do {                                                                                           \
...
size_t dsco_fingerprint_summary(const dsco_fingerprint_t *fp, char *out, size_t cap) {
    ...
    int n = snprintf(out, cap,
                     ...
                     " · conf:%u%% · id:%.12s",
                     fp->cpu_brand[0] ? fp->cpu_brand : "?", fp->arch[0] ? fp->arch : "?",
                     fp->machine_model[0] ? fp->machine_model : "?",
                     fp->perf_cores ? fp->perf_cores : fp->cores_logical, fp->efficiency_cores,
                     (double)fp->mem_total_bytes / (1024.0 * 1024.0 * 1024.0),
                     fp->is_virtual ? dsco_hv_name(fp->hypervisor) : "bare-metal", fp->confidence,
                     fp->composite_id);
    return (n < 0) ? 0 : (size_t)n;
}

size_t dsco_fingerprint_stability_report(const dsco_fingerprint_t *fp, char *out, size_t cap) {
    if (!fp || !out)
        return 0;
    ...
    int n = snprintf(
        ...
        fp->platform_tag, fp->composite_id, fp->confidence, fp->session_id, fp->hw_id,
        fp->tier_a_count, (s & DSCO_FPS_SERIAL) ? fp->serial_number : "✗",
        ...
        fp->tier_b_count, fp_nonempty(fp->cpu_brand) ? fp->cpu_brand : "✗",
        ...
        fp_nonempty(fp->battery_serial) ? fp->battery_serial : "✗", (unsigned)fp->signals_present,
        fp->is_virtual ? dsco_hv_name(fp->hypervisor) : "none", fp->is_virtual ? "" : "",
        fp->tpm_present ? fp->tpm_version : "absent", fp->uefi_mode ? "yes" : "legacy",
        fp->secure_boot_enabled ? "on" : "off", (long long)fp->collected_at,
