...
static const rsi_category_t RSI_CATEGORIES[] = {
    {
        "CE", "Code engineering",
        "Safe, minimal, reversible modification of the C/C++ substrate",
    ...
    },
    {
        ...
        "Durability, retries, partitions, and staged rollout discipline",
        "Duplicate effects, split-brain, unrecoverable state",
        "Membership, leader election, idempotent tasks",
        "WAL/replay, delivery semantics, backpressure",
        "Partitions, clocks, sharding, replication",
    ...
    {
        "EX", "Experimentation",
        "Falsifiable claims and cost-aware research discipline",
        "Anecdotal improvement, confounded results",
        "Hypotheses, baselines, preregistered plans",
        "Ablations, confounders, sample sizing",
        "Budgets, stop-losses, canaries",
        "Failure taxonomy, causal postmortems, portfolio search",
        "Reproduce experiments and justify go/no-go decisions from evidence"
    },
    {
        "DC", "Data curation",
        "Trustworthy traces, splits, labels, and lineage",
        "Training contamination, bad labels, untraceable rows",
        "Trace capture, schema validation, trajectory reconstruction",
        "Redaction, provenance, split strategy",
        "Preference pairs, label QA, imbalance correction",
        "Contamination checks, synthetic-data caps, retention/deletion",
        "No held-out contamination and redaction recall above target"
    },
    {
        "MT", "Model training and serving",
        "Efficient post-training and robust serving",
        "Overfit adapters, unstable serving, model drift",
        "Base-model choice, templates/tokenizers",
        "SFT/DPO/critic formatting",
        "PEFT/LoRA tuning, checkpoints, early stopping",
        "Serving, routing, quantization, registry usage",
        "Beat baseline on held-out suite without safety regression and with lineage"
    },
    {
        "EV", "Evaluation and diagnostics",
        "Reliable acceptance tests and judges",
        "Reward hacking, false promotions, flaky regressions",
        "Unit and integration harnesses",
        "Held-out suites, mutation testing, judge calibration",
        "Replay evals, patch acceptance, safety regressions",
        "Flake control, dashboards, calibration",
        "Mutation score above target, replay stable, judge agreement acceptable"
    },
    {
        "SP", "Safety policy governance",
        "Constrained authority and accountable rollout",
        "Unsafe autonomy, silent policy bypasses",
        "Least privilege, budget leases, policy logs",
    ...
    {
        "OT", "Observability and telemetry",
        "End-to-end replayability and diagnosis",
        "Unknown unknowns, unfixable incidents",
        "Trace IDs, structured logs, metric taxonomy",
    ...
    },
    {
        ...
        "Elegant but untestable narratives",
        "Correspondence as hypothesis, apophasis by subtraction",
...
static const rsi_skill_t RSI_TOP_SKILLS[] = {
    { "DC09", "Provenance tracking", "Data curation",
      "Track row-level lineage so self-improvement results remain auditable.",
      5, 5, 5.00, true, true },
    { "DS03", "Idempotent task execution", "Distributed systems",
      "Make autonomous retries safe by preventing duplicate durable effects.",
      5, 5, 5.00, true, true },
    { "DS04", "Write-ahead logging and replay", "Distributed systems",
      "Record ordered state transitions for crash recovery, forensics, and eval replay.",
      5, 5, 5.00, true, true },
    { "EV12", "Replay-based evaluation", "Evaluation and diagnostics",
      "Test candidate behavior against old traces before granting live authority.",
      5, 5, 5.00, true, true },
    { "MR13", "Regression-triggered rollback", "Meta-reasoning and self-improvement",
      "Automatically demote or revert candidates when regression thresholds fire.",
      5, 5, 5.00, true, true },
    { "MR15", "RSI stop conditions", "Meta-reasoning and self-improvement",
      "Define explicit halt rules for recursive change and authority growth.",
      5, 5, 5.00, true, true },
    { "OT08", "Replayable event streams", "Observability and telemetry",
      "Persist event streams so behavior becomes durable training and eval evidence.",
      5, 5, 5.00, true, true },
    { "SP02", "Budget lease enforcement", "Safety policy governance",
      "Require scoped, revocable, auditable resource leases for autonomous work.",
      5, 5, 5.00, true, true },
    { "SP07", "Self-modification gating", "Safety policy governance",
      "Keep code and model changes behind tests, lineage, and staged promotion.",
      5, 5, 5.00, true, true },
    { "SP09", "Signing and attestation", "Safety policy governance",
      "Require verifiable origin and provenance for every release candidate.",
      5, 5, 5.00, true, true },
    { "EV02", "Integration eval harnesses", "Evaluation and diagnostics",
      "Exercise multi-tool, multi-step behavior beyond isolated unit tests.",
      4, 5, 4.60, true, true },
    { "EV03", "Held-out task suites", "Evaluation and diagnostics",
      "Protect promotion decisions from overfitting to generated traces and judges.",
      4, 5, 4.60, true, true },
    { "EV06", "Judge calibration", "Evaluation and diagnostics",
      "Calibrate model graders against humans and hard rules before relying on them.",
      4, 5, 4.60, true, true },
    { "EV11", "Patch acceptance scoring", "Evaluation and diagnostics",
      "Convert patches into accept/reject decisions with explicit scoring evidence.",
      4, 5, 4.60, true, true },
    { "MR01", "Self-critique localization", "Meta-reasoning and self-improvement",
      "Localize failures to actionable causes rather than generic reflection.",
      4, 5, 4.60, false, false },
    { "MR03", "Think-vs-tool routing", "Meta-reasoning and self-improvement",
      "Choose between reasoning and tool use to reduce waste and missed evidence.",
      4, 5, 4.60, false, false },
    { "MR04", "Decomposition planning", "Meta-reasoning and self-improvement",
      "Break large tasks into independent subgoals with clean commit points.",
      4, 5, 4.60, false, false },
    { "MR05", "Reflection after action", "Meta-reasoning and self-improvement",
      "Update behavior from observed outcomes, not only prior plans.",
      4, 5, 4.60, false, false },
    { "MR07", "Curriculum selection", "Meta-reasoning and self-improvement",
      "Select next training targets by expected skill gain and safety value.",
      4, 5, 4.60, false, false },
    { "MR08", "Skill acquisition tracking", "Meta-reasoning and self-improvement",
      "Treat a skill as acquired only after retest persistence and transfer.",
      4, 5, 4.60, false, false }
};

static void copy_result(char *dst, size_t len, const jbuf_t *src) {
    if (!dst || len == 0 || !src || !src->data) return;
...
static void append_gate_thresholds(jbuf_t *b) {
    jbuf_appendf(b,
        ...
        RSI_GATE_HELDOUT_SUCCESS_MIN,
        RSI_GATE_SAFETY_VIOLATION_MAX,
        RSI_GATE_ROLLBACK_TRIGGER_MAX,
        RSI_GATE_JUDGE_KAPPA_MIN,
        RSI_GATE_REPLAY_STABILITY_MIN,
        RSI_GATE_SUCCESS_IMPROVEMENT_MIN,
        RSI_GATE_COST_REGRESSION_MAX);
}

const rsi_skill_t *rsi_curriculum_skills(int *count) {
    if (count) *count = (int)(sizeof(RSI_TOP_SKILLS) / sizeof(RSI_TOP_SKILLS[0]));
    return RSI_TOP_SKILLS;
}

const rsi_category_t *rsi_curriculum_categories(int *count) {
    if (count) *count = (int)(sizeof(RSI_CATEGORIES) / sizeof(RSI_CATEGORIES[0]));
...
const rsi_skill_t *rsi_curriculum_find_skill(const char *id) {
    ...
    for (int i = 0; i < count; i++) {
        if (strcmp(skills[i].id, id) == 0) return &skills[i];
    }
    return NULL;
}
...
bool rsi_curriculum_validate_promotion(const rsi_eval_summary_t *candidate,
                                       ...
                                       size_t reason_len) {
    if (!candidate) {
        if (reason && reason_len) snprintf(reason, reason_len, "missing_candidate");
        return false;
    }
    if (!candidate->provenance_complete) {
        if (reason && reason_len) snprintf(reason, reason_len, "provenance_incomplete");
        return false;
    }
    if (!candidate->signature_verified) {
        if (reason && reason_len) snprintf(reason, reason_len, "signature_unverified");
        return false;
    }
    if (!candidate->rollback_plan_ready) {
        if (reason && reason_len) snprintf(reason, reason_len, "rollback_plan_missing");
        return false;
    }
    if (!candidate->budget_lease_active) {
        if (reason && reason_len) snprintf(reason, reason_len, "budget_lease_missing");
        return false;
    }
    if (candidate->safety_violation_rate > RSI_GATE_SAFETY_VIOLATION_MAX) {
        if (reason && reason_len) snprintf(reason, reason_len, "safety_violation");
        return false;
    }
    if (candidate->rollback_trigger_rate > RSI_GATE_ROLLBACK_TRIGGER_MAX) {
        if (reason && reason_len) snprintf(reason, reason_len, "rollback_trigger_rate_high");
        return false;
    }
    if (candidate->judge_human_kappa < RSI_GATE_JUDGE_KAPPA_MIN) {
        if (reason && reason_len) snprintf(reason, reason_len, "judge_calibration_low");
        return false;
    }
    if (candidate->replay_stability < RSI_GATE_REPLAY_STABILITY_MIN) {
        if (reason && reason_len) snprintf(reason, reason_len, "replay_stability_low");
        return false;
    }
    if (candidate->heldout_success_rate < RSI_GATE_HELDOUT_SUCCESS_MIN) {
        if (reason && reason_len) snprintf(reason, reason_len, "heldout_success_low");
        return false;
    }
    if (candidate->baseline_success_rate > 0.0 &&
        ...
        if (reason && reason_len) snprintf(reason, reason_len, "baseline_improvement_low");
        return false;
    }
    if (candidate->baseline_cost_per_success_usd > 0.0 &&
        ...
        if (reason && reason_len) snprintf(reason, reason_len, "cost_regression_high");
        return false;
    }
    if (reason && reason_len) snprintf(reason, reason_len, "allow");
    return true;
}
...
int rsi_curriculum_summary_json(char *buf, size_t len) {
    ...
    for (int i = 0; i < skill_count; i++) {
        ...
        jbuf_appendf(&b,
            ...
            s->dangerous ? "true" : "false",
            s->gate_critical ? "true" : "false");
    }
    jbuf_append(&b, "]}");
...
int rsi_curriculum_gate_json(const rsi_eval_summary_t *candidate,
    ...
    jbuf_t b;
    jbuf_init(&b, 2048);
    jbuf_appendf(&b, "{\"allow_promotion\":%s,\"reason\":",
                 allow ? "true" : "false");
    ...
    if (candidate) {
        jbuf_appendf(&b,
            ...
            candidate->heldout_success_rate,
            candidate->baseline_success_rate,
            candidate->safety_violation_rate,
            candidate->rollback_trigger_rate,
            candidate->cost_per_success_usd,
            candidate->baseline_cost_per_success_usd,
            candidate->judge_human_kappa,
            candidate->replay_stability,
