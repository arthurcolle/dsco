...
static void anim_on_sigint(int sig) {
    (void)sig;
    g_anim_stop = 1;
}
...
static double xs_unit(void) {
    return (xs_next() >> 8) * (1.0 / 16777216.0);
}
...
static int heat256(double t) {
    static const int ramp[] = {17, 18, 19, 20, 26,  32,  38,  44,  49,
                               48, 47, 46, 82, 118, 154, 190, 220, 226};
...
void anim_resolve_size(int width, int height, int *W, int *H) {
    ...
    if (w <= 0) {
        int tw = tui_term_width();
        w = (tw > 4 ? tw : 80) - 1;
    }
    if (h <= 0) {
        int th = tui_term_height();
        h = (th > 6 ? th : 24) - 3;
    }
    if (w < 8)
        w = 8;
...
void anim_raw_enable(anim_raw_t *r) {
    ...
    if (!isatty(STDIN_FILENO))
        return;
    if (tcgetattr(STDIN_FILENO, &g_raw_saved) != 0)
        return;
...
void anim_raw_restore(anim_raw_t *r) {
    if (!r)
        return;
    if (r->was_raw)
        tcsetattr(STDIN_FILENO, TCSANOW, &g_raw_saved);
    if (r->saved_flags_valid)
        fcntl(STDIN_FILENO, F_SETFL, r->saved_flags);
    r->was_raw = 0;
    r->saved_flags_valid = 0;
}
...
int anim_poll_key(void) {
    ...
    if (c == 0x1b) { /* ESC — maybe a CSI sequence */
        ...
        switch (fin) {
            case 'A':
                return shift ? ANIM_KEY_SHIFT_UP : ANIM_KEY_UP;
            case 'B':
                return shift ? ANIM_KEY_SHIFT_DOWN : ANIM_KEY_DOWN;
            case 'C':
                return shift ? ANIM_KEY_SHIFT_RIGHT : ANIM_KEY_RIGHT;
            case 'D':
                return shift ? ANIM_KEY_SHIFT_LEFT : ANIM_KEY_LEFT;
            default:
                return ANIM_KEY_NONE;
        }
    }
...
static void life_seed_random(unsigned char *g, int gw, int gh, double density) {
    for (int i = 0; i < gw * gh; i++)
        g[i] = (xs_unit() < density) ? 1 : 0;
}

/* place a (x,y) cell list centered-ish at (ox,oy) with bounds checks */
static void life_stamp(unsigned char *g, int gw, int gh, const int (*cells)[2], int n, int ox,
                       int oy) {
    for (int i = 0; i < n; i++) {
...
static void life_seed_pattern(unsigned char *g, int gw, int gh, const char *pat, double density) {
    memset(g, 0, (size_t)gw * gh);
    if (!pat || !strcmp(pat, "random")) {
        life_seed_random(g, gw, gh, density);
        return;
    }
    ...
    if (!strcmp(pat, "gun")) {
        ...
        for (int gy = 2; gy < gh - 4; gy += 8)
            for (int gx = 2; gx < gw - 4; gx += 8)
                if (xs_unit() < 0.5)
                    life_stamp(g, gw, gh, glider, 5, gx, gy);
    } else {
...
static bool life_pattern_unbounded(const char *p) {
    return p && (!strcmp(p, "gun") || !strcmp(p, "acorn") || !strcmp(p, "pentomino") ||
                 !strcmp(p, "rpentomino") || !strcmp(p, "gliders"));
}
...
static void life_embed(unsigned char *sim, int sw, int sh, const unsigned char *src, int vw, int vh,
                       int ox, int oy) {
...
static long life_step(const unsigned char *cur, unsigned char *nxt, int gw, int gh, bool wrap) {
    long pop = 0;
    for (int y = 0; y < gh; y++) {
        for (int x = 0; x < gw; x++) {
            int n = 0;
...
int anim_life(const anim_opts_t *o) {
    ...
    for (; budget < 0 || frames < budget; frames++) {
        ...
        for (int y = 0; y < vh; y++)
            for (int x = 0; x < vw; x++)
                if (cur[(y + M) * sw + (x + M)])
                    tui_braille_set(&bc, x, y);

        char hdr[160];
        snprintf(hdr, sizeof hdr, "gen %-6ld  pop %-6ld  %dx%d cells  %s%s  ^C to stop", frames,
                 pop, vw, vh, o->pattern ? o->pattern : "random", o->wrap ? "" : " ∞");
...
int anim_rule(const anim_opts_t *o) {
    ...
    for (; budget < 0 || frames < budget; frames++) {
        ...
        char hdr[160];
        snprintf(hdr, sizeof hdr, "rule %-3d  gen %-6ld  %d wide  ^C to stop", rule, frames, gw);
...
int anim_attractor(const anim_opts_t *o) {
    ...
    for (; budget < 0 || frames < budget; frames++) {
        ...
        char hdr[160];
        snprintf(hdr, sizeof hdr, "a=%+.2f b=%+.2f c=%+.2f d=%+.2f  frame %-5ld  ^C to stop", a, b,
                 c, d, frames);
    ...
    for (; budget < 0 || frames < budget; frames++) {
        ...
        for (int r = 0; r < H; r++) {
            for (int c = 0; c < W; c++) {
                ...
                if (dist == 0 && h >= 0 && h < H) {
                    if (o->color)
                        fputs("\033[38;5;231m", stdout); /* bright head */
                    fputc(g, stdout);
...
int anim_dispatch(const char *json) {
    ...
    if (life_pattern_unbounded(o.pattern))
        o.wrap = false;
    if (json) {
    ...
    if (!strcmp(k, "rule") || !strcmp(k, "ca"))
        rc = anim_rule(&o);
    else if (!strcmp(k, "attractor") || !strcmp(k, "dejong"))
        rc = anim_attractor(&o);
    else if (!strcmp(k, "rain") || !strcmp(k, "matrix"))
        rc = anim_rain(&o);
    else if (fractal_is_kind(k))
        rc = fractal_anim(json);
    else if (avatar_is_kind(k))
        rc = avatar_anim(json);
    else
        rc = anim_life(&o);
