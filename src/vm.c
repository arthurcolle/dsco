...
int vm_add_string(vm_t *vm, const char *s) {
    if (vm->str_pool_len >= VM_STR_POOL_MAX)
        return -1;
...
void vm_register_tool(vm_t *vm, const char *name, vm_tool_fn func, int tool_index) {
    if (vm->dispatch_len >= VM_DISPATCH_MAX)
        return;
...
bool vm_dispatch_tool(vm_t *vm, const char *tool_name, const char *input_json, char *result,
                      size_t result_len) {
    if (!vm || !tool_name)
        return false;
...
#define DISPATCH()                                                                                 \
    do {                                                                                           \
        if (vm->pc >= vm->code_len || vm->halted)                                                  \
            goto done;                                                                             \
...
op_dispatch: {
    ...
    if (name_val.type == VM_VAL_STR && name_val.s) {
        ...
        for (int probe = 0; probe < 512; probe++) {
            ...
            if (idx < 0)
                break;
            if (vm->dispatch[idx].hash == h && strcmp(vm->dispatch[idx].name, name_val.s) == 0) {
                vm->cache_hits++;
                if (vm->result_buf) {
...
op_emit: {
    ...
    if (v.type == VM_VAL_STR && v.s && vm->result_buf) {
        ...
        if (len >= vm->result_buf_len)
            len = vm->result_buf_len - 1;
