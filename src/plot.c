...
static int pb_done(pbuf_t *b) {
    return (int)(b->p - b->base);
}

/* Conditional color: only emit when opts.color is on. */
static void pb_col(pbuf_t *b, bool on, const char *seq) {
    if (on && seq)
        pb_puts(b, seq);
}
static void pb_reset(pbuf_t *b, bool on) {
    if (on)
        pb_puts(b, P_RESET);
}
...
static void pb_hbar(pbuf_t *b, double frac, int cells, bool color, const char *fc) {
    if (frac < 0)
        frac = 0;
    if (frac > 1)
        frac = 1;
...
static void pb_braille(pbuf_t *b, tui_braille_t *bc, bool color, const char *col, bool axes,
...
int plot_line_xy(char *out, size_t cap, const double *x, const double *y, int n,
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (i == 0)
            ...
            tui_braille_line(&bc, prev_px, prev_py, px, py);
        prev_px = px;
        prev_py = py;
    }
...
int plot_area(char *out, size_t cap, const double *y, int n, const plot_opts_t *opt) {
    ...
    for (int col = 0; col < bc.px_w; col++) {
        ...
        for (int py = top; py < bc.px_h; py++)
            tui_braille_set(&bc, col, py);
    }
    pb_braille(&b, &bc, o.color, o.color_seq, o.axes, ymn, ymx);
    tui_braille_free(&bc);
    return b.ovf ? -1 : pb_done(&b);
...
int plot_bar(char *out, size_t cap, const char *const *labels, const double *v, int n,
             const plot_opts_t *opt) {
    if (!out || !v || n <= 0) {
        if (out && cap)
            out[0] = '\0';
...
static int plot_columns_impl(pbuf_t *b, const char *const *labels, const double *v, int n,
                             const plot_opts_t *o) {
    double mn, mx;
    minmax(v, n, &mn, &mx);
    double base = mn < 0 ? mn : 0;
    ...
    for (int r = 0; r < rows; r++) {
        for (int i = 0; i < n; i++) {
            ...
            if (cell_fill >= 1.0)
                ...
                g = EIGHTHS_V[(int)(cell_fill * 8.0 + 0.5)];
...
int plot_column(char *out, size_t cap, const char *const *labels, const double *v, int n,
                const plot_opts_t *opt) {
    if (!out || !v || n <= 0) {
        if (out && cap)
            out[0] = '\0';
...
int plot_hist(char *out, size_t cap, const double *samples, int n, int bins,
    ...
    plot_opts_t o = norm(opt);
    if (bins <= 0)
        bins = (int)(sqrt((double)n) + 0.5);
    if (bins < 1)
        bins = 1;
    if (bins > 64)
        bins = 64;
    double mn, mx;
    minmax(samples, n, &mn, &mx);
    double span = mx - mn;
    if (span < 1e-12)
        span = 1;
    double counts[64] = {0};
    ...
    pb_init(&b, out, cap);
    pb_title(&b, &o);
    plot_bar(b.p, (size_t)(b.end - b.p), labptr, counts, bins,
             &(plot_opts_t){.width = o.width, .color = o.color, .axes = false});
...
static int viridis256(double t) {
    ...
    if (t > 1)
        t = 1;
    static const int ramp[] = {17, 18, 19, 20, 26,  32,  38,  44,  49,
...
int plot_heatmap(char *out, size_t cap, const double *grid, int rows, int cols,
    ...
    if (o.color) {
        ...
        pb_putf(&b, "%.3g ", mn);
        for (int i = 0; i < 16; i++)
            pb_putf(&b, "\033[48;5;%dm ", viridis256(i / 15.0));
        pb_putf(&b, "%s %.3g\n", P_RESET, mx);
    }
    return b.ovf ? -1 : pb_done(&b);
}
...
int plot_boxplot(char *out, size_t cap, const double *samples, int n, const plot_opts_t *opt) {
    if (!out || !samples || n <= 0) {
        if (out && cap)
            out[0] = '\0';
        return -1;
    ...
    if (span < 1e-12)
        span = 1;
    int W = o.width;
    int p_mn = 0, p_q1 = (int)((q1 - mn) / span * (W - 1)),
        p_md = (int)((med - mn) / span * (W - 1));
    ...
    for (int i = 0; i < W; i++) {
        if (i == p_md)
            ...
            pb_puts(&b, "▓");
        else if (i == p_mn || i == p_mx)
            pb_puts(&b, "├");
        else if ((i > p_mn && i < p_q1) || (i > p_q3 && i < p_mx))
            pb_puts(&b, "─");
        else
            pb_puts(&b, " ");
    }
...
/* ── candlestick (OHLC) ───────────────────────────────────────────────── */
int plot_candlestick(char *out, size_t cap, const double *op, const double *hi, const double *lo,
                     const double *cl, int n, const plot_opts_t *opt) {
    if (!out || !op || !hi || !lo || !cl || n <= 0) {
        if (out && cap)
            out[0] = '\0';
        return -1;
    }
...
/* ── gauge ────────────────────────────────────────────────────────────── */
int plot_gauge(char *out, size_t cap, double value, double lo, double hi, const plot_opts_t *opt) {
    if (!out)
        return -1;
    plot_opts_t o = norm(opt);
    if (hi - lo < 1e-12)
...
int plot_sparkline(char *out, size_t cap, const double *y, int n, const plot_opts_t *opt) {
    ...
    if (f) {
        if (o.color)
            fputs(o.color_seq ? o.color_seq : P_BCYAN, f);
...
int plot_pie(char *out, size_t cap, const char *const *labels, const double *v, int n,
             const plot_opts_t *opt) {
    if (!out || !v || n <= 0) {
        if (out && cap)
            out[0] = '\0';
...
int plot_waterfall(char *out, size_t cap, const char *const *labels, const double *d, int n,
                   const plot_opts_t *opt) {
    if (!out || !d || n <= 0) {
        if (out && cap)
            out[0] = '\0';
...
int plot_bullet(char *out, size_t cap, const char *label, double value, double target,
                const double *ranges, int nr, const plot_opts_t *opt) {
    if (!out)
        return -1;
    plot_opts_t o = norm(opt);
    pbuf_t b;
    ...
    if (label && label[0]) {
        pb_col(&b, o.color, P_GREY);
        pb_putf(&b, "%-12.12s ", label);
        pb_reset(&b, o.color);
    }
    for (int x = 0; x < W; x++) {
        if (x == pt) {
            pb_col(&b, o.color, P_RED);
            pb_puts(&b, "┃");
            pb_reset(&b, o.color);
            continue;
        }
...
int plot_lollipop(char *out, size_t cap, const char *const *labels, const double *v, int n,
                  const plot_opts_t *opt) {
    if (!out || !v || n <= 0) {
        if (out && cap)
            out[0] = '\0';
    ...
    double mn, mx;
    minmax(v, n, &mn, &mx);
    double base = mn < 0 ? mn : 0, span = mx - base;
    if (span < 1e-12)
        span = 1;
    int lblw = 0;
    for (int i = 0; i < n; i++) {
...
int plot_slope(char *out, size_t cap, const char *const *labels, const double *l, const double *r,
...
int plot_ecdf(char *out, size_t cap, const double *samples, int n, const plot_opts_t *opt) {
    if (!out || !samples || n <= 0) {
        if (out && cap)
            out[0] = '\0';
        return -1;
    ...
    for (int i = 0; i < n; i++)
        y[i] = (double)(i + 1) / n; /* 0..1 cumulative */
    plot_opts_t lo = o;
    lo.title = NULL;
    pbuf_t b;
    pb_init(&b, out, cap);
...
int plot_calendar(char *out, size_t cap, const double *v, int n, const plot_opts_t *opt) {
    if (!out || !v || n <= 0) {
        if (out && cap)
            out[0] = '\0';
        return -1;
    ...
    for (int day = 0; day < 7; day++) {
        ...
        for (int w = 0; w < weeks; w++) {
            ...
            if (k > 5)
                k = 5;
            if (o.color)
                pb_putf(&b, "\033[48;5;%dm  \033[0m", ramp[k]);
            else {
                const char *sh = k == 0 ? "·" : k < 2 ? "░" : k < 4 ? "▒" : "█";
                pb_puts(&b, sh);
                pb_puts(&b, sh);
            }
        }
...
int plot_ridgeline(char *out, size_t cap, const char *const *names, const double *const *series,
                   const int *lens, int ns, const plot_opts_t *opt) {
    if (!out || !series || ns <= 0) {
        if (out && cap)
            out[0] = '\0';
        return -1;
...
int plot_violin(char *out, size_t cap, const double *samples, int n, const plot_opts_t *opt) {
    if (!out || !samples || n <= 0) {
        if (out && cap)
            out[0] = '\0';
        return -1;
...
int plot_bignum(char *out, size_t cap, double value, const char *caption, const plot_opts_t *opt) {
    if (!out)
        return -1;
    plot_opts_t o = norm(opt);
    pbuf_t b;
...
int plot_attractor(char *out, size_t cap, const char *kind, double a, double cb, double c, double d,
                   long iters, const plot_opts_t *opt) {
    if (!out || !cap) {
        if (out && cap)
            out[0] = '\0';
        return -1;
...
#define ATTR_STEP(X, Y, NX, NY)                                                                    \
    do {                                                                                           \
        if (clifford) {                                                                            \
            NX = sin(a * (Y)) + c * cos(a * (X));                                                  \
            NY = sin(cb * (X)) + d * cos(cb * (Y));                                                \
        } else {                                                                                   \
            NX = sin(a * (Y)) - cos(cb * (X));                                                     \
            NY = sin(c * (X)) - cos(d * (Y));                                                      \
        }                                                                                          \
    ...
    for (long i = 0; i < iters; i++) {
        ...
        if (px < 0 || px >= bc.px_w || py < 0 || py >= bc.px_h)
            continue;
        tui_braille_set(&bc, px, py);
...
int plot_fractal(char *out, size_t cap, const char *set, double cx, double cy, double scale,
    ...
    for (int row = 0; row < Hc; row++) {
        ...
        for (int col = 0; col < Wc; col++) {
            ...
            if (o.color) {
                ...
                int ri;
                if (mu_top < 0)
                    ri = (int)sizeof(ramp) - 2; /* interior */
                else {
                    double t = mu_top / iters;
                    if (t > 1)
                        t = 1;
...
int plot_dispatch(const char *input_json, char *out, size_t cap) {
    ...
    if (jtype) {
        snprintf(type, sizeof type, "%s", jtype);
        free(jtype);
    }
    ...
    if (jtitle) {
        snprintf(title, sizeof title, "%s", jtitle);
        free(jtitle);
        if (title[0])
            o.title = title;
    }
    ...
    int rc = -1;
    if (strcmp(type, "line") == 0 || strcmp(type, "area") == 0 || strcmp(type, "scatter") == 0 ||
        strcmp(type, "sparkline") == 0) {
        int xn = 0;
        double *x = json_doubles(input_json, "x", &xn, PLOT_MAX_PTS);
        if (strcmp(type, "area") == 0)
            rc = plot_area(out, cap, data, n, &o);
        else if (strcmp(type, "scatter") == 0)
            rc = plot_scatter(out, cap, x ? x : data, data, n, &o);
        else if (strcmp(type, "sparkline") == 0)
            rc = plot_sparkline(out, cap, data, n, &o);
        else
            rc = plot_line_xy(out, cap, x, data, n, &o);
        free(x);
        ...
        if (op && hp && lp && cp && m > 0)
            ...
            snprintf(out, cap, "candlestick needs open/high/low/close arrays");
            rc = (int)strlen(out);
        }
        free(op);
        free(hp);
        ...
        int rn = 0;
        double *rg = json_doubles(input_json, "ranges", &rn, 16);
        double val = json_get_double(input_json, "value", n > 0 ? data[0] : 0);
        ...
        int m = ln < rn2 ? ln : rn2;
        if (lf && rt && m > 0)
            rc = plot_slope(out, cap, labels, lf, rt, m, &o);
        else {
            snprintf(out, cap, "slope needs left[] and right[] arrays");
            rc = (int)strlen(out);
        }
        free(lf);
        free(rt);
        ...
        if (jk) {
            snprintf(kind, sizeof kind, "%s", jk);
            free(jk);
        }
        ...
        if (o.width < 50)
            o.width = 64; /* attractors want room to breathe */
        if (o.height < 14)
            o.height = 18;
        ...
        if (o.width < 50)
            o.width = 70; /* fractals reward resolution */
        if (o.height < 16)
            o.height = 22;
        ...
        if (sp) {
            ...
            while (*q && nser < 24) {
                ...
                if (*q == ']' || *q == '\0')
                    break;
                if (*q == '[') {
                    double *buf = calloc(PLOT_MAX_PTS, sizeof(double));
                    int cnt = 0;
                    q++;
        ...
        if (nser > 0)
            ...
            snprintf(out, cap, "ridgeline needs series:[[...],[...]]");
            rc = (int)strlen(out);
        }
