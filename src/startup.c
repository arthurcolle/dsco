...
const char *dsco_profile_name(dsco_profile_t profile) {
    switch (profile) {
        case DSCO_PROFILE_FULL:   return "full";
        case DSCO_PROFILE_LITE:   return "lite";
        case DSCO_PROFILE_WORKER: return "worker";
    }
    return "full";
}
...
const char *dsco_caps_to_string(dsco_caps_t caps, char *buf, size_t buflen) {
    ...
    buf[0] = '\0';
    struct cap_name { dsco_caps_t bit; const char *name; };
    static const struct cap_name names[] = {
        { DSCO_CAP_SECURITY, "security" },
        { DSCO_CAP_PROVIDER, "provider" },
        { DSCO_CAP_TOOLS,    "tools" },
        { DSCO_CAP_TUI,      "tui" },
        { DSCO_CAP_VOS,      "vos" },
        { DSCO_CAP_IPC,      "ipc" },
        { DSCO_CAP_MCP,      "mcp" },
        { DSCO_CAP_MEMORY,   "memory" },
        { DSCO_CAP_ACCEL,    "accel" },
        { DSCO_CAP_TRUST,    "trust" },
        { DSCO_CAP_TRACE,    "trace" },
    };
    size_t pos = 0;
    bool first = true;
    ...
    if (first) snprintf(buf, buflen, "none");
    return buf;
}
