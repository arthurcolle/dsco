...
static int infer_atom_count(const char *task) {
    ...
    while (*p) {
        if (*p == ';') {
            count++;
        } else if (strncasecmp(p, " and ", 5) == 0 || strncasecmp(p, " then ", 6) == 0 ||
                   strncasecmp(p, " also ", 6) == 0 || strncasecmp(p, " plus ", 6) == 0 ||
...
bool cost_db_load(cost_db_t *db) {
    ...
    FILE *f = fopen(path, "r");
    if (!f)
        return true; /* no history yet is fine */
...
#define JSGET_INT(key, dst)                                                                        \
    do {                                                                                           \
        p = strstr(line, "\"" key "\":");                                                          \
        if (p)                                                                                     \
            dst = atoi(p + (int)strlen("\"" key "\":"));                                           \
    } while (0)
#define JSGET_DBL(key, dst)                                                                        \
    do {                                                                                           \
        p = strstr(line, "\"" key "\":");                                                          \
        if (p)                                                                                     \
            dst = atof(p + (int)strlen("\"" key "\":"));                                           \
    } while (0)
#define JSGET_LL(key, dst)                                                                         \
    do {                                                                                           \
        p = strstr(line, "\"" key "\":");                                                          \
        if (p)                                                                                     \
            dst = atoll(p + (int)strlen("\"" key "\":"));                                          \
    } while (0)
...
bool cost_db_save(cost_db_t *db) {
    ...
    for (int i = 0; i < cap; i++) {
        int idx = (db->count < COST_DB_CAPACITY) ? i : (db->head + i) % COST_DB_CAPACITY;
        const cost_record_t *r = &db->entries[idx];
        fprintf(f,
                "{\"v\":%d,\"tl\":%d,\"ac\":%d,\"tk\":%d,"
                "\"c\":%.8f,\"tp\":\"%s\",\"ts\":%lld}\n",
                LEARNED_COST_VERSION, r->task_len, r->atom_count, r->tokens, r->cost, r->topology,
...
void learn_from_execution(cost_db_t *db, const char *task, const char *topology, int actual_tokens,
                          double actual_cost) {
    if (!db || !task || !topology || actual_tokens <= 0 || actual_cost < 0.0)
        return;
...
int find_similar_executions(const cost_db_t *db, const char *task, const char *topology, int k,
                            cost_record_t *out_records) {
    if (!db || !task || !topology || k <= 0 || !out_records)
        return 0;
    if (k > LEARNED_COST_K_MAX)
        k = LEARNED_COST_K_MAX;
    ...
    for (int i = 0; i < db->count; i++) {
        ...
        if (strcmp(r->topology, topology) != 0)
            continue;
    ...
    for (int i = 0; i < actual_k; i++) {
        ...
        for (int j = i + 1; j < n_scored; j++) {
            if (scored[j].dist < scored[min_j].dist)
                min_j = j;
        }
        if (min_j != i) {
            scored_t tmp = scored[i];
            scored[i] = scored[min_j];
            scored[min_j] = tmp;
        }
...
bool predict_cost(const cost_db_t *db, const char *task, const char *topology,
                  cost_prediction_result_t *out) {
    if (!db || !task || !topology || !out)
        return false;
