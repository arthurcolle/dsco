...
agent_profile_t *agent_profile_get(int idx) {
    agent_profiles_init();
    if (idx < 0 || idx >= g_profile_count) return NULL;
...
bool agent_profile_save(const agent_profile_t *p) {
    ...
    if (g_profile_count >= MAX_AGENT_PROFILES) return false;
...
const agent_profile_t *agent_profile_active(void) {
    if (!g_active_name[0]) return NULL;
...
bool agent_profile_tool_allowed(const char *tool_name, const char *group_hint) {
    ...
    if (!p) return true;  /* no active profile = allow all */

    /* If neither tool list nor group list is set, allow all */
    if (p->tool_count == 0 && p->group_count == 0) return true;
    ...
    if (p->tool_count > 0) {
        ...
        if (p->group_count == 0) return false;
    }
    ...
    if (p->group_count > 0 && group_hint && group_hint[0]) {
        ...
        if (p->tool_count == 0) return false;
    }

    /* tool_count > 0 but not found, group_count > 0 but no group_hint: reject */
    if (p->group_count > 0 && (!group_hint || !group_hint[0])) return false;
...
bool agent_profile_from_json(agent_profile_t *p, const char *json) {
    ...
    if (!name || !name[0]) { free(name); return false; }
    snprintf(p->name, sizeof(p->name), "%s", name);
    free(name);

    char *desc = json_get_str(json, "description");
    if (desc) { snprintf(p->description, sizeof(p->description), "%s", desc); free(desc); }

    char *model = json_get_str(json, "model_hint");
    if (model) { snprintf(p->model_hint, sizeof(p->model_hint), "%s", model); free(model); }

    char *prompt = json_get_str(json, "prompt_prefix");
    if (prompt) { snprintf(p->prompt_prefix, sizeof(p->prompt_prefix), "%s", prompt); free(prompt); }
...
bool agent_profiles_load(void) {
    ...
    FILE *f = fopen(path, "r");
    if (!f) return true;  /* no file = empty registry, not an error */
