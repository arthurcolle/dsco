...
static void ast_add_node(ast_file_t *f, ast_node_t *node) {
    if (f->count >= AST_MAX_NODES)
        return;
    if (f->count >= f->capacity) {
        int newcap = f->capacity ? f->capacity * 2 : 64;
        if (newcap > AST_MAX_NODES)
            newcap = AST_MAX_NODES;
...
static char *extract_ident(const char *p) {
    ...
    int len = (int)(p - start);
    if (len > 4096)
        len = 4096; /* cap identifier length */
    char *s = malloc(len + 1);
    if (!s)
        return NULL;
    memcpy(s, start, len);
...
static int estimate_complexity(const char *body, size_t len) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (i == 0 || !is_ident_char(body[i - 1])) {
            if (starts_with(body + i, "if") && !is_ident_char(body[i + 2]))
                cc++;
            else if (starts_with(body + i, "else") && !is_ident_char(body[i + 4]))
                cc++;
            else if (starts_with(body + i, "for") && !is_ident_char(body[i + 3]))
                cc++;
            else if (starts_with(body + i, "while") && !is_ident_char(body[i + 5]))
                cc++;
            else if (starts_with(body + i, "case") && !is_ident_char(body[i + 4]))
                cc++;
            else if (starts_with(body + i, "&&"))
                cc++;
            else if (starts_with(body + i, "||"))
                cc++;
            else if (starts_with(body + i, "?") && body[i] == '?')
                cc++;
        }
    }
    return cc;
...
ast_file_t *ast_parse_file(const char *path) {
    ...
    for (size_t i = 0; i <= file_len; i++) {
        if (i == file_len || src[i] == '\n') {
            ...
            if (line_len == 0 || (line_len == 1 && src[line_start] == '\r')) {
                ...
                for (int j = line_start; j < (int)i; j++) {
                    if (!isspace((unsigned char)src[j])) {
                        only_ws = false;
                        break;
                    }
                }
        ...
        if (!in_line_comment && !isspace((unsigned char)src[i]))
            has_code = true;
    }
    ...
    while (*p) {
        ...
        if (*p == '(') {
            ...
            const char *paren = p;
            p--;
            while (p > decl_start && isspace((unsigned char)*p))
                p--;
            const char *name_end = p + 1;
            while (p > decl_start && is_ident_char(*p))
                p--;
            if (!is_ident_char(*p))
                p++;
            ...
            if (name_end > name_start) {
                ...
                while (rt_len > 0 && isspace((unsigned char)return_type_start[rt_len - 1]))
                    rt_len--;
...
static void ast_collect_dir(ast_project_t *proj, const char *dir_path, int depth) {
    ...
    while ((entry = readdir(dir))) {
        ...
        if (!S_ISREG(st.st_mode) || !is_c_or_h(name))
            continue;
        ...
        proj->total_structs += ast_count_type(af, AST_STRUCT) + ast_count_type(af, AST_TYPEDEF);
...
int ast_function_list_json(ast_file_t *f, char *buf, size_t len) {
    ...
    bool first = true;
    for (int i = 0; i < f->count; i++) {
        if (f->nodes[i].type != AST_FUNCTION)
            continue;
        if (!first)
            jbuf_append(&b, ",");
        first = false;
