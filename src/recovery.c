...
bool execute_with_fallback(recovery_fn_t primary_fn, recovery_fn_t *fallback_fns, int n_fallbacks,
    ...
    if (primary_fn(arg, errbuf, errlen))
        return true;

    for (int i = 0; i < n_fallbacks; i++) {
        if (!fallback_fns[i])
            continue;
        if (fallback_fns[i](arg, errbuf, errlen))
            return true;
    }
    return false;
}
...
static int collect_step_atoms_r(int step_id, int *out, int cap) {
    ...
    for (int i = 0; i < s->child_step_count && n < cap; i++)
        n += collect_step_atoms_r(s->child_step_ids[i], out + n, cap - n);
    return n;
}
...
static int collect_plan_atoms(int plan_id, int *out, int cap) {
    ...
    for (int i = 0; i < plan->root_step_count && n < cap; i++)
        n += collect_step_atoms_r(plan->root_step_ids[i], out + n, cap - n);
    return n;
}
...
int recovery_log_dump(const recovery_log_t *log, const char *path) {
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        if (!e)
            continue;
        fprintf(fp, "%ld,%d,%d,%s,%s\n", (long)e->timestamp, e->step_id, e->attempt,
                recovery_action_name(e->action_taken), e->error_str[0] ? e->error_str : "(none)");
    }
...
const char *recovery_action_name(recovery_action_t a) {
    switch (a) {
        ...
        case RECOVERY_ACTION_FALLBACK:
            return "fallback";
        case RECOVERY_ACTION_BACKTRACK:
            return "backtrack";
        case RECOVERY_ACTION_GIVE_UP:
            return "give_up";
        default:
            return "unknown";
    }
