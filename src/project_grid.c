...
static __attribute__((unused)) void free_tile_recursive(dsco_grid_t *g, int id) {
    ...
    if (!t->in_use) return;
    for (int i = 0; i < t->child_count; i++) free_tile_recursive(g, t->child_ids[i]);
    t->in_use = false;
    t->child_count = 0;
    t->project_idx = -1;
}
    ...
    *hue_out = hue_in;
    switch (th) {
        ...
        case DSCO_THEME_MONO:
            *hue_out = 0; *s = 0.0f; *l = 0.55f;
            break;
        case DSCO_THEME_TERMINAL:
        default:
            /* avoid the muddy yellow-green band */
            if (hue_in > 50 && hue_in < 90) *hue_out = hue_in + 50;
            *s = 0.62f; *l = 0.62f;
            break;
    }
}
...
dsco_rgb_t dsco_grid_state_color(int state) {
    ...
    switch (state) {
        case 0: { dsco_rgb_t c = {120, 120, 130}; return c; }  /* IDLE — slate */
        case 1: { dsco_rgb_t c = {72, 200, 130}; return c; }   /* RUNNING — green */
        case 2: { dsco_rgb_t c = {235, 190, 80}; return c; }   /* PAUSED — amber */
        case 3: { dsco_rgb_t c = {235, 80, 90}; return c; }    /* QUARANTINED — red */
        case 4: { dsco_rgb_t c = {90, 90, 100}; return c; }    /* CLOSED — dim */
        case 5: { dsco_rgb_t c = {210, 70, 70}; return c; }    /* DEAD — dark red */
    }
...
dsco_border_glyphs_t dsco_grid_border_glyphs(dsco_border_t b) {
    ...
    switch (b) {
        case DSCO_BORDER_SINGLE:
            g.h = "─"; g.v = "│"; g.tl = "┌"; g.tr = "┐"; g.bl = "└"; g.br = "┘"; break;
...
void dsco_grid_set_preset(dsco_grid_t *g, int rows, int cols) {
    ...
    if (cols < 1) cols = 1;
    /* free everything */
    for (int i = 0; i < DSCO_GRID_MAX_TILES; i++) g->tiles[i].in_use = false;
    g->root_id = -1;
    g->focused_id = -1;
    g->zoomed_id = -1;
...
void dsco_grid_cycle_preset(dsco_grid_t *g) {
    ...
    dsco_grid_set_preset(g, k_preset_table[g->preset_idx][0],
                            k_preset_table[g->preset_idx][1]);
}
...
static void leaves_dfs(const dsco_grid_t *g, int id, int *leaves, int *count, int cap) {
    if (id < 0 || id >= DSCO_GRID_MAX_TILES || *count >= cap) return;
    const dsco_tile_t *t = &g->tiles[id];
    if (!t->in_use) return;
    if (t->kind == DSCO_TILE_LEAF) { leaves[(*count)++] = id; return; }
    for (int i = 0; i < t->child_count; i++) leaves_dfs(g, t->child_ids[i], leaves, count, cap);
}
...
void dsco_grid_assign_projects(dsco_grid_t *g, int project_count) {
    ...
    if (g->focused_id < 0 && n > 0) g->focused_id = leaves[0];
}
...
void dsco_grid_foreach_leaf(const dsco_grid_t *g, dsco_grid_leaf_cb cb, void *ctx) {
    ...
    int n = 0;
    leaves_dfs(g, g->root_id, leaves, &n, DSCO_GRID_MAX_TILES);
    for (int i = 0; i < n; i++) cb(&g->tiles[leaves[i]], ctx);
}
...
int dsco_grid_find_leaf(const dsco_grid_t *g, int project_idx) {
    ...
    leaves_dfs(g, g->root_id, leaves, &n, DSCO_GRID_MAX_TILES);
    for (int i = 0; i < n; i++) {
        if (g->tiles[leaves[i]].project_idx == project_idx) return leaves[i];
    }
    return -1;
}

void dsco_grid_focus(dsco_grid_t *g, int tile_id) {
    if (tile_id < 0 || tile_id >= DSCO_GRID_MAX_TILES) return;
    if (!g->tiles[tile_id].in_use) return;
    if (g->tiles[tile_id].kind != DSCO_TILE_LEAF) return;
    g->focused_id = tile_id;
}

void dsco_grid_focus_project(dsco_grid_t *g, int project_idx) {
    int tid = dsco_grid_find_leaf(g, project_idx);
    if (tid >= 0) g->focused_id = tid;
}
...
int dsco_grid_split(dsco_grid_t *g, int tile_id, dsco_tile_kind_t dir) {
    if (tile_id < 0 || tile_id >= DSCO_GRID_MAX_TILES) return -1;
    dsco_tile_t *t = &g->tiles[tile_id];
    if (!t->in_use || t->kind != DSCO_TILE_LEAF) return -1;
    if (dir != DSCO_TILE_HSPLIT && dir != DSCO_TILE_VSPLIT) return -1;
    ...
    if (new_leaf < 0 || new_sibling < 0) return -1;
...
void dsco_grid_zoom_toggle(dsco_grid_t *g) {
    if (g->zoomed_id >= 0) { g->zoomed_id = -1; return; }
...
void dsco_grid_focus_dir(dsco_grid_t *g, int dx, int dy) {
    ...
    for (int i = 0; i < DSCO_GRID_MAX_TILES; i++) {
        if (!g->tiles[i].in_use || g->tiles[i].kind != DSCO_TILE_LEAF) continue;
        if (i == g->focused_id) continue;
        ...
        long score;
        if (dx != 0) score = (long)labs(dc) * 1 + (long)labs(dr) * 4;
        else         score = (long)labs(dr) * 1 + (long)labs(dc) * 4;
        if (score < best_score) { best_score = score; best_id = i; }
    }
    if (best_id >= 0) g->focused_id = best_id;
}
...
void dsco_grid_layout(dsco_grid_t *g, int row0, int col0, int rows, int cols) {
    if (g->zoomed_id >= 0 && g->tiles[g->zoomed_id].in_use) {
        ...
        t->rect.row0 = row0; t->rect.col0 = col0; t->rect.rows = rows; t->rect.cols = cols;
        return;
    }
...
static void layout_recursive(dsco_grid_t *g, int id, int r0, int c0, int rows, int cols) {
    ...
    dsco_tile_t *t = &g->tiles[id];
    if (!t->in_use) return;
    t->rect.row0 = r0; t->rect.col0 = c0; t->rect.rows = rows; t->rect.cols = cols;
    if (t->kind == DSCO_TILE_LEAF || t->child_count == 0) return;
    ...
    if (total_w == 0.0f) total_w = (float)t->child_count;
    ...
    for (int i = 0; i < t->child_count; i++) {
        ...
        if (t->kind == DSCO_TILE_VSPLIT) {
            int cw = (i == t->child_count - 1)
                     ? cols - used
                     : (int)((float)cols * w / total_w);
            layout_recursive(g, t->child_ids[i], r0, c0 + used, rows, cw);
            used += cw;
        } else { /* HSPLIT */
            int ch = (i == t->child_count - 1)
                     ? rows - used
                     : (int)((float)rows * w / total_w);
