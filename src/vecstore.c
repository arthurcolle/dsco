...
vecstore_t *vecstore_open(vfs_db_t *vfs, const char *collection) {
    if (!vfs || !collection || !collection[0]) return NULL;
...
bool vecstore_insert(vecstore_t *vs, const char *id,
    ...
    if (!vs || !id || !vec || dim <= 0 || dim > VECSTORE_MAX_DIM) return false;
...
float *vecstore_get(vecstore_t *vs, const char *id, int *out_dim) {
    ...
    if (!blob || blob_len < sizeof(float)) return NULL;
...
int vecstore_query(vecstore_t *vs, const float *query_vec, int dim,
    ...
    if (!vs || !query_vec || dim <= 0 || !out || max_results <= 0) return 0;
    ...
    for (int i = 0; i < key_count && n_cand < cap; i++) {
        ...
        if (!vec) { free(keys[i]); keys[i] = NULL; continue; }
