...
static const char *const RARITY_NAMES[PET_RARITY_COUNT] = {
...
static pet_rarity_t roll_rarity(uint32_t *rng) {
    int total = 0;
    for (int i = 0; i < PET_RARITY_COUNT; i++) total += RARITY_WEIGHTS[i];
    double roll = mulberry(rng) * total;
    for (int i = 0; i < PET_RARITY_COUNT; i++) {
        roll -= RARITY_WEIGHTS[i];
        if (roll < 0) return (pet_rarity_t)i;
    }
...
static void roll_stats(uint32_t *rng, pet_rarity_t rarity, int out[PET_STAT_COUNT]) {
    ...
    while (dump == peak) dump = pick(rng, PET_STAT_COUNT);
    for (int i = 0; i < PET_STAT_COUNT; i++) {
        int v;
        if (i == peak)      v = floor + 50 + (int)(mulberry(rng) * 30);
        else if (i == dump) v = floor - 10 + (int)(mulberry(rng) * 15);
        else                v = floor + (int)(mulberry(rng) * 40);
        if (v > 100) v = 100;
        if (v < 1)   v = 1;
        out[i] = v;
    }
...
void pet_roll(const char *seed, pet_bones_t *out) {
    ...
    out->hat     = (rarity == PET_RARITY_COMMON)
                     ? PET_HAT_NONE
                     : (pet_hat_t)pick(&rng, PET_HAT_COUNT);
...
static void subst_eye(const char *src, const char *eye, char *dst, size_t n) {
    ...
    for (size_t i = 0; src[i] && di + 8 < n; ) {
        if (src[i] == '{' && src[i+1] == 'E' && src[i+2] == '}') {
            for (const char *e = eye; *e && di + 1 < n; e++) dst[di++] = *e;
            i += 3;
        } else {
            dst[di++] = src[i++];
        }
...
int pet_sprite_frame_count(pet_species_t s) { (void)s; return 3; }
...
int pet_render_sprite(const pet_bones_t *b, int frame, char lines[6][96]) {
    ...
    char tmp[5][96];
    for (int i = 0; i < 5; i++) subst_eye(body[i], eye, tmp[i], sizeof(tmp[i]));
    ...
    if (line_blank(tmp[0])) {
        ...
        for (int ff = 0; ff < 3; ff++)
            if (!line_blank(BODIES[sp][ff][0])) { all_blank = 0; break; }
        if (all_blank) start = 1;
    }
...
void pet_render_face(const pet_bones_t *b, char *buf, size_t n) {
    ...
    switch (b->species) {
        case PET_DUCK: case PET_GOOSE: snprintf(buf, n, "(%s>", e); break;
        case PET_BLOB:                 snprintf(buf, n, "(%s%s)", e, e); break;
        case PET_CAT:                  snprintf(buf, n, "=%sω%s=", e, e); break;
        case PET_DRAGON:               snprintf(buf, n, "<%s~%s>", e, e); break;
        case PET_OCTOPUS:              snprintf(buf, n, "~(%s%s)~", e, e); break;
        case PET_OWL:                  snprintf(buf, n, "(%s)(%s)", e, e); break;
        case PET_PENGUIN:              snprintf(buf, n, "(%s>)", e); break;
        case PET_TURTLE:               snprintf(buf, n, "[%s_%s]", e, e); break;
        case PET_SNAIL:                snprintf(buf, n, "%s(@)", e); break;
        case PET_GHOST:                snprintf(buf, n, "/%s%s\\", e, e); break;
        case PET_AXOLOTL:              snprintf(buf, n, "}%s.%s{", e, e); break;
        case PET_CAPYBARA:             snprintf(buf, n, "(%soo%s)", e, e); break;
        case PET_CACTUS:               snprintf(buf, n, "|%s  %s|", e, e); break;
        case PET_ROBOT:                snprintf(buf, n, "[%s%s]", e, e); break;
...
const char *pet_rarity_color(pet_rarity_t r) {
    switch (r) {
        case PET_RARITY_COMMON:    return TUI_DIM;
        case PET_RARITY_UNCOMMON:  return TUI_GREEN;
        case PET_RARITY_RARE:      return TUI_BCYAN;
        case PET_RARITY_EPIC:      return TUI_BMAGENTA;
        case PET_RARITY_LEGENDARY: return TUI_BYELLOW;
...
const char *pet_status_glyph(pet_status_t st) {
    switch (st) {
        case PET_ST_PENDING: return "·";
        case PET_ST_WORKING: return "◉";
        case PET_ST_DONE:    return "✓";
        case PET_ST_ERROR:   return "✗";
        case PET_ST_IDLE:    return "-";
        default:             return "·";
    }
}
const char *pet_status_color(pet_status_t st) {
    switch (st) {
        case PET_ST_PENDING: return TUI_DIM;
        case PET_ST_WORKING: return TUI_BCYAN;
        case PET_ST_DONE:    return TUI_GREEN;
        case PET_ST_ERROR:   return TUI_RED;
        case PET_ST_IDLE:    return TUI_DIM;
        default:             return TUI_RESET;
    }
}
const char *pet_status_word(pet_status_t st) {
    switch (st) {
        case PET_ST_PENDING: return "pending";
        case PET_ST_WORKING: return "working";
        case PET_ST_DONE:    return "done";
        case PET_ST_ERROR:   return "error";
        case PET_ST_IDLE:    return "idle";
        default:             return "?";
    }
}
...
static pet_bones_t bones_for_status(const pet_bones_t *src, pet_status_t st) {
    pet_bones_t b = *src;
    switch (st) {
        case PET_ST_ERROR:   b.eye = 2; break; /* × */
        case PET_ST_DONE:    b.eye = 3; break; /* ◉ */
        case PET_ST_PENDING: b.eye = 0; break; /* · */
        default: break;
    }
    return b;
}
...
static void emit_sprite_line(FILE *out, const char *line, const pet_bones_t *b,
                             int frame, int row) {
...
static pet_t *roster_find(pet_roster_t *r, int id) {
    for (int i = 0; i < r->count; i++)
        if (r->pets[i].id == id) return &r->pets[i];
    return NULL;
}
...
int pet_roster_upsert(pet_roster_t *r, int id, int project_id,
    ...
    if (!p) {
        if (r->count >= r->cap) {
            ...
            if (!np) { pthread_mutex_unlock(&r->mu); return -1; }
            r->pets = np;
            r->cap = nc;
        }
        p = &r->pets[r->count++];
    ...
    if (status == PET_ST_DONE || status == PET_ST_ERROR)
        if (p->end_time == 0) p->end_time = pet_now();
...
void pet_roster_set_status(pet_roster_t *r, int id, pet_status_t status, double cost_usd) {
    ...
    if (p) {
        p->status = status;
        if (cost_usd > 0) p->cost_usd = cost_usd;
...
void pet_roster_counts(pet_roster_t *r, int *pending, int *working,
    ...
    for (int i = 0; i < r->count; i++) {
        switch (r->pets[i].status) {
            case PET_ST_PENDING: pe++; break;
            case PET_ST_WORKING: wo++; break;
            case PET_ST_DONE:    dn++; break;
            case PET_ST_ERROR:   er++; break;
            default: break;
        }
    }
    if (total) *total = r->count;
    pthread_mutex_unlock(&r->mu);
    if (pending) *pending = pe;
    if (working) *working = wo;
    if (done)    *done = dn;
    if (error)   *error = er;
}
...
static int status_priority(pet_status_t s) {
    switch (s) {
        case PET_ST_WORKING: return 0;
        case PET_ST_PENDING: return 1;
        case PET_ST_IDLE:    return 2;
        case PET_ST_ERROR:   return 3;
        case PET_ST_DONE:    return 4;
        default:             return 5;
    }
}
...
static void render_spark(FILE *out, const pet_t *p) {
    ...
    if (n <= 0) return;
    double mx = 0;
    for (int i = 0; i < n; i++) if (p->activity_ring[i] > mx) mx = p->activity_ring[i];
    if (mx <= 0) return;
    fputs(TUI_DIM, out);
    for (int i = 0; i < n; i++) {
...
void pet_roster_render(FILE *out, pet_roster_t *r, int width, int max_rows) {
    ...
    if (!order) { pthread_mutex_unlock(&r->mu); return; }
    ...
    fprintf(out, "  %s%sPETS%s  %s%d agents%s  %s◉ %d%s %s· %d%s %s✓ %d%s %s✗ %d%s\n",
            TUI_BOLD, TUI_BCYAN, TUI_RESET, TUI_DIM, n, TUI_RESET,
            TUI_BCYAN, wo, TUI_RESET, TUI_DIM, pe, TUI_RESET,
            TUI_GREEN, dn, TUI_RESET, TUI_RED, er, TUI_RESET);
    ...
    for (int oi = 0; oi < limit; oi++) {
        ...
        const char *bob = (p->status == PET_ST_WORKING)
                          ? ((frame / 2) % 2 ? " " : "") : "";

        char lbl[40];
        snprintf(lbl, sizeof(lbl), "%-20.20s", p->label[0] ? p->label : "agent");

        fprintf(out, "  %s%s%s%-8s%s %s#%-3d%s %s%s%s ",
                pet_status_color(p->status), pet_status_glyph(p->status), TUI_RESET,
                "", "",                                  /* spacing kept simple */
                TUI_DIM, p->id, TUI_RESET,
                pet_rarity_color(p->bones.rarity), face, TUI_RESET);
        fprintf(out, "%s%s%s%s ", bob, TUI_BOLD, lbl, TUI_RESET);

        fprintf(out, "%s%-7s%s", pet_status_color(p->status), pet_status_word(p->status), TUI_RESET);
        if (p->cost_usd > 0)
...
int pet_roster_next_unnotified(pet_roster_t *r, pet_status_t *out_status,
    ...
    for (int i = 0; i < r->count; i++) {
        ...
        if (!p->notified && (p->status == PET_ST_DONE || p->status == PET_ST_ERROR)) {
            p->notified = true;
            id = p->id;
            if (out_status) *out_status = p->status;
            if (out_bones)  *out_bones = p->bones;
...
static void pet_roster_global_init(void) { pet_roster_init(&g_pet_roster); }
