...
static void walk_loc(const char *dir, const char * const *exts, int nexts,
                     long *loc_out, long *files_out) {
    DIR *d = opendir(dir);
    if (!d) return;
    struct dirent *de;
    char path[4096];
...
static int self_exe_path(char *buf, size_t len) {
    ...
    if (!p) return -1;
    if (!fgets(buf, len, p)) { pclose(p); return -1; }
    pclose(p);
    size_t k = strlen(buf);
    while (k && (buf[k-1] == '\n' || buf[k-1] == '\r')) buf[--k] = 0;
    return buf[0] ? 0 : -1;
...
int introspect_print_codebase_stats(FILE *out) {
    ...
    fprintf(out, "  binary  : ");
    char exe[4096] = {0};
    if (self_exe_path(exe, sizeof(exe)) == 0) fprintf(out, "%s\n", exe);
    ...
    fprintf(out, "  reg cap : %d slots (always=%d warm=%d working=%d disc=%d)\n",
            TOOL_REGISTER_CAP, TOOL_REG_ALWAYS, TOOL_REG_WARM,
            TOOL_REG_WORKING, TOOL_REG_DISCOVERY);
...
static int spawn_self(const char *exe, const char *angle, const char *prompt,
                      self_child_t *child) {
    int pipefd[2];
    ...
    if (!full) { close(pipefd[0]); close(pipefd[1]); return -1; }
    snprintf(full, plen, "%s%s", angle, prompt);

    pid_t pid = fork();
    if (pid < 0) { free(full); close(pipefd[0]); close(pipefd[1]); return -1; }
...
static void child_append(self_child_t *c, const char *data, size_t n) {
    ...
    if (c->buf_len + n + 1 > c->buf_cap) {
        ...
        while (nc < c->buf_len + n + 1) nc *= 2;
        if (nc > INTROSPECT_CHILD_BUF) nc = INTROSPECT_CHILD_BUF;
        if (c->buf_len + n + 1 > nc) {
            /* truncate quietly */
            n = nc - c->buf_len - 1;
...
int introspect_run_selves(FILE *out, int n, const char *prompt) {
    ...
    if (n < 1) n = 3;
    if (n > INTROSPECT_MAX_SELVES) n = INTROSPECT_MAX_SELVES;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (spawn_self(exe, SELF_ANGLES[i], prompt, &children[i]) == 0) {
            ...
            fprintf(out, "  ✓ spawned self #%d %.*s (pid=%d)\n", i,
                    20, SELF_ANGLES[i], (int)children[i].pid);
        } else {
    ...
    while (live > 0) {
        for (int i = 0; i < n; i++) {
            ...
            if (r > 0) child_append(c, buf, (size_t)r);
            else if (r == 0) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        fprintf(out, "\n── self #%d %.*s (exit=%d, %zu bytes) ──\n",
                i, 20, SELF_ANGLES[c->angle_idx], c->exit_code, c->buf_len);
