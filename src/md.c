...
static void fput_safe_span(FILE *out, const char *s, size_t n) {
    for (size_t i = 0; i < n; i++)
        fput_safe_byte(out, (unsigned char)s[i]);
}
...
static bool is_fence_closer(const char *line, char open_ch, int open_len) {
    ...
    if (!parse_fence_line(line, &ch, &n, &after))
        return false;
    if (ch != open_ch || n < open_len)
        return false;
    while (*after) {
        if (*after != ' ' && *after != '\t')
            return false;
...
static const char *SUPER_DIGITS[] = {"\xE2\x81\xB0", "\xC2\xB9",     "\xC2\xB2",     "\xC2\xB3",
                                     "\xE2\x81\xB4", "\xE2\x81\xB5", "\xE2\x81\xB6", "\xE2\x81\xB7",
                                     "\xE2\x81\xB8", "\xE2\x81\xB9"};

/* Unicode subscript digits */
static const char *SUB_DIGITS[] = {"\xE2\x82\x80", "\xE2\x82\x81", "\xE2\x82\x82", "\xE2\x82\x83",
...
static const latex_func_t LATEX_FUNCS[] = {{"\\sin", "sin"},
                                           {"\\cos", "cos"},
                                           {"\\tan", "tan"},
                                           {"\\cot", "cot"},
                                           {"\\sec", "sec"},
...
static const emoji_entry_t EMOJI_TABLE[] = {{":rocket:", "\xF0\x9F\x9A\x80"},
                                            {":star:", "\xE2\xAD\x90"},
                                            ...
                                            {":check:", "\xE2\x9C\x94\xEF\xB8\x8F"},
                                            {":cross:", "\xE2\x9C\x96\xEF\xB8\x8F"},
                                            {":plus:", "\xE2\x9E\x95"},
                                            {":minus:", "\xE2\x9E\x96"},
...
static void render_inline(FILE *out, const char *text) {
    ...
    while (*p) {
        ...
        if ((p[0] == '*' && p[1] == '*' && p[2] == '*') ||
            (p[0] == '_' && p[1] == '_' && p[2] == '_' && underscore_can_open(text, p))) {
            char marker = p[0];
            const char *end = p + 3;
            while (*end) {
                if (end[0] == marker && end[1] == marker && end[2] == marker &&
                    (marker == '*' || underscore_can_close(end, 3)))
                    break;
                end++;
            }
            if (*end) {
                ...
                int n = (int)(end - p - 3);
                if (n >= (int)sizeof(inner))
                    n = (int)sizeof(inner) - 1;
                memcpy(inner, p + 3, n);
                inner[n] = '\0';
        ...
        if ((p[0] == '*' && p[1] == '*' && p[2] != '*') ||
            ...
            while (*end) {
                if (end[0] == marker && end[1] == marker &&
                    (marker == '*' || underscore_can_close(end, 2)))
                    break;
                end++;
            }
            if (*end) {
        ...
        if ((*p == '*' || (*p == '_' && underscore_can_open(text, p))) && p[1] != *p &&
            p[1] != ' ') {
            char marker = *p;
            const char *end = p + 1;
            while (*end && *end != '\n') {
                if (*end == marker && (end == p + 1 || *(end - 1) != ' ') &&
                    (marker == '*' || underscore_can_close(end, 1)))
                    break;
                end++;
            }
        ...
        if (*p == '<') {
            ...
            if (p[1] == 'h' && p[2] >= '1' && p[2] <= '6' && p[3] == '>') {
                ...
                if (close) {
                    const char *hcolors[] = {TUI_BWHITE,   TUI_BCYAN, TUI_BBLUE,
                                             TUI_BMAGENTA, TUI_DIM,   TUI_DIM};
        ...
        if (*p == '&') {
            ...
            if (str_starts(p, "&mdash;")) {
                fputs("\xE2\x80\x94", out);
                p += 7;
                continue;
            }
            if (str_starts(p, "&ndash;")) {
                fputs("\xE2\x80\x93", out);
                p += 7;
                continue;
            }
            if (str_starts(p, "&hellip;")) {
                fputs("\xE2\x80\xA6", out);
                p += 8;
                continue;
            }
            if (str_starts(p, "&laquo;")) {
                fputs("\xC2\xAB", out);
                p += 7;
                continue;
            }
            if (str_starts(p, "&raquo;")) {
                fputs("\xC2\xBB", out);
                p += 7;
                continue;
            }
            if (str_starts(p, "&lsquo;")) {
                fputs("\xE2\x80\x98", out);
                p += 7;
                continue;
            }
            if (str_starts(p, "&rsquo;")) {
                fputs("\xE2\x80\x99", out);
                p += 7;
                continue;
            }
            if (str_starts(p, "&ldquo;")) {
                fputs("\xE2\x80\x9C", out);
                p += 7;
                continue;
            }
            if (str_starts(p, "&rdquo;")) {
                fputs("\xE2\x80\x9D", out);
                p += 7;
                continue;
            }
            if (str_starts(p, "&bull;")) {
                fputs("\xE2\x80\xA2", out);
                p += 6;
                continue;
            }
            if (str_starts(p, "&trade;")) {
                fputs("\xE2\x84\xA2", out);
                p += 7;
                continue;
            }
            if (str_starts(p, "&copy;")) {
                fputs("\xC2\xA9", out);
                p += 6;
                continue;
            }
            ...
            if (str_starts(p, "&micro;")) {
                fputs("\xC2\xB5", out);
                p += 7;
                continue;
            }
            if (str_starts(p, "&para;")) {
                fputs("\xC2\xB6", out);
                p += 6;
                continue;
            }
            if (str_starts(p, "&sect;")) {
                fputs("\xC2\xA7", out);
                p += 6;
                continue;
            }
            if (str_starts(p, "&euro;")) {
                fputs("\xE2\x82\xAC", out);
                p += 6;
                continue;
            }
            if (str_starts(p, "&pound;")) {
                fputs("\xC2\xA3", out);
                p += 7;
                continue;
            }
            ...
            if (str_starts(p, "&cent;")) {
                fputs("\xC2\xA2", out);
                p += 6;
                continue;
            }
            if (str_starts(p, "&times;")) {
                fputs("\xC3\x97", out);
                p += 7;
                continue;
            }
            if (str_starts(p, "&divide;")) {
                fputs("\xC3\xB7", out);
                p += 8;
                continue;
            }
            if (str_starts(p, "&plusmn;")) {
                fputs("\xC2\xB1", out);
                p += 8;
                continue;
            }
            if (str_starts(p, "&frac12;")) {
                fputs("\xC2\xBD", out);
                p += 8;
                continue;
            }
            if (str_starts(p, "&frac14;")) {
                fputs("\xC2\xBC", out);
                p += 8;
                continue;
            }
            if (str_starts(p, "&frac34;")) {
                fputs("\xC2\xBE", out);
                p += 8;
                continue;
            }
            if (str_starts(p, "&larr;")) {
                fputs("\xE2\x86\x90", out);
                p += 6;
                continue;
            }
            if (str_starts(p, "&rarr;")) {
                fputs("\xE2\x86\x92", out);
                p += 6;
                continue;
            }
            if (str_starts(p, "&uarr;")) {
                fputs("\xE2\x86\x91", out);
                p += 6;
                continue;
            }
            if (str_starts(p, "&darr;")) {
                fputs("\xE2\x86\x93", out);
                p += 6;
                continue;
            }
            if (str_starts(p, "&harr;")) {
                fputs("\xE2\x86\x94", out);
                p += 6;
                continue;
            }
            if (str_starts(p, "&infin;")) {
                fputs("\xE2\x88\x9E", out);
                p += 7;
                continue;
            }
...
static bool parse_table_separator(char **cells, int cols, md_align_t *align) {
    ...
    for (int c = 0; c < cols; c++) {
        ...
        if (left_colon && right_colon)
            align[c] = MD_ALIGN_CENTER;
        else if (right_colon)
            align[c] = MD_ALIGN_RIGHT;
        else
            align[c] = MD_ALIGN_LEFT;
    }
...
static bool table_store_row(md_renderer_t *r, char **cells, int cols) {
    if (!r || r->table_rows >= MD_TABLE_MAXROW)
        return false;
    ...
    for (int row = 0; row < r->table_rows; row++) {
        for (int c = 0; c < r->table_cols; c++) {
            ...
            if (r->table_cells[row][c])
                render_inline(out, r->table_cells[row][c]);
        }
        fprintf(out, "\n");
    }
}
...
static void flush_table_or_fallback(FILE *out, md_renderer_t *r) {
    ...
    if (r->table_has_sep && r->table_rows > 0)
        render_table(out, r);
    else
        render_table_fallback(out, r);
    table_free(r);
...
static int md_rendered_width(const char *text) {
    ...
    if (!tmp)
        return vis_len(text); /* fallback */
    render_inline(tmp, text);
    fflush(tmp);
    long pos = ftell(tmp);
    fclose(tmp);
    if (pos >= 0 && pos < (long)sizeof(buf))
        buf[pos] = '\0';
    else
        buf[0] = '\0';
    return vis_len(buf);
...
static bool cell_is_total(const char *cell) {
    ...
    if (strncmp(cell, "**", 2) == 0)
        return true;
    if (strncasecmp(cell, "Total", 5) == 0)
        return true;
    if (strncasecmp(cell, "Grand Total", 11) == 0)
        return true;
    if (strncasecmp(cell, "Subtotal", 8) == 0)
        return true;
    if (strncasecmp(cell, "Net", 3) == 0)
        return true;
    if (strncasecmp(cell, "Sum", 3) == 0)
        return true;
    return false;
}
...
static void render_table(FILE *out, md_renderer_t *r) {
    ...
    for (int row = 0; row < r->table_rows; row++) {
        ...
        for (int c = 0; c < r->table_cols; c++) {
            const char *cell =
                (r->table_cells[row][c] && r->table_cells[row][c][0]) ? r->table_cells[row][c] : "";
...
const char *md_lang_from_path(const char *path) {
    ...
    for (int i = 0; exts[i].ext; i++) {
        size_t elen = strlen(exts[i].ext);
        if (path_len >= elen && strncasecmp(path + path_len - elen, exts[i].ext, elen) == 0)
            return exts[i].lang;
    }
    return NULL;
}
...
void md_render_code_line(FILE *out, const char *line, const char *lang) {
    ...
    bool highlight = is_c || is_py || is_js || is_sh || is_rust || is_go || is_java || is_rb ||
                     is_sql || is_html || is_css || is_json || is_yaml || is_md;
        ...
        if (isalpha((unsigned char)*p) || *p == '_') {
            ...
            if (is_c) {
                const char *kws[] = {
                    ...
                    "case",      "break",    "continue", "return",   "goto",      "struct",
                    "union",     "enum",     "typedef",  "const",    "static",    "extern",
                    "volatile",  "inline",   "sizeof",   "void",     "int",       "char",
                    "long",      "short",    "unsigned", "signed",   "float",     "double",
                    "bool",      "true",     "false",    "NULL",     "auto",      "register",
                    "restrict",  "#include", "#define",  "#ifdef",   "#ifndef",   "#endif",
                    "#if",       "#else",    "#elif",    "class",    "public",    "private",
                    "protected", "virtual",  "override", "template", "namespace", "using",
                    "new",       "delete",   "try",      "catch",    "throw",     NULL};
                for (int i = 0; kws[i]; i++) {
            ...
            if (is_js) {
                const char *kws[] = {
                    ...
                    "continue",  "return",     "class",    "extends", "new",       "delete",
                    "typeof",    "instanceof", "this",     "super",   "import",    "export",
                    "from",      "default",    "try",      "catch",   "finally",   "throw",
                    "async",     "await",      "yield",    "true",    "false",     "null",
                    "undefined", "void",       "of",       "in",      "interface", "type",
                    "enum",      "implements", "abstract", "private", "public",    "protected",
                    "readonly",  "static",     "declare",  "module",  "namespace", NULL};
                for (int i = 0; kws[i]; i++) {
                    if (strcmp(word, kws[i]) == 0) {
                        is_kw = true;
                        break;
            ...
            if (is_sh) {
                const char *kws[] = {"if",       "then",   "else", "elif",  "fi",     "for",
                                     "while",    "do",     "done", "case",  "esac",   "in",
                                     "function", "return", "exit", "local", "export", "source",
                                     "echo",     "printf", "read", "set",   "unset",  "shift",
                                     "eval",     "exec",   "true", "false", "test",   NULL};
                for (int i = 0; kws[i]; i++) {
                    if (strcmp(word, kws[i]) == 0) {
                        is_kw = true;
                        break;
            ...
            if (is_rust) {
                const char *kws[] = {
                    ...
                    "loop",   "match",  "return", "break",  "continue", "struct", "enum",  "impl",
                    "trait",  "pub",    "use",    "mod",    "crate",    "self",   "super", "Self",
                    "type",   "where",  "async",  "await",  "move",     "ref",    "as",    "in",
                    "true",   "false",  "None",   "Some",   "Ok",       "Err",    "Box",   "Vec",
                    "String", "Option", "Result", "unsafe", "dyn",      NULL};
                for (int i = 0; kws[i]; i++) {
                    if (strcmp(word, kws[i]) == 0) {
                        is_kw = true;
                        break;
            ...
            if (is_go) {
                const char *kws[] = {
                    ...
                    "switch",  "case",    "break",  "continue", "return", "struct", "interface",
                    "type",    "map",     "chan",   "go",       "defer",  "select", "package",
                    "import",  "true",    "false",  "nil",      "append", "make",   "new",
                    "len",     "cap",     "delete", "copy",     "close",  "panic",  "recover",
                    "print",   "println", "error",  "string",   "int",    "bool",   "byte",
            ...
            if (is_java) {
                const char *kws[] = {
                    ...
                    "static",   "final",     "abstract", "void",       "int",
                    "long",     "double",    "float",    "boolean",    "char",
                    "byte",     "short",     "if",       "else",       "for",
                    "while",    "do",        "switch",   "case",       "break",
                    "continue", "return",    "new",      "this",       "super",
                    "try",      "catch",     "finally",  "throw",      "throws",
                    "null",     "true",      "false",    "instanceof", "synchronized",
                    "volatile", "transient", "var",      "val",        "fun",
                    "when",     "is",        "in",       "object",     "companion",
                    "data",     "sealed",    "override", "open",       "lateinit",
                    "suspend",  "coroutine", NULL};
                for (int i = 0; kws[i]; i++) {
                    if (strcmp(word, kws[i]) == 0) {
                        is_kw = true;
                        break;
            ...
            if (is_sql) {
                ...
                const char *kws[] = {
                    ...
                    "INNER",   "OUTER",  "ON",      "AND",        "OR",       "NOT",
                    "IN",      "IS",     "NULL",    "AS",         "ORDER",    "BY",
                    "GROUP",   "HAVING", "LIMIT",   "OFFSET",     "DISTINCT", "COUNT",
                    "SUM",     "AVG",    "MIN",     "MAX",        "BETWEEN",  "LIKE",
                    ...
                    "DEFAULT", "CHECK",  "UNIQUE",  "IF",         "WITH",     NULL};
                for (int i = 0; kws[i]; i++) {
                    if (strcmp(upper, kws[i]) == 0) {
                        is_kw = true;
                        break;
                    }
                }
...
static void render_code_block(FILE *out, const char *code, const char *lang, int tw,
    ...
    while (*p) {
        ...
        if (g_tui_features && g_tui_features->diff_code_blocks &&
            ...
            md_render_code_line(out, line, lang);
        }
        fprintf(out, "\n");
        if (heap_line)
            free(line);
...
static void render_line(md_renderer_t *r, char *line) {
    ...
    if (r->state != MD_STATE_NORMAL) {
        ...
        if (len >= 40 && r->last_para_line_valid && strcmp(r->last_para_line, line) == 0) {
    ...
    /* ── Table detection ── */
    {
        ...
        if (parsed_row) {
            ...
            if (!is_separator && cols >= 2) {
                ...
                r->table_rows = 0;
                r->table_has_sep = false;
                for (int c = 0; c < MD_TABLE_MAXCOL; c++)
                    r->table_align[c] = MD_ALIGN_LEFT;
    ...
    if (line[0] == '[' && r->ref_count < MD_REF_MAX) {
        ...
        if (close && close[1] == ':' && close[2] != ':') {
            ...
            if (*url) {
                ...
                if (id_len >= (int)sizeof(ref->id))
                    id_len = (int)sizeof(ref->id) - 1;
                ...
                if (url_len >= (int)sizeof(ref->url))
                    url_len = (int)sizeof(ref->url) - 1;
    ...
    if (line[0] == '[' && line[1] == '^' && r->footnote_count < MD_FOOTNOTE_MAX) {
        ...
        if (close && close[1] == ':') {
            ...
            if (id_len >= (int)sizeof(fn->id))
                id_len = (int)sizeof(fn->id) - 1;
    ...
    if (line[0] == '<') {
        ...
        const char *block_tags[] = {"div",   "section",    "article", "nav",
    ...
    /* ── Unordered list (with nesting) ── */
    {
        ...
        if ((marker == '-' || marker == '*' || marker == '+') && lp[1] == ' ') {
            int depth = indent / 2;
            if (depth >= MD_LIST_MAXDEPTH)
                depth = MD_LIST_MAXDEPTH - 1;
            const char *text = lp + 2;

            if (text[0] == '[' && (text[1] == ' ' || text[1] == 'x' || text[1] == 'X') &&
                text[2] == ']') {
                bool checked = (text[1] == 'x' || text[1] == 'X');
                for (int i = 0; i < depth; i++)
                    fprintf(out, "  ");
                fprintf(out, "  %s%s%s ", checked ? TUI_GREEN : TUI_DIM,
                        checked ? "\xE2\x98\x91" : "\xE2\x98\x90", TUI_RESET);
                render_inline(out, text + 4);
            } else {
                const char *bullets[] = {"\xE2\x80\xA2", "\xE2\x97\xA6", "\xE2\x96\xB8",
                                         "\xE2\x96\xB9", "\xE2\x80\xA3"};
    ...
    if (len > 0 && line[0] != ' ' && line[0] != '\t' && r->state == MD_STATE_NORMAL &&
        !r->in_list) {
    ...
    /* ── Normal paragraph ── */
    if (suppress_dup_paragraph_line)
        return;
    render_inline(out, line);
    fprintf(out, "\n");
}
...
static void erase_partial_echo(md_renderer_t *r) {
    if (!r->out_is_tty || r->partial_echo_pos <= 0)
        return;
...
void md_feed(md_renderer_t *r, const char *text, size_t len) {
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        if (ch == '\n') {
            ...
            if (r->out_is_tty && r->state == MD_STATE_NORMAL)
                erase_partial_echo(r);
            ...
            if (r->line_len < MD_LINE_MAX - 1) {
                ...
                if (r->line_overflow_len >= r->line_overflow_cap) {
                    int new_cap = r->line_overflow_cap > 0 ? r->line_overflow_cap * 2
                                                           : MD_LINE_OVERFLOW_INITIAL;
                    if (new_cap <= 0)
                        new_cap = MD_LINE_OVERFLOW_INITIAL;
...
void md_flush(md_renderer_t *r) {
    ...
    /* Flush pending table */
    if (r->state == MD_STATE_TABLE)
        flush_table_or_fallback(r->out, r);
