...
static plan_t *plan_find(int id) {
    for (int i = 0; i < PLAN_MAX; i++)
        if (s_plans[i].active && s_plans[i].id == id)
            return &s_plans[i];
    return NULL;
}
...
static step_t *step_find(int id) {
    for (int i = 0; i < STEP_MAX; i++)
        if (s_steps[i].active && s_steps[i].id == id)
            return &s_steps[i];
    return NULL;
}
...
static atom_t *atom_find(int id) {
    for (int i = 0; i < ATOM_MAX; i++)
        if (s_atoms[i].active && s_atoms[i].id == id)
            return &s_atoms[i];
    return NULL;
}
...
static dialog_t *dialog_find(int id) {
    for (int i = 0; i < DIALOG_MAX; i++)
        if (s_dialogs[i].active && s_dialogs[i].id == id)
            return &s_dialogs[i];
    return NULL;
}
...
const char *plan_mode_name(plan_mode_t m) {
    switch (m) {
        case PLAN_MODE_TOP_DOWN:
            return "top_down";
        case PLAN_MODE_BOTTOM_UP:
            return "bottom_up";
        case PLAN_MODE_HYBRID:
            return "hybrid";
        default:
            return "unknown";
    }
...
plan_mode_t plan_mode_parse(const char *s) {
    ...
    if (strcmp(s, "top_down") == 0)
        return PLAN_MODE_TOP_DOWN;
    if (strcmp(s, "bottom_up") == 0)
        return PLAN_MODE_BOTTOM_UP;
    return PLAN_MODE_HYBRID;
}
...
step_type_t step_type_parse(const char *s) {
    ...
    if (strcmp(s, "gate") == 0)
        return STEP_GATE;
    if (strcmp(s, "dialog") == 0)
        return STEP_DIALOG;
    if (strcmp(s, "milestone") == 0)
...
bool plan_delete(int plan_id) {
    ...
    for (int i = 0; i < STEP_MAX; i++) {
        if (!s_steps[i].active || s_steps[i].plan_id != plan_id)
            continue;
    ...
    /* deactivate dialogs */
    for (int i = 0; i < DIALOG_MAX; i++) {
        if (!s_dialogs[i].active || s_dialogs[i].plan_id != plan_id)
            continue;
        free(s_dialogs[i].prompt);
        s_dialogs[i].prompt = NULL;
        free(s_dialogs[i].response);
        s_dialogs[i].response = NULL;
...
bool step_add_tag(int step_id, const char *tag) {
    step_t *s = step_find(step_id);
    if (!s || !tag || s->tag_count >= PLAN_TAG_MAX)
        return false;
...
bool step_add_note(int step_id, const char *note) {
    step_t *s = step_find(step_id);
    if (!s || !note || s->note_count >= PLAN_NOTE_MAX)
        return false;
...
bool step_add_dep(int step_id, int dep_step_id) {
    step_t *s = step_find(step_id);
    if (!s || s->dep_count >= STEP_MAX_DEPS)
        return false;
...
int step_add_atom(int step_id, const char *title, atom_type_t type) {
    step_t *s = step_find(step_id);
    if (!s || s->atom_count >= STEP_MAX_ATOMS)
        return -1;
...
bool atom_set_tool(int atom_id, const char *tool_name, const char *input_json) {
    ...
    if (!a)
        return false;
    free(a->tool_name);
    a->tool_name = safe_strdup(tool_name ? tool_name : "");
    free(a->input_json);
    a->input_json = safe_strdup(input_json ? input_json : "{}");
    return true;
}
...
bool atom_wire(int src_atom_id, int dst_atom_id, const char *key) {
    ...
    if (dst->input_from_count >= ATOM_MAX_DEPS)
        return false;
...
bool atom_run(int atom_id, char *result_buf, size_t rlen) {
    ...
    switch (a->type) {
        ...
        case ATOM_TOOL_CALL: {
            ...
            ok =
                tools_execute(a->tool_name, a->input_json ? a->input_json : "{}", result_buf, rlen);
        ...
        case ATOM_ASSERT: {
            ...
            bool passed = (cond[0] != '\0' && strcmp(cond, "false") != 0 && strcmp(cond, "0") != 0);
            snprintf(result_buf, rlen, "assert_%s: %s", passed ? "passed" : "failed", cond);
...
int plan_run_next(int plan_id) {
    int ids[1];
    if (plan_ready_atoms(plan_id, ids, 1) < 1)
        return -1;
    char buf[4096];
...
int plan_decompose(int plan_id, int focus_step_id, const char **subtitles, int count) {
    plan_t *p = plan_find(plan_id);
    if (!p || !subtitles || count <= 0)
        return 0;
    int added = 0;
    for (int i = 0; i < count; i++) {
        if (plan_add_step(plan_id, focus_step_id, subtitles[i], STEP_COMPOSITE) >= 0)
            added++;
    }
    return added;
}
...
int plan_aggregate_atoms(int plan_id, int parent_step_id, const char *step_title,
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (!a || s->atom_count >= STEP_MAX_ATOMS)
            break;
...
int plan_dialog_ask(int plan_id, int step_id, const char *prompt, const char **choices,
    ...
    if (choices && choice_count > 0) {
        ...
        for (int i = 0; i < n; i++) {
            strncpy(d->choices[i], choices[i] ? choices[i] : "", PLAN_CHOICE_LEN - 1);
...
dialog_t *dialog_get(int dialog_id) {
    return dialog_find(dialog_id);
}

int plan_pending_dialogs(int plan_id, int *dialog_ids_out, int max_out) {
    if (!dialog_ids_out || max_out <= 0)
        return 0;
    int count = 0;
    for (int i = 0; i < DIALOG_MAX && count < max_out; i++) {
        if (s_dialogs[i].active && s_dialogs[i].plan_id == plan_id && !s_dialogs[i].answered)
...
static void atom_to_json(jbuf_t *b, atom_t *a) {
    jbuf_append(b, "{");
    jbuf_append(b, "\"id\":");
    jbuf_append_int(b, a->id);
    jbuf_append(b, ",\"title\":");
    jbuf_append_json_str(b, a->title ? a->title : "");
    jbuf_append(b, ",\"type\":");
    jbuf_append_json_str(b, atom_type_name(a->type));
    jbuf_append(b, ",\"status\":");
    jbuf_append_json_str(b, plan_status_name(a->status));
    if (a->tool_name && a->tool_name[0]) {
...
static void step_to_json(jbuf_t *b, step_t *s, int depth) {
    ...
    jbuf_append(b, "{");
    jbuf_append(b, "\"id\":");
    jbuf_append_int(b, s->id);
    jbuf_append(b, ",\"title\":");
    jbuf_append_json_str(b, s->title ? s->title : "");
    jbuf_append(b, ",\"type\":");
    jbuf_append_json_str(b, step_type_name(s->type));
    jbuf_append(b, ",\"status\":");
    jbuf_append_json_str(b, plan_status_name(s->status));
    jbuf_append(b, ",\"priority\":");
    jbuf_append_int(b, s->priority);
...
int plan_to_json(int plan_id, char *buf, size_t len) {
    ...
    jbuf_append(&b, "{");
    jbuf_append(&b, "\"id\":");
    jbuf_append_int(&b, p->id);
    jbuf_append(&b, ",\"title\":");
    jbuf_append_json_str(&b, p->title ? p->title : "");
    jbuf_append(&b, ",\"goal\":");
    jbuf_append_json_str(&b, p->goal ? p->goal : "");
    jbuf_append(&b, ",\"mode\":");
    jbuf_append_json_str(&b, plan_mode_name(p->mode));
    jbuf_append(&b, ",\"status\":");
    jbuf_append_json_str(&b, plan_status_name(p->status));
    jbuf_append(&b, ",\"steps\":[");
    for (int i = 0; i < p->root_step_count; i++) {
...
int plan_summary(int plan_id, char *buf, size_t len) {
    ...
    return snprintf(buf, len,
                    ...
                    p->id, p->title ? p->title : "", plan_mode_name(p->mode),
                    plan_status_name(p->status), total, done, in_prog, blocked, failed);
}

/* ── plan_tree_render ────────────────────────────────────────────────────── */

static void render_step(step_t *s, jbuf_t *b, const char *prefix, bool is_last, int depth) {
    if (depth > 10)
        return;
    ...
    snprintf(line, sizeof(line), "%s [%s] %s", status_glyph(s->status), step_type_name(s->type),
             s->title ? s->title : "");
    jbuf_append(b, line);
    ...
    for (int i = 0; i < s->atom_count; i++) {
        ...
        jbuf_append(b, last_leaf ? "\xe2\x94\x94\xe2\x94\x80 " : "\xe2\x94\x9c\xe2\x94\x80 ");
...
int plan_tree_render(int plan_id, char *buf, size_t len) {
    ...
    snprintf(header, sizeof(header),
             ...
             "goal: %s\n"
             "mode: %s  status: %s\n",
             p->id, p->title ? p->title : "", (p->goal && p->goal[0]) ? p->goal : "(no goal set)",
             plan_mode_name(p->mode), plan_status_name(p->status));
