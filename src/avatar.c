...
static inline V3 v3scl(V3 a, double s) {
    return v3(a.x * s, a.y * s, a.z * s);
}
static inline double v3dot(V3 a, V3 b) {
    return a.x * b.x + a.y * b.y + a.z * b.z;
}
static inline V3 v3cross(V3 a, V3 b) {
    return v3(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x);
}
static inline double v3len(V3 a) {
    return sqrt(v3dot(a, a));
}
static inline V3 v3norm(V3 a) {
    double l = v3len(a);
    return l > 1e-12 ? v3scl(a, 1.0 / l) : a;
}
...
static uint32_t mul32(uint32_t *st) {
    uint32_t z = (*st += 0x6D2B79F5u);
    z = (z ^ (z >> 15)) * (z | 1u);
    z ^= z + (z ^ (z >> 7)) * (z | 61u);
    return z ^ (z >> 14);
}
static double mulf(uint32_t *st) {
    return mul32(st) / 4294967296.0;
}
static double mrange(uint32_t *st, double lo, double hi) {
    return lo + (hi - lo) * mulf(st);
}

static aface_t aface_from_name(const char *name) {
    uint32_t st = fnv1a(name && name[0] ? name : "anon");
    aface_t f;
    /* Proportions are centered on MediaPipe's canonical 468-point face mesh:
     * width/height ~= 0.87, depth/height ~= 0.57, with identity jitter. */
    f.hw = mrange(&st, 0.61, 0.68);
    f.hh = mrange(&st, 0.72, 0.79);
    f.hd = mrange(&st, 0.46, 0.54);
    f.eye_sep = mrange(&st, 0.23, 0.29);
    f.eye_y = mrange(&st, 0.06, 0.13);
    f.eye_size = mrange(&st, 0.135, 0.165);
    f.nose_len = mrange(&st, 0.11, 0.17);
    f.nose_w = mrange(&st, 0.12, 0.15);
    f.mouth_w = mrange(&st, 0.18, 0.25);
    f.mouth_y = mrange(&st, -0.34, -0.27);
    f.brow_y = f.eye_y + mrange(&st, 0.15, 0.20);
    f.ear_size = mrange(&st, 0.14, 0.18);
    f.iris_dx = 0.0;
    f.iris_dy = 0.0;
    ...
    f.skin = (int)(mulf(&st) * 5.0);
    if (f.skin > 4)
        f.skin = 4;
    f.iris = (int)(mulf(&st) * 4.0);
    if (f.iris > 3)
        f.iris = 3;
    f.hair = (int)(mulf(&st) * 5.0);
    if (f.hair > 4)
        f.hair = 4;
    f.haircol = (int)(mulf(&st) * 5.0);
    if (f.haircol > 4)
        f.haircol = 4;
    f.shirt = (int)(mulf(&st) * 4.0);
    if (f.shirt > 3)
        f.shirt = 3;
    f.pants = (int)(mulf(&st) * 2.0);
    if (f.pants > 1)
        f.pants = 1;
...
static double sdSph(V3 p, double s) {
    return v3len(p) - s;
}
...
static void opU(double *d, int *mat, double nd, int nm) {
    if (nd < *d) {
        *d = nd;
        *mat = nm;
    }
...
static double sdHead(V3 p, const aface_t *f, int *mat) {
    ...
    for (int s = -1; s <= 1; s += 2) {
        double ck =
            sdEll(v3sub(p, v3(s * f->hw * 0.74, -0.08, f->hd * 0.72)), v3(0.16, 0.13, 0.11));
    ...
    for (int s = -1; s <= 1; s += 2) {
        double tmp =
            sdEll(v3sub(p, v3(s * f->hw * 0.94, 0.20, f->hd * 0.55)), v3(0.10, 0.22, 0.18));
    ...
    for (int s = -1; s <= 1; s += 2) {
        double fem =
            sdEll(v3sub(p, v3(s * f->hw * 0.34, f->hh * 0.42, f->hd * 0.74)), v3(0.20, 0.18, 0.10));
        skin = smin(skin, fem, 0.12);
    }
    /* glabella — supraorbital bridge between the brows */
    double glab = sdEll(v3sub(p, v3(0, f->brow_y + brow_lift * 0.35 + 0.010, f->hd * 0.860)),
                        v3(f->nose_w * 0.55, 0.055, 0.045));
    ...
    for (int s = -1; s <= 1; s += 2) {
        ...
        double helix =
            fabs(sdEll(v3sub(p, eo), v3(0.058, f->ear_size, f->ear_size * 0.88))) - 0.014;
        double antih = fabs(sdEll(v3sub(p, v3(eo.x - s * 0.012, eo.y + 0.04, eo.z + 0.022)),
                                  v3(0.028, f->ear_size * 0.60, f->ear_size * 0.50))) -
                       0.008;
    ...
    double br_ridge =
        sdCap(p, v3(0, 0.22, f->hd * 0.890), v3(0, -0.040, f->hd * 0.960), f->nose_w * 0.32);
    for (int s = -1; s <= 1; s += 2) {
        double lat = sdEll(v3sub(p, v3(s * f->nose_w * 0.55, -0.04, f->hd * 0.930)),
                           v3(f->nose_w * 0.38, 0.14, f->nose_len * 0.55));
        skin = smin(skin, lat, 0.030);
    }
    double ntip = sdEll(v3sub(p, v3(0, -0.145, f->hd * 0.970)),
                        v3(f->nose_w * 0.80, f->nose_len * 0.42, f->nose_len * 0.60));
    double colba =
        sdEll(v3sub(p, v3(0, -0.180, f->hd * 0.945)), v3(f->nose_w * 0.35, 0.030, 0.030));
    double col =
        sdCap(p, v3(0, -0.155, f->hd * 0.920), v3(0, -0.250, f->hd * 0.870), f->nose_w * 0.18);
    ...
    for (int s = -1; s <= 1; s += 2) {
        ...
        double aw = sdEll(v3sub(p, v3(s * f->nose_w * 0.90, -0.175, f->hd * 0.945)),
                          v3(0.052, 0.042, 0.048));
        skin = smin(skin, aw, 0.024);
    }

    /* nostrils */
    for (int s = -1; s <= 1; s += 2) {
        double nos = sdEll(v3sub(p, v3(s * f->nose_w * 0.58, -0.222, f->hd * 0.930)),
                           v3(0.024, 0.018, 0.022));
        skin = fmax(skin, -nos);
    }
    ...
    for (int s = -1; s <= 1; s += 2) {
        ...
        double bbone =
            sdEll(v3sub(p, v3(s * f->eye_sep, f->eye_y + f->eye_size * 0.88, f->hd * 0.820)),
    ...
    *mat = M_SKIN;
    ...
    for (int s = -1; s <= 1; s += 2) {
        ...
        double sclera =
            sdEll(v3sub(p, ec), v3(f->eye_size * 0.98, f->eye_size * 0.92, f->eye_size * 0.95));
        opU(&d, mat, sclera, M_EYE);
        double fz = ez + f->eye_size * 0.82;
        V3 ic = v3(s * f->eye_sep + s * f->iris_dx, f->eye_y + f->iris_dy, fz);
        opU(&d, mat,
            sdEll(v3sub(p, ic), v3(f->eye_size * 0.52, f->eye_size * 0.52, f->eye_size * 0.12)),
            M_IRIS);
        opU(&d, mat,
            sdEll(v3sub(p, v3(ic.x, ic.y, ic.z + 0.008)),
                  v3(f->eye_size * 0.24, f->eye_size * 0.24, f->eye_size * 0.08)),
            M_PUPIL);
        ...
        /* lid crease */
        if (f->blink > 0.4) {
            V3 crA = v3(s * (f->eye_sep - f->eye_size * 0.92), f->eye_y + f->eye_size * 0.70,
                        ez + f->eye_size * 0.38);
            V3 crB = v3(s * (f->eye_sep + f->eye_size * 0.88), f->eye_y + f->eye_size * 0.60,
                        ez + f->eye_size * 0.35);
            double crvol = sdEll(v3sub(p, v3(s * f->eye_sep, f->eye_y + f->eye_size * 0.65,
                                             ez + f->eye_size * 0.35)),
        ...
        opU(&d, mat,
            sdEll(v3sub(p, v3(s * f->eye_sep, lowY, ez + f->eye_size * 0.22)),
                  v3(f->eye_size * 1.10, f->eye_size * 0.28, f->eye_size * 0.82)),
            M_SKIN);

        /* medial canthus */
        opU(&d, mat,
            sdSph(v3sub(p, v3(s * (f->eye_sep - f->eye_size * 0.92), f->eye_y - 0.008,
                              ez + f->eye_size * 0.35)),
                  0.018),
            M_EYE);
        ...
        V3 bm0 =
            v3(s * (f->eye_sep - f->eye_size * 1.0), f->brow_y + brow_lift + 0.004, f->hd * 0.825);
        V3 bm1 =
            v3(s * (f->eye_sep + f->eye_size * 0.15), f->brow_y + brow_lift + 0.012, f->hd * 0.822);
        V3 bl1 = v3(s * (f->eye_sep + f->eye_size * 0.95),
                    f->brow_y + brow_lift + 0.025 + smile * 0.010, f->hd * 0.815);
    ...
    double ulip_c = sdEll(v3sub(p, v3(0, f->mouth_y + 0.062 + smile * 0.014, f->hd * 0.887)),
                          v3(mw * 0.28, 0.036, 0.052));
    double ulip_l =
        sdEll(v3sub(p, v3(-mw * 0.48, f->mouth_y + 0.048 + smile * 0.026, f->hd * 0.882)),
              v3(mw * 0.30, 0.033, 0.048));
    double ulip_r =
        sdEll(v3sub(p, v3(mw * 0.48, f->mouth_y + 0.048 + smile * 0.026, f->hd * 0.882)),
              v3(mw * 0.30, 0.033, 0.048));
    ...
    for (int s = -1; s <= 1; s += 2) {
        double peak = sdSph(
            v3sub(p, v3(s * mw * 0.22, f->mouth_y + 0.088 + smile * 0.010, f->hd * 0.888)), 0.024);
        upperlip = smin(upperlip, peak, 0.018);
    }
    double lowerlip = sdEll(v3sub(p, v3(0, f->mouth_y - 0.052 - open * 0.050, f->hd * 0.876)),
                            v3(mw * 0.78, 0.052 + open * 0.014, 0.058));
    double lips = smin(upperlip, lowerlip, 0.018);
    for (int s = -1; s <= 1; s += 2) {
        double comm = sdSph(
            v3sub(p, v3(s * mw * 0.90, f->mouth_y + 0.006 + smile * 0.038, f->hd * 0.878)), 0.020);
        lips = smin(lips, comm, 0.014);
    }
    lips = fmax(lips, -sdCap(p, v3(-mw * 0.88, f->mouth_y + 0.008 - open * 0.012, f->hd * 0.895),
                             v3(mw * 0.88, f->mouth_y + 0.008 - open * 0.012, f->hd * 0.895),
                             0.007 + open * 0.020));
    ...
    for (int s = -1; s <= 1; s += 2) {
        double fold =
            sdCap(p, v3(s * f->nose_w * 0.96, -0.085, f->hd * 0.895),
    ...
    skin = fmax(skin, -sdCap(p, v3(0, -0.168, f->hd * 0.893),
                             v3(0, f->mouth_y + 0.082, f->hd * 0.884), f->philtrum_d) +
                          0.003);

    /* mentolabial sulcus — groove between the lower lip and the chin */
    skin =
        fmax(skin, -sdCap(p, v3(-mw * 0.62, f->mouth_y - 0.105 - open * 0.020, f->hd * 0.860),
                          v3(mw * 0.62, f->mouth_y - 0.105 - open * 0.020, f->hd * 0.860), 0.020) -
                       0.002);

    /* chin cleft */
    if (f->chin_d > 0.04) {
        skin = fmax(skin, -sdCap(p, v3(0, -0.530, f->hd * 0.790), v3(0, -0.455, f->hd * 0.820),
                                 f->chin_d * 0.055));
    }
    ...
    if (f->hair > 0) {
        ...
        double shell = fabs(sdEll(v3sub(p, v3(0, lift, -0.02)),
                                  v3(f->hw * grow, f->hh * grow, f->hd * grow))) -
                       thick;
        double fopen = sdEll(v3sub(p, v3(0, -0.02, f->hd * 0.52)),
                             v3(f->hw * 0.90, f->hh * 0.72, f->hd * 0.92));
        double hair_d = fmax(shell, -fopen);
        if (f->hair == 1)
            hair_d = fmax(hair_d, -(p.y + 0.02 - f->hh * 0.19));
        if (f->hair == 4)
            hair_d = fmax(hair_d, -(p.y + 0.12 - f->hh * 0.22));
        for (int s = -1; s <= 1; s += 2) {
            double rec = sdEll(v3sub(p, v3(s * f->hw * 0.72, f->hh * 0.52, f->hd * 0.70)),
                               v3(0.18, 0.14, 0.12));
    ...
    if (f->glasses) {
        ...
        for (int s = -1; s <= 1; s += 2) {
            double frame = fabs(sdRBox(v3sub(p, v3(s * f->eye_sep, f->eye_y, fzg)),
                                       v3(f->eye_size * 1.08, f->eye_size * 0.88, 0.018), 0.035)) -
                           0.011;
            opU(&d, mat, frame, M_GLASS);
            opU(&d, mat,
                sdEll(v3sub(p, v3(s * f->nose_w * 1.0, f->eye_y - f->eye_size * 0.55, fzg - 0.012)),
                      v3(0.014, 0.022, 0.012)),
                M_GLASS);
        ...
        opU(&d, mat,
            sdCap(p, v3(-f->eye_sep * 0.60, f->eye_y + 0.010, fzg),
                  v3(f->eye_sep * 0.60, f->eye_y + 0.010, fzg), 0.009),
            M_GLASS);
    }
    return d;
}
...
static double sdBody(V3 p, const aface_t *f, int *mat) {
    ...
    *mat = m;
    ...
    for (int s = -1; s <= 1; s += 2) {
        ...
        opU(&d, mat, sdRBox(v3sub(p, v3(s * 0.38, sh - 4.12, 0.12)), v3(0.13, 0.06, 0.22), 0.04),
            M_SKIN); /* foot */
    }
    return d;
}
...
static int shade_mat(int mat, double lum, const aface_t *f) {
    switch (mat) {
        case M_SKIN:
            return ramp_pick(SKIN[f->skin], 6, lum);
        case M_EYE:
            return ramp_pick(EYEW, 6, 0.4 + 0.6 * lum);
        case M_IRIS:
            return ramp_pick(IRIS[f->iris], 5, lum);
        case M_PUPIL:
            return ramp_pick(PUP, 4, lum);
        case M_HAIR:
            return ramp_pick(HAIRC[f->haircol], 5, lum);
        case M_BROW:
            return ramp_pick(HAIRC[f->haircol], 5, lum * 0.6);
        case M_LIP:
            return ramp_pick(LIPS, 6, lum);
        case M_GLASS:
            return ramp_pick(GLASSC, 4, lum);
        case M_SHIRT:
            return ramp_pick(SHIRTC[f->shirt], 5, lum);
        case M_PANTS:
            return ramp_pick(PANTSC[f->pants], 4, lum);
        default:
            return -1;
    }
}
...
static int mat_vote_score(int mat, int count, bool cell_vote) {
    int score = count * 16 + mat_priority(mat);
    switch (mat) {
        case M_PUPIL:
        case M_IRIS:
        case M_EYE:
...
static bool project_px(V3 p, V3 ro, V3 fwd, V3 rgt, V3 up, double tanH, double aspect, int gw,
                       int gh, int *x, int *y) {
    V3 q = v3sub(p, ro);
...
static double calc_ao(V3 p, V3 n, const aface_t *f) {
    ...
    for (int i = 1; i <= 3; i++) {
        ...
        int m;
        double d = sdScene(v3add(p, v3scl(n, h)), f, &m);
        occ += (h - d) * sca;
        sca *= 0.75;
    }
...
static double soft_shadow(V3 ro, V3 rd, const aface_t *f, double k) {
    ...
    for (int i = 0; i < 11; i++) {
        int m;
        double h = sdScene(v3add(ro, v3scl(rd, t)), f, &m);
        if (h < 0.002)
            return 0.0;
        res = fmin(res, k * h / t);
...
#undef SP
    ...
    if (mat == M_SKIN) {
        ...
        lum += (hash13(v3scl(p, 90.0)) - 0.5) * 0.018;
        ...
        if (f->stubble > 0.001) {
            ...
            double mous =
                (p.y > f->mouth_y + 0.02 && p.y < f->mouth_y + 0.14 && fabs(p.x) < f->mouth_w)
                    ? 0.75
                    : 0.0;
...
static void aface_frame(tui_braille_t *bc, int *cell_color, const aface_t *f) {
    ...
    V3 tgt = v3(0, ty, 0);
    V3 ro = v3(dist * cos(f->pitch) * sin(f->yaw), ty + dist * sin(f->pitch),
               dist * cos(f->pitch) * cos(f->yaw));
        ...
        for (int px = 0; px < gw; px++) {
            ...
            if (hitcnt) {
                ...
                for (int mm = 1; mm < M_NMAT; mm++) {
                    if (!matcnt[mm])
                        continue;
                    int score = mat_vote_score(mm, matcnt[mm], false);
                    if (score > bestscore) {
                        bestscore = score;
                        hitmat = mm;
                    }
            ...
            double dotlum = 0.0;
            if (hitcnt) {
                double l = clampd(lum, 0, 1);
                double base = 0.05 + 0.80 * pow(l, 1.45);
                if (hitmat == M_SKIN)
                    base = 0.035 + 0.66 * pow(l, 1.65);
                else if (hitmat == M_HAIR)
                    base = 0.10 + 0.68 * pow(l, 1.35);
                else if (hitmat == M_EYE)
                    base = fmax(base, 0.62);
                else if (hitmat == M_IRIS || hitmat == M_LIP)
                    base *= 0.44;
                else if (hitmat == M_PUPIL || hitmat == M_BROW || hitmat == M_GLASS)
                    base *= 0.22;
                dotlum = cover * base;
            }
            if (dotlum > thr)
    ...
    if (f->view <= 1) {
        ...
        if (brow_half < 2)
            brow_half = 2;
        int mouth_half = gw / 28;
        if (mouth_half < 3)
            mouth_half = 3;
        mouth_half = (int)(mouth_half * (1.0 + smile * 0.18));
        int x, y;
        for (int s = -1; s <= 1; s += 2) {
            if (project_px(v3(s * f->eye_sep, f->eye_y - 0.002, f->hd * 1.020), ro, fwd, rgt, up,
                           tanH, aspect, gw, gh, &x, &y)) {
                braille_clear_line(bc, x - eye_half, y, x + eye_half, y, stroke);
                braille_clear_dot(bc, x, y - 1);
            }
            if (project_px(v3(s * f->eye_sep, f->brow_y + brow_lift + 0.016, f->hd * 1.000), ro,
                           fwd, rgt, up, tanH, aspect, gw, gh, &x, &y)) {
                int tilt = s > 0 ? -1 : 1;
                braille_clear_line(bc, x - brow_half, y + tilt, x + brow_half, y - tilt, stroke);
            }
            if (project_px(v3(s * f->nose_w * 0.50, -0.215, f->hd * 1.030), ro, fwd, rgt, up, tanH,
                           aspect, gw, gh, &x, &y)) {
                braille_clear_dot(bc, x, y);
            }
        }
        if (project_px(v3(0, -0.135, f->hd * 1.055), ro, fwd, rgt, up, tanH, aspect, gw, gh, &x,
                       &y)) {
            braille_clear_dot(bc, x, y);
        }
        if (project_px(v3(0, f->mouth_y + 0.006 - open * 0.012 + smile * 0.022, f->hd * 1.030), ro,
                       fwd, rgt, up, tanH, aspect, gw, gh, &x, &y)) {
            int lift = smile > 0.2 ? 1 : 0;
            int mouth_r = (open > 0.60 && gw >= 180) ? 1 : 0;
            braille_clear_line(bc, x - mouth_half, y + lift, x + mouth_half, y - lift, mouth_r);
            if (open > 0.08) {
                int gap = 1 + (int)(open * 4.0);
                int open_half = (int)(mouth_half * (0.72 + open * 0.18));
                braille_clear_line(bc, x - open_half, y + gap, x + open_half, y + gap, mouth_r);
            }
        }
    }
    for (int c = 0; c < W * H; c++) {
        ...
        for (int mm = 1; mm < M_NMAT; mm++) {
            ...
            if (!n)
                continue;
            int score = mat_vote_score(mm, n, true);
            if (score > bestscore) {
                bestscore = score;
                best = mm;
            }
        ...
        double meanlum =
            acc[c * M_NMAT + best] / (cnt[c * M_NMAT + best] ? cnt[c * M_NMAT + best] : 1);
...
static int view_of(const char *s) {
    if (!s)
        return 0;
    if (!strcmp(s, "full") || !strcmp(s, "body") || !strcmp(s, "fullbody"))
        return 2;
    if (!strcmp(s, "bust") || !strcmp(s, "half") || !strcmp(s, "portrait"))
        return 1;
    return 0;
}
static aface_t aface_parse(const char *json, char **owned_name, char **owned_title) {
    ...
    *owned_title = title;
    ...
    if (json) {
        ...
        if (json_get_int(json, "view", -99) != -99)
            f.view = json_get_int(json, "view", f.view);
...
bool avatar_is_kind(const char *kind) {
    return kind && (!strcmp(kind, "face") || !strcmp(kind, "avatar") || !strcmp(kind, "body") ||
                    !strcmp(kind, "portrait"));
}
...
int avatar_plot(char *out, size_t cap, const char *json) {
    ...
    char *name, *title;
    aface_t f = aface_parse(json, &name, &title);
    ...
    int W, H;
    anim_resolve_size(rw ? rw : dw, rh ? rh : dh, &W, &H);
    bool color = json ? json_get_bool(json, "color", true) : true;

    tui_braille_t bc;
    tui_braille_init(&bc, W * 2, H * 4);
    ...
    if (mf) {
        ...
        if (color)
            fprintf(mf, "\033[2m  %s view  zoom %.1fx  skin %d  hair %s  iris %d%s\033[0m\n",
...
int avatar_anim(const char *json) {
    ...
    int rh = json ? json_get_int(json, "height", 0) : 0;
    int W, H;
    anim_resolve_size(rw, rh, &W, &H);
    bool color = json ? json_get_bool(json, "color", true) : true;
    long gens = json ? json_get_int(json, "gens", 0) : 0;
    int fps = json ? json_get_int(json, "fps", 24) : 24;
    if (f.aa < 1)
        f.aa = 1; /* animation: keep AA modest for framerate unless asked */
    ...
    for (; budget < 0 || frames < budget; frames++) {
        ...
        if (stdin_params) {
            ...
            if (stdinbuf[0]) {
                ...
                f.hh = nf.hh;
                f.hd = nf.hd;
                f.eye_sep = nf.eye_sep;
                f.eye_y = nf.eye_y;
                f.eye_size = nf.eye_size;
                f.nose_len = nf.nose_len;
                f.nose_w = nf.nose_w;
                f.mouth_w = nf.mouth_w;
                f.mouth_y = nf.mouth_y;
                f.brow_y = nf.brow_y;
                f.ear_size = nf.ear_size;
                f.blink = nf.blink;
                f.yaw = nf.yaw;
                f.pitch = nf.pitch;
                f.lower_lid = nf.lower_lid;
                f.mouth_open = nf.mouth_open;
                f.smile = nf.smile;
                f.brow_raise = nf.brow_raise;
                free(_n);
                free(_t);
            }
        } else {
        ...
        if (f.view >= 1)
            f.pitch += 0.006 * sin(frames * 0.022);
        (void)0;
        ...
        if (stdin_params)
            printf(
                "\033[2mlive face  yaw=%+.2f  blink=%.1f  frame %-5ld  ^C to stop\033[0m\033[K\n",
                f.yaw, f.blink, frames);
        else
            printf(
                "\033[2myaw=%+.2f  blink=%.1f  zoom %.1fx  frame %-5ld  ^C to stop\033[0m\033[K\n",
