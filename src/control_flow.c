...
static bool json_extract_field(const char *ctx_json, const char *key, char *buf, size_t bufsz) {
    if (!ctx_json || !key || !*key || !buf || bufsz == 0)
        return false;
...
bool condition_parse(const char *expr_str, condition_t *cond_out) {
    ...
    if (lt && lt == le)
        lt = (le ? strchr(le + 2, '<') : NULL); /* skip <= */
    const char *gt = strchr(p, '>');
    if (gt && gt == ge)
        gt = (ge ? strchr(ge + 2, '>') : NULL); /* skip >= */
...
#undef TRY_OP
    ...
    if (lhs_len >= COND_OPERAND_LEN)
        lhs_len = COND_OPERAND_LEN - 1;
    ...
    switch (op) {
        ...
        case COP_GT:
            ...
            break;
        case COP_CONTAINS:
            rhs_start += strlen(" contains ");
            break;
        case COP_EXISTS:
            rhs_start += strlen(" exists");
            break;
        default:
            break;
    }
...
bool condition_evaluate(const condition_t *cond, const char *ctx_json) {
    ...
    if (cond->op == COP_TRUTHY) {
        bool truthy = (lhs_str[0] != '\0' && strcmp(lhs_str, "0") != 0 &&
                       strcmp(lhs_str, "false") != 0 && strcmp(lhs_str, "null") != 0);
    ...
    switch (cond->op) {
        case COP_EQ:
            result =
                both_num ? (fabs(lhs_num - rhs_num) < 1e-9) : (strcmp(lhs_str, cond->rhs) == 0);
            break;
        case COP_NEQ:
            result =
                both_num ? (fabs(lhs_num - rhs_num) >= 1e-9) : (strcmp(lhs_str, cond->rhs) != 0);
...
bool step_set_control_flow(int step_id, const control_flow_t *cf) {
    ...
    if (s_registry_count >= CF_REGISTRY_CAP)
        return false;
...
static plan_status_t run_step_list(const int *ids, int count, const char *state_json) {
    for (int i = 0; i < count; i++) {
        ...
        plan_status_t st = (nested && nested->type != CF_NONE)
