...
static void read_seed_file(void) {
    ...
    if (env && env[0]) {
        ...
        while (tok) {
            ...
            if (colon) {
                ...
                if (hlen < sizeof(host)) { memcpy(host, tok, hlen); host[hlen] = '\0'; }
                port = atoi(colon + 1);
            } else {
                snprintf(host, sizeof(host), "%s", tok);
            }
            if (host[0] && port > 0) try_connect(host, (uint16_t)port);
            tok = strtok(NULL, ",; \t\n");
        }
        return;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (colon) {
            ...
            if (hlen < sizeof(host)) { memcpy(host, line, hlen); host[hlen] = '\0'; }
            port = atoi(colon + 1);
        } else {
...
static void DNSSD_API mdns_cb(DNSServiceRef sdRef, DNSServiceFlags flags,
    ...
    (void)sdRef; (void)flags; (void)interfaceIndex; (void)ctx;
    (void)txtLen; (void)txtRecord; (void)fullname;
...
static void *mdns_loop(void *arg) {
    ...
    DNSServiceErrorType e = DNSServiceRegister(
        &s_sdref, 0, 0, NULL, "_dsco._tcp", NULL, NULL,
        htons(s_port), 0, NULL, NULL, NULL);
    if (e != kDNSServiceErr_NoError) return NULL;
    ...
    DNSServiceRefDeallocate(s_sdref); s_sdref = NULL;
    if (browse_ref) DNSServiceRefDeallocate(browse_ref);
    return NULL;
}
#endif /* __APPLE__ */
...
void peer_bootstrap_reseed(void) {
    if (s_node) read_seed_file();
}

#else /* !HAVE_LIBSODIUM — stubs */

void peer_bootstrap_init(void *n, uint16_t p) { (void)n; (void)p; }
void peer_bootstrap_stop(void) {}
void peer_bootstrap_reseed(void) {}
