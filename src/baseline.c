...
static void generate_instance_id(char *out, size_t out_len) {
    ...
    snprintf(out, out_len, "%s-%ld-%d-%06d", host, (long)tv.tv_sec, (int)getpid(), (int)tv.tv_usec);
}
...
bool baseline_start(const char *model, const char *mode) {
    ...
    if (parent && parent[0]) {
        snprintf(g_baseline.parent_instance_id, sizeof(g_baseline.parent_instance_id), "%s",
                 parent);
    ...
    const char *ins = "INSERT INTO instances(instance_id, parent_instance_id, pid, model, mode)"
                      " VALUES(?1, ?2, ?3, ?4, ?5);";
    ...
    if (model && model[0])
        sqlite3_bind_text(st, 4, model, -1, SQLITE_TRANSIENT);
    else
...
void baseline_stop(void) {
    ...
    sqlite3_stmt *st = NULL;
    const char *upd = "UPDATE instances SET ended_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE "
                      "instance_id = ?1;";
...
bool baseline_log(const char *category, const char *title, const char *detail,
    ...
    sqlite3_stmt *st = NULL;
    const char *ins = "INSERT INTO events(instance_id, category, title, detail, metadata_json)"
                      " VALUES(?1, ?2, ?3, ?4, ?5);";
    ...
    if (detail)
        ...
        sqlite3_bind_null(st, 4);
    if (metadata_json)
        sqlite3_bind_text(st, 5, metadata_json, -1, SQLITE_TRANSIENT);
    else
        sqlite3_bind_null(st, 5);
...
static double estimate_cost(const char *model, int in_tok, int out_tok, int cache_read,
                            int cache_write) {
...
char *baseline_credit_report(const char *root_instance_id) {
    ...
    const char *sql =
        ...
        "  UNION ALL"
        "  SELECT i.instance_id, i.parent_instance_id, i.pid, i.model, i.started_at, i.ended_at, "
        "t.depth + 1"
    ...
    while (sqlite3_step(st) == SQLITE_ROW) {
        ...
        snprintf(row, sizeof(row),
                 ...
                 pid, model ? model : "unknown", depth, dur ? dur : "?", in_t, out_t, cr_t, cw_t,
                 cost, tools, turns);
        jbuf_append(&buf, row);
        count++;
    }
...
double baseline_daily_cost(void) {
    ...
    const char *sql = "SELECT COALESCE(SUM(est_cost_usd), 0.0) FROM events "
...
static void extract_query_param(const char *uri, const char *key, char *out, size_t out_len) {
    out[0] = '\0';
    if (!uri || !key)
        return;
...
static void resolve_instance_filter(const char *uri, const char *default_filter, char *out,
                                    size_t out_len) {
    char from_query[256];
...
static int resolve_int_param(const char *uri, const char *key, int def, int min, int max) {
    char buf[64];
    extract_query_param(uri, key, buf, sizeof(buf));
    if (!buf[0])
        return def;
    int v = atoi(buf);
    if (v < min)
...
static bool build_events_json(jbuf_t *b, const char *instance_filter) {
    sqlite3_stmt *st = NULL;
    const char *sql = "SELECT id, ts, instance_id, category, title, COALESCE(detail,''), "
    ...
    if (instance_filter && instance_filter[0])
        jbuf_append_json_str(b, instance_filter);
    else
        jbuf_append(b, "null");
...
static bool build_timeline_html(jbuf_t *b, const char *instance_filter) {
    jbuf_append(
        b, "<!doctype html><html><head><meta charset='utf-8'>"
           ...
           "<style>"
           "body{font-family:ui-sans-serif,system-ui,-apple-system,Segoe "
           "UI,Helvetica,Arial;margin:0;padding:24px;background:#f6f8fb;color:#142033;}"
           "h1{margin:0 0 8px 0;font-size:24px;}"
           "p{margin:6px 0 12px 0;color:#51607a;}"
           ".card{background:#fff;border:1px solid #d9e1ee;border-radius:12px;padding:14px "
           "16px;margin:12px 0;box-shadow:0 2px 8px rgba(12,24,48,.05);}"
           ".row{display:flex;gap:12px;align-items:baseline;flex-wrap:wrap;}"
           ".chip{display:inline-block;padding:2px "
           "8px;border-radius:999px;background:#e8eef9;color:#1b3f76;font-size:12px;}"
           ".muted{color:#6b7890;font-size:13px;}"
           "pre{white-space:pre-wrap;word-break:break-word;background:#f7f9fc;border:1px solid "
           "#e1e7f0;border-radius:8px;padding:8px;margin:8px 0 0 0;font-size:12px;}"
           ...
           "button{padding:7px 12px;border-radius:8px;border:1px solid "
           "#1f4a93;background:#1f4a93;color:#fff;cursor:pointer;}"
           "</style></head><body>");
    ...
    jbuf_append(b, "<thead><tr><th>Instance</th><th>Parent</th><th>PID</th><th>Mode</th><th>Model</"
                   "th><th>Started</th><th>Ended</th></tr></thead><tbody>");

    sqlite3_stmt *inst = NULL;
    const char *inst_sql =
        "SELECT instance_id, COALESCE(parent_instance_id,''), pid, COALESCE(mode,''), "
        "COALESCE(model,''), started_at, COALESCE(ended_at,'') "
    ...
    const char *ev_sql = "SELECT ts, instance_id, category, title, COALESCE(detail,'') "
                         "FROM events "
                         "WHERE (?1 IS NULL OR instance_id = ?1) "
                         "ORDER BY ts_epoch DESC "
                         "LIMIT 500;";
...
int baseline_serve_http(int port, const char *default_instance_filter) {
    ...
    fprintf(stderr,
            ...
            msg, baseline_instance_id(), baseline_db_path());
    ...
    while (1) {
        ...
        resolve_instance_filter(uri, default_instance_filter, instance_filter,
                                sizeof(instance_filter));
        ...
        if (strcmp(path, "/") == 0 || strcmp(path, "/index.html") == 0) {
            ...
            char session_filter[256];
            char type_filter[128];
...
bool trace_span_begin(const char *trace_id, const char *name, const char *parent_span,
    ...
    sqlite3_stmt *st = NULL;
    const char *sql = "INSERT INTO trace_spans(span_id, trace_id, parent_span, name, start_epoch)"
                      " VALUES(?1, ?2, ?3, ?4, ?5);";
...
bool trace_span_end(const char *span_id, const char *status, const char *metadata_json) {
    ...
    sqlite3_stmt *st = NULL;
    const char *sql = "UPDATE trace_spans SET end_epoch = ?1, status = ?2, metadata_json = ?3"
                      " WHERE span_id = ?4;";
...
void trace_query_recent(int limit) {
    ...
    sqlite3_stmt *st = NULL;
    const char *sql = "SELECT trace_id, name, start_epoch, "
...
void trace_print_waterfall(const char *trace_id) {
    if (!g_baseline.ready || !g_baseline.db || !trace_id)
        return;

    sqlite3_stmt *st = NULL;
    const char *sql = "SELECT span_id, parent_span, name, start_epoch, "
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (width < 1)
            width = 1;
        if (offset + width > bar_width)
            width = bar_width - offset;
        if (offset < 0)
            offset = 0;
        ...
        const char *color = strcmp(rows[i].status, "error") == 0     ? "\033[31m"
                            : strcmp(rows[i].status, "timeout") == 0 ? "\033[33m"
                                                                     : "\033[32m";
        fprintf(stderr, "  %s%-20s  %7.0fms  [", rows[i].depth ? "  " : "", rows[i].name, dur_ms);
