...
static void compute_hmac(const uint8_t key[32], const uint8_t prev[HMAC_LEN], uint64_t seq,
                         int64_t ts, const char *tag, uint16_t tag_len, const char *msg,
                         uint32_t msg_len, uint8_t out[HMAC_LEN]) {
#ifdef HAVE_LIBSODIUM
...
audit_log_t *audit_log_open(const char *path, const uint8_t key[32]) {
    ...
    if (key) {
        ...
        if (!buf[0])
            sealed_store_get("OPENAI_API_KEY", buf, sizeof(buf));
        crypto_generichash(al->key, 32, (const uint8_t *)buf, strlen(buf), NULL, 0);
    ...
    while (1) {
        ...
        if (lseek(al->fd, payload, SEEK_CUR) < 0)
            break;
        last_seq = hdr.seq;
...
int64_t audit_log_write(audit_log_t *al, const char *tag, const char *msg) {
    ...
    compute_hmac(al->key, al->prev_hmac, hdr.seq, hdr.ts, tag, tl, msg, ml, hdr.hmac);
...
bool audit_log_verify(audit_log_t *al, int64_t *bad_seq) {
    ...
    while (1) {
        ...
        compute_hmac(al->key, prev, hdr.seq, hdr.ts, tag, hdr.tag_len, msg, hdr.msg_len, expected);

        if (memcmp(expected, hdr.hmac, HMAC_LEN) != 0) {
            if (bad_seq)
                *bad_seq = (int64_t)hdr.seq;
            ok = false;
            free(payload);
            break;
        }
