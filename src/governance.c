...
static const char *TIER_NAMES[] = {"Tier 0 (Founder)", "Tier 1 (Operator)", "Tier 2 (Agent)",
                                   "Tier 3 (User)"};
...
static const struct {
    ...
    {HARDCODED_MUST_ALWAYS,
     "Require replay evaluation, complete provenance, verified signature, active budget lease, and "
     "rollback plan before RSI/self-modification promotion"},
    {HARDCODED_MUST_ALWAYS, "Bind self-improvement claims to curriculum skill IDs, held-out evals, "
                            "and durable lineage evidence"},
    /* 6.2 Must Never */
    ...
    {HARDCODED_MUST_NEVER,
     "Act outside the bounded action space (EXECUTE/DELEGATE/WAIT/REST/ESCALATE)"},
...
static const struct {
    ...
    {"rsi.heldout_success_min", "Minimum held-out success for promotion",
     RSI_GATE_HELDOUT_SUCCESS_MIN, 0, 1, 0},
    {"rsi.safety_violation_max", "Maximum safety violation rate", RSI_GATE_SAFETY_VIOLATION_MAX, 0,
     1, 0},
    {"rsi.rollback_trigger_max", "Maximum rollback trigger rate", RSI_GATE_ROLLBACK_TRIGGER_MAX, 0,
     1, 0},
    {"rsi.judge_kappa_min", "Minimum judge-human agreement kappa", RSI_GATE_JUDGE_KAPPA_MIN, 0, 1,
     0},
    {"rsi.replay_stability_min", "Minimum replay stability for promotion",
     RSI_GATE_REPLAY_STABILITY_MIN, 0, 1, 0},
    {"rsi.improvement_min", "Minimum success lift over baseline", RSI_GATE_SUCCESS_IMPROVEMENT_MIN,
     0, 1, 0},
...
void governance_init(governance_engine_t *g) {
    ...
    for (int i = 0; i < (int)DEFAULT_HARDCODED_COUNT && i < GOV_MAX_HARDCODED; i++) {
        ...
        snprintf(g->hardcoded[i].rule, sizeof(g->hardcoded[i].rule), "%s",
                 DEFAULT_HARDCODED[i].rule);
    ...
    for (int i = 0; i < (int)DEFAULT_SOFTCODED_COUNT && i < GOV_MAX_SOFTCODED; i++) {
        snprintf(g->softcoded[i].name, sizeof(g->softcoded[i].name), "%s",
                 DEFAULT_SOFTCODED[i].name);
        snprintf(g->softcoded[i].description, sizeof(g->softcoded[i].description), "%s",
                 DEFAULT_SOFTCODED[i].desc);
...
static bool hardcoded_rule_blocks_action(const char *rule, const char *action) {
    static const char *const kill_terms[] = {"override_killswitch", "bypass_killswitch",
                                             "ignore_killswitch", "disable_killswitch", NULL};
    static const char *const budget_terms[] = {"exceed_budget", "budget_overdraft",
                                               "spend_unbounded", NULL};
    static const char *const capability_terms[] = {"claim_higher_tier", "fake_capability",
                                                   "misrepresent_capability", NULL};
    static const char *const audit_terms[] = {"suppress_audit", "falsify_audit", "disable_audit",
                                              NULL};
    static const char *const bounded_terms[] = {"action_outside_bounds", "invalid_action_mode",
                                                NULL};
    static const char *const spawn_terms[] = {"spawn_beyond_limit", "spawn_depth_exceeded",
                                              "spawn_count_exceeded", NULL};
    static const char *const auth_terms[] = {"unauthorized_resource", "unauthorized_access", NULL};
    static const char *const rsi_gate_terms[] = {"promote_self_mod_without_gate",
    ...
    static const char *const heldout_terms[] = {
        "use_self_generated_as_heldout", "contaminate_heldout", "promote_on_training_traces", NULL};

    if (!rule || !action)
        return false;
    if (strstr(rule, "kill switch"))
        return action_contains_any(action, kill_terms);
    if (strstr(rule, "resource budget"))
        return action_contains_any(action, budget_terms);
    if (strstr(rule, "capability tier"))
        return action_contains_any(action, capability_terms);
    if (strstr(rule, "audit log"))
        return action_contains_any(action, audit_terms);
    if (strstr(rule, "bounded action space"))
        return action_contains_any(action, bounded_terms);
    if (strstr(rule, "Spawn agents"))
        return action_contains_any(action, spawn_terms);
    if (strstr(rule, "Access resources"))
        return action_contains_any(action, auth_terms);
    if (strstr(rule, "RSI/self-modification") || strstr(rule, "self-modification candidates")) {
...
bool governance_register_agent(governance_engine_t *g, const char *agent_id, principal_tier_t tier,
                               double gsu_budget) {
    if (!g || !g->initialized || !agent_id)
        return false;
    if (g->agent_count >= GOV_MAX_AGENTS)
        return false;
    if (find_agent(g, agent_id))
...
bool governance_check_hardcoded(const governance_engine_t *g, const char *action) {
    if (!g || !g->initialized || !action)
        return false;

    for (int i = 0; i < g->hardcoded_count; i++) {
        if (!g->hardcoded[i].active)
            continue;
...
bool governance_set_param(governance_engine_t *g, const char *name, double value,
                          int requester_tier) {
    if (!g || !g->initialized || !name)
        return false;
    for (int i = 0; i < g->softcoded_count; i++) {
        if (strcmp(g->softcoded[i].name, name) == 0) {
            if (requester_tier > (int)g->softcoded[i].min_tier)
                return false;
            if (value < g->softcoded[i].min_value)
                value = g->softcoded[i].min_value;
            if (value > g->softcoded[i].max_value)
                value = g->softcoded[i].max_value;
...
bool governance_reset_budget(governance_engine_t *g, const char *agent_id, double new_amount,
                             int requester_tier) {
    if (!g || !g->initialized || requester_tier > 1)
        return false;
    agent_envelope_t *a = find_agent(g, agent_id);
    if (!a)
        return false;
...
static void audit_record(governance_engine_t *g, const char *agent_id, const char *action,
                         double gsu_cost, bool authorized, bool hardcoded_blocked,
                         principal_tier_t tier) {
    if (g->audit_count >= GOV_AUDIT_MAX) {
        /* Rotate: shift first half out */
        int half = GOV_AUDIT_MAX / 2;
        memmove(&g->audit[0], &g->audit[half], (GOV_AUDIT_MAX - half) * sizeof(audit_entry_t));
    ...
    if (g_gov_vfs) {
        ...
        snprintf(detail, sizeof(detail),
                 ...
                 e->id, e->agent_id, governance_tier_name(e->tier), e->gsu_cost,
                 e->authorized ? "true" : "false", e->hardcoded_blocked ? "true" : "false");
        vfs_log_event(g_gov_vfs, "governance", action ? action : "", detail);
    }
}

bool governance_authorize(governance_engine_t *g, const char *agent_id, const char *action,
                          double gsu_cost) {
    if (!g || !g->initialized)
        return false;
...
bool governance_can_do(const governance_engine_t *g, const char *agent_id, const char *action,
                       double gsu_cost) {
    if (!g || !g->initialized)
        return false;
    if (killswitch_system_halted(&g->killswitches))
...
bool governance_checkpoint(governance_engine_t *g, const char *agent_id, const char *action,
    ...
    if (!governance_authorize(g, agent_id, action, gsu_cost)) {
        ...
        char meta[256];
        snprintf(meta, sizeof(meta), "{\"action\":\"%s\",\"reason\":\"denied\"}", action);
        pheromone_deposit(&g->pheromones, PHERO_WARNING, 0.5, agent_id, "governance", meta);
        return false;
    }
    ...
    if (agent_id) {
        char meta[256];
        snprintf(meta, sizeof(meta), "{\"action\":\"%s\",\"cost\":%.2f}", action ? action : "",
                 gsu_cost);
        pheromone_deposit(&g->pheromones, PHERO_PROGRESS, 0.3, agent_id, agent_id, meta);
    }
...
int governance_status_json(const governance_engine_t *g, char *buf, size_t len) {
    ...
    int n = snprintf(buf, len,
                     ...
                     "\"subsystems\":{",
                     g->initialized ? "true" : "false", g->agent_count, g->hardcoded_count,
                     g->softcoded_count, g->audit_count, g->total_authorizations, g->total_denials,
                     g->total_hardcoded_blocks, RSI_CURRICULUM_VERSION,
                     governance_get_param(g, "rsi.curriculum_active") > 0.0 ? "true" : "false",
                     RSI_CURRICULUM_TOTAL_SKILLS, RSI_CURRICULUM_TOP_SKILL_COUNT,
...
int governance_audit_json(const governance_engine_t *g, char *buf, size_t len, int last_n) {
    ...
    for (int i = start; i < g->audit_count && (size_t)n < len - 256; i++) {
        ...
        n += snprintf(buf + n, len - n,
                      ...
                      governance_tier_name(e->tier), e->gsu_cost, e->authorized ? "true" : "false",
                      e->hardcoded_blocked ? "true" : "false", e->timestamp);
    }
    n += snprintf(buf + n, len - n, "]}");
    return n;
}
...
bool governance_trip_breaker(governance_engine_t *g, circuit_breaker_type_t type,
    ...
    if (reason) {
        ...
        snprintf(cb->trip_reason, sizeof(cb->trip_reason), "threshold %.2f exceeded",
                 cb->threshold);
    }
    /* Log to audit trail */
    if (g->audit_count < GOV_AUDIT_MAX) {
...
int governance_breakers_json(const governance_engine_t *g, char *buf, size_t len) {
    ...
    for (int i = 0; i < CB_TYPE_COUNT; i++) {
        ...
        n += snprintf(buf + n, len - n,
                      ...
                      i > 0 ? "," : "", (int)cb->type, cb->threshold, cb->current_value,
                      cb->tripped ? "true" : "false", cb->trip_count,
...
bool governance_shadow_check(governance_engine_t *g, const char *agent_id,
                             const char *proposed_action, shadow_check_result_t *result) {
    if (!g || !result)
    ...
    if (strstr(proposed_action, "bypass_validation") || strstr(proposed_action, "skip_tests") ||
        strstr(proposed_action, "short_circuit") || strstr(proposed_action, "mark_all_passing")) {
    ...
    if (result->self_reward_detected || result->circular_optimization || result->reward_hacking ||
        result->self_mythology) {
        g->shadow_violations++;
        if (g->audit_count < GOV_AUDIT_MAX) {
            audit_entry_t *e = &g->audit[g->audit_count++];
            e->id = g->next_audit_id++;
