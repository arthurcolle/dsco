...
static const char *PHASE_NAMES[] = {
    "idle", "observe", "orient", "decide", "act", "complete"
};

static const char *ACTION_NAMES[] = {
    "EXECUTE", "DELEGATE", "WAIT", "REST", "ESCALATE"
};
...
bool ooda_observe(ooda_engine_t *e, const char *content,
    ...
    if (e->current.phase != OODA_PHASE_OBSERVE) return false;
    if (e->current.observation_count >= OODA_MAX_OBSERVATIONS) return false;
...
bool ooda_orient_add(ooda_engine_t *e, const char *factor,
    ...
    if (e->current.phase != OODA_PHASE_ORIENT) return false;
    if (e->current.factor_count >= OODA_MAX_FACTORS) return false;
...
ooda_action_t ooda_decide(ooda_engine_t *e) {
    if (!e || !e->initialized) return OODA_ACTION_ESCALATE;
    ...
    if (e->current.phase != OODA_PHASE_DECIDE) return OODA_ACTION_ESCALATE;
    ...
    if (d->confidence >= exec_thresh) {
        ...
        snprintf(d->reason, sizeof(d->reason),
                 "High confidence (%.2f >= %.2f), proceeding with execution",
                 d->confidence, exec_thresh);
    } else if (d->confidence >= deleg_thresh) {
        d->action = OODA_ACTION_EXECUTE;
        d->capability = CAPABILITY_PROFICIENT;
        d->requires_confirmation = true;
        snprintf(d->reason, sizeof(d->reason),
                 "Moderate confidence (%.2f >= %.2f), executing with monitoring",
                 d->confidence, deleg_thresh);
    } else if (d->confidence >= esc_thresh) {
        d->action = OODA_ACTION_DELEGATE;
        d->capability = CAPABILITY_COMPETENT;
        snprintf(d->reason, sizeof(d->reason),
                 "Low confidence (%.2f), delegating to more capable agent",
                 d->confidence);
    } else {
        d->action = OODA_ACTION_ESCALATE;
        d->capability = CAPABILITY_NOVICE;
        snprintf(d->reason, sizeof(d->reason),
                 "Very low confidence (%.2f < %.2f), escalating to human",
                 d->confidence, esc_thresh);
    }
...
bool ooda_act_result(ooda_engine_t *e, bool success, const char *result) {
    if (!e || !e->initialized) return false;
    if (e->current.phase != OODA_PHASE_ACT) return false;

    e->current.action_taken = true;
    e->current.action_success = success;
    if (result)
        snprintf(e->current.action_result, sizeof(e->current.action_result),
                 "%s", result);
...
bool ooda_complete(ooda_engine_t *e) {
    if (!e || !e->initialized) return false;
    if (e->current.phase == OODA_PHASE_IDLE ||
    ...
    case OODA_ACTION_EXECUTE:  e->execute_count++; break;
    case OODA_ACTION_DELEGATE: e->delegate_count++; break;
    case OODA_ACTION_WAIT:     e->wait_count++; break;
    case OODA_ACTION_REST:     e->rest_count++; break;
    case OODA_ACTION_ESCALATE: e->escalate_count++; break;
    }
...
const ooda_decision_t *ooda_current_decision(const ooda_engine_t *e) {
    if (!e || e->current.phase < OODA_PHASE_ACT) return NULL;
...
int ooda_cycle_to_json(const ooda_cycle_t *c, char *buf, size_t len) {
    ...
    return snprintf(buf, len,
        ...
        c->id, ooda_phase_name(c->phase),
        c->observation_count, c->factor_count,
        c->principal_tier, c->safety_critical ? "true" : "false",
        ooda_action_name(c->decision.action), c->decision.confidence,
        ooda_capability_name(c->decision.capability),
        c->decision.reason,
        c->action_taken ? "true" : "false",
        c->action_success ? "true" : "false");
}
...
int ooda_to_json(const ooda_engine_t *e, char *buf, size_t len) {
    ...
    int n = snprintf(buf, len,
        ...
        "\"current\":",
        e->total_cycles, e->avg_confidence,
        e->execute_count, e->delegate_count,
        e->wait_count, e->rest_count, e->escalate_count,
        ooda_success_rate(e, 10),
