...
int dsco_accel_init(void) {
    ...
    switch (g_info.batch_backend) {
        case DSCO_ACCEL_BACKEND_MLX:        batch_n = "mlx"; break;
        case DSCO_ACCEL_BACKEND_ACCELERATE: batch_n = "accelerate"; break;
        case DSCO_ACCEL_BACKEND_SIMD:       batch_n = cpu_n; break;
        default: break;
    }
    snprintf(g_info.banner, sizeof(g_info.banner),
             "dsco-accel: cpu=%s, batch=%s, %d cores%s",
             cpu_n, batch_n, g_info.cpu_count,
             g_info.mlx_loaded ? ", mlx" : "");
    return 0;
}
...
const dsco_accel_info_t *dsco_accel_info(void) { return &g_info; }
...
extern int dsco_mlx_cosine_batch(const float *q, int dim,
                                  const float *cands, int n, float *out);

int dsco_accel_cosine_batch(const float *q, int dim,
                             const float *cands, int n, float *out) {
    if (!q || !cands || !out || dim <= 0 || n <= 0) return -1;
    ...
    cblas_sgemv(CblasRowMajor, CblasNoTrans, n, dim, 1.0f,
                cands, dim, q, 1, 0.0f, out, 1);
...
int dsco_accel_topk(const float *x, int n, int k, int *out_idx, float *out_val) {
    if (!x || n <= 0 || k <= 0 || !out_idx || !out_val) return -1;
    if (k > n) k = n;
    /* min-heap of size k */
    for (int i = 0; i < k; i++) { out_idx[i] = i; out_val[i] = x[i]; }
    /* heapify (sift down) */
    for (int start = (k - 2) / 2; start >= 0; start--) {
        ...
        for (;;) {
            int child = 2 * root + 1;
            if (child >= k) break;
            if (child + 1 < k && out_val[child + 1] < out_val[child]) child++;
            if (out_val[root] <= out_val[child]) break;
            float tv = out_val[root]; out_val[root] = out_val[child]; out_val[child] = tv;
            int ti = out_idx[root]; out_idx[root] = out_idx[child]; out_idx[child] = ti;
            root = child;
        }
    }
    for (int i = k; i < n; i++) {
        if (x[i] > out_val[0]) {
            out_val[0] = x[i]; out_idx[0] = i;
            int root = 0;
            for (;;) {
                int child = 2 * root + 1;
                if (child >= k) break;
    ...
    for (int i = 0; i < k - 1; i++) {
        int best = i;
        for (int j = i + 1; j < k; j++) if (out_val[j] > out_val[best]) best = j;
        if (best != i) {
            float tv = out_val[i]; out_val[i] = out_val[best]; out_val[best] = tv;
            int ti = out_idx[i]; out_idx[i] = out_idx[best]; out_idx[best] = ti;
        }
    }
    return k;
}
...
int dsco_accel_dequant_int8(const int8_t *src, const float *scales,
    ...
    if (!src || !scales || !out || n <= 0 || dim <= 0) return -1;
    for (int i = 0; i < n; i++) {
        float s = scales[i];
        const int8_t *row = src + (size_t)i * dim;
        float *o = out + (size_t)i * dim;
        for (int j = 0; j < dim; j++) o[j] = (float)row[j] * s;
    }
    return 0;
}
...
int dsco_accel_quant_int8(const float *src, int n, int dim,
    ...
    if (!src || !scales_out || !dst || n <= 0 || dim <= 0) return -1;
    for (int i = 0; i < n; i++) {
