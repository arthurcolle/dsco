...
static void persist_locked(void) {
    ...
    for (int i = 0; i < s_count; i++) {
        ...
        fprintf(f,
                ...
                "\"lat\":%.3f,\"obs\":%d,\"ts\":%lld}\n",
                COST_MODEL_VERSION, e->name, e->cost_per_1k_ema, e->latency_ema, e->observations,
...
void cost_model_learn(const char *topology_name, int total_tokens, double actual_cost_usd,
    ...
    if (!topology_name || total_tokens <= 0 || actual_cost_usd < 0)
        return;

    double new_per_1k = (total_tokens > 0) ? actual_cost_usd / (total_tokens / 1000.0) : 0.0;
    ...
    if (e->observations == 0) {
        ...
        e->latency_ema = alpha * actual_latency_s + (1.0 - alpha) * e->latency_ema;
    }
...
double cost_model_predict(const char *topology_name, int input_tokens, int output_tokens) {
    ...
    double result = -1.0;
    for (int i = 0; i < s_count; i++) {
        if (strcmp(s_entries[i].name, topology_name) == 0 && s_entries[i].observations >= 2) {
...
int cost_model_stats_json(char *buf, size_t buflen) {
    ...
    for (int i = 0; i < s_count && rem > 8; i++) {
        const cost_entry_t *e = &s_entries[i];
        n = snprintf(p, rem,
                     "%s{\"name\":\"%s\",\"cost_per_1k\":%.6f,\"cost_stddev\":%.6f,\"lat_s\":%.2f,"
                     "\"obs\":%d}",
                     i ? "," : "", e->name, e->cost_per_1k_ema, sqrt(e->cost_per_1k_var),
...
bool cost_model_predict_full(const char *topology_name, int input_tokens, int output_tokens,
    ...
    for (int i = 0; i < s_count; i++) {
        ...
        if (strcmp(e->name, topology_name) != 0)
            continue;
        if (e->observations < 1)
            break;
