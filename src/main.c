...
static void usage(const char *prog) {
    fprintf(stderr,
        ...
        "       %s matrix <status|doctor|canary|logs|repair>  Matrix inference appliance\n"
        "\n"
        "Options:\n"
...
static int run_route_explain_fast(int argc, char **argv) {
    ...
    const char *local_fallback = getenv("DSCO_LOCAL_FALLBACK_MODEL");
    printf("local_fallback_model: %s\n",
           (local_fallback && local_fallback[0]) ? local_fallback : "(none)");
...
static int route_catalog_cli(void);
...
static bool main_is_dispatch_subcommand(const char *arg) {
    return arg &&
           (strcmp(arg, "tools") == 0 ||
            ...
            strcmp(arg, "route-catalog") == 0 ||
            strcmp(arg, "matrix") == 0 ||
            strcmp(arg, "mcp") == 0 ||
            strcmp(arg, "pets") == 0 ||
            strcmp(arg, "cluster") == 0 ||
            strcmp(arg, "config") == 0 ||
...
static int main_dispatch_subcommand_normalized(int argc, char **argv,
    ...
    *handled = true;
    ...
    else if (strcmp(sub, "cluster") == 0)
        rc = dsco_cluster_cli(nargc, nargv);
    else if (strcmp(sub, "runtime") == 0)
        rc = runtime_cli(nargc, nargv);
    else if (strcmp(sub, "route-catalog") == 0)
        rc = route_catalog_cli();
    else if (strcmp(sub, "matrix") == 0)
        rc = matrix_inference_cli(nargc, nargv);
    else if (strcmp(sub, "mcp") == 0) {
        const char *action = nargc >= 3 ? nargv[2] : "help";
        if (strcmp(action, "serve") == 0) {
            ...
            FILE *out = (strcmp(action, "help") == 0 || strcmp(action, "-h") == 0 ||
                         strcmp(action, "--help") == 0) ? stdout : stderr;
            fprintf(out,
                    "Usage: %s mcp serve [--toolsets SETS] [--tier TIER]\n"
                    "\n"
                    "Expose governed dsco tools over MCP stdio JSON-RPC.\n"
                    "  --toolsets SETS   comma list: core,ast,swarm,market,crypto,all\n"
                    "  --tier TIER       untrusted (default), standard, or trusted\n",
                    nargv[0]);
            rc = out == stdout ? 0 : 2;
        }
...
static int route_catalog_cli(void) {
    dsco_route_catalog_t catalog;
    dsco_route_catalog_init(&catalog);
    printf("{\"catalog_cap\":%d,\"materialized_routes\":%zu,\"executor_cap\":%d,\"routes\":[",
           DSCO_ROUTE_CATALOG_CAP, dsco_route_catalog_count(&catalog),
           dsco_swarm_max_children());
    printf("]}\n");
    dsco_route_catalog_destroy(&catalog);
    return 0;
}
...
static void runtime_cli_usage(FILE *out, const char *argv0) {
    fprintf(out,
            ...
            "  route-catalog dry-run    Emit a no-network route-catalog capacity plan\n",
            argv0);
}
    ...
    if (oneshot_prompt) {
        ...
        while (turns < oneshot_hard_ceiling && !g_main_interrupted) {
            ...
            if (!sr.ok) {
                ...
                if (!oneshot_local_fallback_attempted && local_fb_model && local_fb_model[0]) {
