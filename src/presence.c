...
static void *presence_thread(void *arg) {
    ...
    int kq = kqueue();
    if (kq < 0) { audit_log("presence", "kqueue() failed"); return NULL; }
    ...
    EV_SET(&ev, 1, EVFILT_TIMER, EV_ADD | EV_ENABLE,
           NOTE_SECONDS, POLL_INTERVAL_S, NULL);
    ...
    while (atomic_load(&s_running)) {
        ...
        if (triggered.filter == EVFILT_USER) break;

        if (!atomic_load(&s_running)) break;
        if (atomic_load(&s_locked)) continue;   /* already locked; skip */
...
void presence_start(void) {
    if (atomic_exchange(&s_running, 1)) return;  /* already running */
    pthread_create(&s_thread, NULL, presence_thread, NULL);
}

void presence_stop(void) {
    if (!atomic_exchange(&s_running, 0)) return;
...
bool presence_is_locked(void) { return atomic_load(&s_locked) != 0; }
...
void   presence_start(void)  {}
void   presence_stop(void)   {}
double presence_idle_seconds(void) { return 0.0; }
bool   presence_is_locked(void)    { return false; }
void   presence_mark_unlocked(void) {}
void   presence_poke(void)         {}
