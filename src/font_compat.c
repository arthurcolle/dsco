...
static font_cache_entry_t s_font_cache[12];
...
static CTFontRef create_font(float point_size, bool bold, bool italic) {
    ...
    for (size_t i = 0; i < sizeof(s_font_cache) / sizeof(s_font_cache[0]); i++) {
        ...
        if (entry->font && fabsf(entry->point_size - point_size) < 0.01f &&
    ...
    const char *configured = getenv("DSCO_PIXEL_FONT");
    CTFontRef font = NULL;
    if (configured && *configured) {
        CFStringRef name = CFStringCreateWithCString(
            kCFAllocatorDefault, configured, kCFStringEncodingUTF8);
        if (name) {
    ...
    if (!font) {
        ...
        font = CTFontCreateWithName(CFSTR("IosevkaTerm Nerd Font Mono"),
                                    point_size, NULL);
    }
    if (!font)
        font = CTFontCreateUIFontForLanguage(kCTFontUIFontUserFixedPitch,
                                             point_size, NULL);
    if (!font) {
    ...
    if (font && (bold || italic)) {
        ...
        CTFontRef strong = CTFontCreateCopyWithSymbolicTraits(
            font, point_size, NULL, wanted, wanted);
        if (strong) {
            CFRelease(font);
            font = strong;
        }
...
int font_compat_measure_utf8_styled(const char *utf8, float point_size,
    ...
    if (!utf8 || !*utf8 || point_size < 4.0f) return 0;
    ...
    CFAttributedStringRef attributed = attrs ? CFAttributedStringCreate(
        kCFAllocatorDefault, string, attrs) : NULL;
...
int font_compat_line_height(float point_size, bool bold) {
    ...
    if (!font) return -1;
    int height = (int)ceil(CTFontGetAscent(font) + CTFontGetDescent(font) +
...
int font_compat_draw_rgb(uint8_t *rgb, int width, int height, int stride,
                         ...
                         float opacity) {
    return font_compat_draw_rgb_styled(rgb, width, height, stride, x, y,
                                       max_width, utf8, point_size, bold, false,
...
int font_compat_draw_rgb_styled(uint8_t *rgb, int width, int height, int stride,
    ...
    uint8_t *mask = calloc((size_t)mask_w, (size_t)mask_h);
    CGColorSpaceRef gray = CGColorSpaceCreateDeviceGray();
    CGContextRef context = gray ? CGBitmapContextCreate(
        mask, (size_t)mask_w, (size_t)mask_h, 8, (size_t)mask_w, gray,
        (CGBitmapInfo)kCGImageAlphaNone) : NULL;
    if (context) {
...
int font_compat_measure_utf8(const char *utf8, float point_size, bool bold) {
    (void)utf8; (void)point_size; (void)bold;
    return -1;
}

int font_compat_measure_utf8_styled(const char *utf8, float point_size,
                                    bool bold, bool italic) {
    (void)utf8; (void)point_size; (void)bold; (void)italic;
    return -1;
}

int font_compat_line_height(float point_size, bool bold) {
    (void)point_size; (void)bold;
    return -1;
}
...
int font_compat_draw_rgb(uint8_t *rgb, int width, int height, int stride,
                         ...
                         uint8_t red, uint8_t green, uint8_t blue,
                         float opacity) {
    (void)rgb; (void)width; (void)height; (void)stride; (void)x; (void)y;
    (void)max_width; (void)utf8; (void)point_size; (void)bold;
    (void)red; (void)green; (void)blue; (void)opacity;
    return -1;
}
...
int font_compat_draw_rgb_styled(uint8_t *rgb, int width, int height, int stride,
                                ...
                                float opacity) {
    (void)rgb; (void)width; (void)height; (void)stride; (void)x; (void)y;
    (void)max_width; (void)utf8; (void)point_size; (void)bold; (void)italic;
    (void)red; (void)green; (void)blue; (void)opacity;
    return -1;
}
