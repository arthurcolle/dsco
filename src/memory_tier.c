...
double memory_tier_halflife(memory_tier_t t) {
    switch (t) {
    case MEM_WORKING:  return MEM_WORKING_HALFLIFE;
    case MEM_EPISODIC: return MEM_EPISODIC_HALFLIFE;
    case MEM_SEMANTIC: return MEM_SEMANTIC_HALFLIFE;
    default: return 0;
    }
}
...
double memory_calc_strength(memory_tier_t tier, double created_at, double now) {
    ...
    if (halflife <= 0) return 1.0; /* semantic = no decay */
...
static int find_slot(memory_store_t *m) {
    ...
    for (int i = 0; i < MEMTIER_MAX_ENTRIES; i++) {
        if (!m->entries[i].active) return i;
    }
    ...
    double t = now_sec();
    for (int i = 0; i < MEMTIER_MAX_ENTRIES; i++) {
        if (m->entries[i].pinned) continue;
...
int memory_store_tagged(memory_store_t *m, memory_tier_t tier,
    ...
    if (!m || !m->initialized || !key || !value) return -1;
...
const memory_entry_t *memory_recall(memory_store_t *m, const char *key) {
    if (!m || !m->initialized || !key) return NULL;
    m->total_recalls++;
...
int memory_recall_by_tag(memory_store_t *m, const char *tag,
    ...
    if (!m || !m->initialized || !tag || !out) return 0;
    int count = 0;
...
int memory_recall_tier(memory_store_t *m, memory_tier_t tier,
                       const memory_entry_t **out, int max) {
    if (!m || !m->initialized || !out) return 0;
    int count = 0;
    for (int i = 0; i < MEMTIER_MAX_ENTRIES && count < max; i++) {
...
int memory_search(memory_store_t *m, const char *query,
                  const memory_entry_t **out, int max) {
    if (!m || !m->initialized || !query || !out) return 0;
    int count = 0;
    for (int i = 0; i < MEMTIER_MAX_ENTRIES && count < max; i++) {
        if (!m->entries[i].active) continue;
        if (strstr(m->entries[i].key, query) ||
            strstr(m->entries[i].value, query)) {
            out[count++] = &m->entries[i];
        }
    }
    return count;
...
bool memory_update(memory_store_t *m, const char *key, const char *value) {
    if (!m || !m->initialized || !key || !value) return false;
    memory_entry_t *e = find_by_key(m, key);
    if (!e) return false;
    snprintf(e->value, sizeof(e->value), "%s", value);
    e->last_accessed = now_sec();
...
bool memory_forget(memory_store_t *m, const char *key) {
    if (!m || !m->initialized || !key) return false;
    memory_entry_t *e = find_by_key(m, key);
    if (!e) return false;
    m->tier_count[e->tier]--;
    m->count--;
...
bool memory_promote(memory_store_t *m, const char *key) {
    ...
    memory_entry_t *e = find_by_key(m, key);
    if (!e) return false;
    if (e->tier >= MEM_SEMANTIC) return false; /* already at top */
...
int memory_consolidate(memory_store_t *m) {
    ...
    if (t - m->last_consolidation < MEM_CONSOLIDATION_INTERVAL) return 0;
    ...
    for (int i = 0; i < MEMTIER_MAX_ENTRIES; i++) {
        ...
        if (e->access_count >= MEM_CONSOLIDATION_ACCESS_COUNT &&
            e->tier == MEM_WORKING) {
            should_promote = true;
        }
        if (e->importance >= MEM_CONSOLIDATION_IMPORTANCE &&
            e->tier < MEM_SEMANTIC) {
            should_promote = true;
        }
        /* Episodic → Semantic: need more evidence */
        if (e->tier == MEM_EPISODIC &&
            e->access_count >= MEM_CONSOLIDATION_ACCESS_COUNT * 2 &&
...
int memory_status_json(const memory_store_t *m, char *buf, size_t len) {
    ...
    return snprintf(buf, len,
        ...
        m->count,
        m->tier_count[MEM_WORKING], m->tier_count[MEM_EPISODIC],
        m->tier_count[MEM_SEMANTIC],
        m->total_stores, m->total_recalls,
        m->total_promotions, m->total_evictions,
        m->total_consolidations);
}
...
int memory_to_json(const memory_store_t *m, char *buf, size_t len) {
    ...
    for (int i = 0; i < MEMTIER_MAX_ENTRIES && (size_t)n < len - 512; i++) {
        ...
        n += snprintf(buf + n, len - n,
            ...
            first ? "" : ",", e->id, memory_tier_name(e->tier),
            e->key, e->importance, strength,
...
int memory_tier_to_json(const memory_store_t *m, memory_tier_t tier,
    ...
    if (!m || !buf) return 0;
    double t = now_sec();
    int n = snprintf(buf, len, "{\"tier\":\"%s\",\"entries\":[",
                     memory_tier_name(tier));

    bool first = true;
    for (int i = 0; i < MEMTIER_MAX_ENTRIES && (size_t)n < len - 512; i++) {
        const memory_entry_t *e = &m->entries[i];
        if (!e->active || e->tier != tier) continue;
        ...
        n += snprintf(buf + n, len - n,
            ...
            first ? "" : ",", e->key,
            (int)(strlen(e->value) > 80 ? 80 : strlen(e->value)), e->value,
