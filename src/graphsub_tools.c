...
static bool tool_graphsub_register(const char *input, char *result, size_t rlen) {
    ...
    char *resp = graphsub_agent_register(
        agent_id,
        model ? model : "unknown",
    ...
    free(model); free(capabilities); free(tools_list);
    free(hostname); free(bridge_peer);
    return resp != NULL;
}
...
static bool tool_graphsub_pheromone(const char *input, char *result, size_t rlen) {
    ...
    if (!action) action = strdup("deposit");
    ...
    if (strcmp(action, "deposit") == 0) {
        ...
        char *resp = graphsub_pheromone_deposit(
            ...
            intensity, ttl, metadata);

        if (resp) { snprintf(result, rlen, "%s", resp); free(resp); }
        else snprintf(result, rlen, "{\"error\":\"deposit failed\"}");

        free(target_type); free(target_id); free(signal_type); free(metadata);
    } else if (strcmp(action, "query") == 0) {
        char *target_id = json_get_str(input, "target_id");
        char *signal_type = json_get_str(input, "signal_type");
        double min_intensity = json_get_double(input, "min_intensity", 0.0);

        char *resp = graphsub_pheromone_query(
            target_id ? target_id : "",
            signal_type,  /* may be NULL */
            min_intensity);

        if (resp) { snprintf(result, rlen, "%s", resp); free(resp); }
        else snprintf(result, rlen, "{\"error\":\"query failed\"}");

        free(target_id); free(signal_type);
    } else if (strcmp(action, "sweep") == 0) {
        char *target_type = json_get_str(input, "target_type");
        char *signal_type = json_get_str(input, "signal_type");

        char *resp = graphsub_pheromone_sweep(
            target_type,  /* may be NULL */
            signal_type); /* may be NULL */

        if (resp) { snprintf(result, rlen, "%s", resp); free(resp); }
        else snprintf(result, rlen, "{\"error\":\"sweep failed\"}");

        free(target_type); free(signal_type);
    } else {
        snprintf(result, rlen, "{\"error\":\"unknown action: %s (use deposit|query|sweep)\"}", action);
    }
...
static bool tool_graphsub_query(const char *input, char *result, size_t rlen) {
    ...
    char *resp = graphsub_graph_traverse(
        start ? start : "",
        depth,
        edge_filter,  /* may be NULL */
        limit);

    if (resp) { snprintf(result, rlen, "%s", resp); free(resp); return true; }
    else { snprintf(result, rlen, "{\"error\":\"traverse failed\"}"); return false; }

    free(start); free(edge_filter);
}
...
static bool tool_graphsub_memory_sync(const char *input, char *result, size_t rlen) {
    ...
    char *resp = graphsub_memory_sync(
        agent_id,
        episodes ? episodes : "[]",
        type ? type : "episodic");

    if (resp) { snprintf(result, rlen, "%s", resp); free(resp); return true; }
    else { snprintf(result, rlen, "{\"error\":\"memory sync failed\"}"); return false; }

    free(episodes); free(type);
}
...
static bool tool_graphsub_swarm(const char *input, char *result, size_t rlen) {
    ...
    char *resp = graphsub_swarm_register(
        swarm_id,
    ...
    if (resp) { snprintf(result, rlen, "%s", resp); free(resp); return true; }
    else { snprintf(result, rlen, "{\"error\":\"swarm register failed\"}"); return false; }

    if (swarm_id != auto_id) free(swarm_id);
    free(topology); free(agents); free(task);
}
...
static bool tool_graphsub_fleet(const char *input, char *result, size_t rlen) {
    ...
    if (resp) { snprintf(result, rlen, "%s", resp); free(resp); return true; }
    else { snprintf(result, rlen, "{\"error\":\"fleet registration failed\"}"); return false; }
