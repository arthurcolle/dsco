...
#define SPINE_AGENT_MAX_TURNS 12
#define SPINE_AGENT_MAX_TOOLS 48
...
static void usage(FILE *out) {
    fprintf(out, "usage: %s [-a] [-e] [-t tier] [-m model] [-S schema] [-c command] [-f script]\n", SPINE_NAME);
    fputs("  :tool JSON   governed DSCO tool\n"
          ...
          "  -a           bounded agent loop (model tool_use → governed tool → resample)\n"
...
static int complete_agent(spine_t *s, const char *prompt) {
    ...
    int tools_used = 0;
    int rc = 1;
    for (int turn = 1; turn <= s->max_turns; turn++) {
        char *request = llm_build_request_ex(&conv, &ss, 4096);
        if (!request) break;
        fprintf(stderr, "[agent turn %d/%d]\n", turn, s->max_turns);
        ...
        for (int i = 0; i < r.parsed.count; i++)
            if (r.parsed.blocks[i].type && !strcmp(r.parsed.blocks[i].type, "tool_use"))
                has_tools = true;
        conv_add_assistant_raw(&conv, &r.parsed);
        if (!has_tools) {
            free(r.actual_model); free(r.generation_id);
            rc = 0;
            break;
        }
        for (int i = 0; i < r.parsed.count; i++) {
            content_block_t *b = &r.parsed.blocks[i];
            if (!b->type || strcmp(b->type, "tool_use")) continue;
            if (++tools_used > s->max_tools) {
                conv_add_tool_result_named(&conv, b->tool_id, b->tool_name,
                    "Agent tool budget exhausted. Produce a final answer using available evidence.", true);
                continue;
            }
            char why[256];
            if (!tools_is_allowed_for_tier(b->tool_name, s->tier, why, sizeof(why))) {
                fprintf(stderr, "[tool denied] %s: %s\n", b->tool_name, why);
                conv_add_tool_result_named(&conv, b->tool_id, b->tool_name, why, true);
                continue;
            }
            char valid[256];
            if (!tools_validate_input(b->tool_name, b->tool_input, valid, sizeof(valid))) {
                fprintf(stderr, "[tool invalid] %s: %s\n", b->tool_name, valid);
                conv_add_tool_result_named(&conv, b->tool_id, b->tool_name, valid, true);
                continue;
            }
            s->result[0] = 0;
            fprintf(stderr, "[tool] %s\n", b->tool_name);
            bool ok = tools_execute_for_tier(b->tool_name, b->tool_input, s->tier,
                                             s->result, s->result_cap);
            dsco_strip_terminal_controls_inplace(s->result);
            conv_add_tool_result_named(&conv, b->tool_id, b->tool_name, s->result, !ok);
        }
        free(r.actual_model); free(r.generation_id);
    }
    if (rc) fprintf(stderr, "[agent stopped: turn/tool limit or provider failure]\n");
    conv_free(&conv);
    return rc;
}

static int complete(spine_t *s, const char *prompt, bool structured) {
    if (s->agent_mode && !structured) return complete_agent(s, prompt);
    const char *key = getenv("ANTHROPIC_API_KEY");
    if (!key || !*key) key = getenv("OPENROUTER_API_KEY");
    FAIL_IF(!key || !*key, "missing ANTHROPIC_API_KEY or OPENROUTER_API_KEY");
    conversation_t conv; conv_init(&conv); conv_add_user_text(&conv, prompt);
...
int main(int argc, char **argv) {
    ...
    int opt;
    while ((opt = getopt(argc, argv, "ac:f:t:m:S:eh")) != -1) {
        if (opt == 'a') agent_mode = true;
        else if (opt == 'c') command = optarg;
    ...
    spine_t s = {.tier = tier, .result_cap = RESULT_CAP, .stop_on_error = stop_on_error,
                 .model = model, .schema = schema, .agent_mode = agent_mode,
                 .max_turns = SPINE_AGENT_MAX_TURNS, .max_tools = SPINE_AGENT_MAX_TOOLS};
