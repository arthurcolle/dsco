...
static const char *GOAL_STATE_NAMES[] = {"nascent",   "stalking", "striking",
                                         "gripping",  "captured", "escaped",
                                         "abandoned", "blocked",  "regrouping"};

static const char *STRATEGY_NAMES[] = {
    "direct", "flanking", "tournament", "escalation", "divide", "ambush", "attrition", "pincer",
    "blitz", "siege", "feint", "opportunistic",
    /* military-history canon — order must match strategy_type_t */
    "envelopment", "encirclement", "guerrilla", "scorched_earth", "fabian", "defense_in_depth",
    "oblique", "infiltration", "interior_lines", "defeat_in_detail", "turning_movement",
    "breakthrough", "shock", "decapitation", "blockade", "raid", "indirect", "tempo", "deterrence",
    "counterattack", "maneuver", "hedgehog", "screen", "asymmetric",
    /* extended canon — order must match strategy_type_t */
    "othismos", "hammer_and_anvil", "peltast_softening", "manipular_tactics", "circumvallation",
    "castrametation", "win_without_fighting", "besiege_wei_to_rescue_zhao", "feigned_retreat",
    "tulughma", "chevauchee", "castellation", "fleet_in_being", "guerre_de_course",
    "crossing_the_t", "breaking_the_line", "wolfpack", "central_position", "grand_battery",
    "hard_war", "anaconda_plan", "stormtrooper_tactics", "elastic_defense", "bite_and_hold",
    "auftragstaktik", "deep_operation", "kesselschlacht", "maskirovka", "douhet_doctrine",
    "maoist_protracted_war", "clear_hold_build", "mutual_assured_destruction", "flexible_response",
    "reflexive_control", "hybrid_warfare", "network_centric_warfare"};

static const char *GRIP_NAMES[] = {"tentative", "holding", "locked", "death_grip"};
...
static void record_hunt(talons_engine_t *t, const goal_t *g, bool won) {
     ...
     * tamper-evident audit log. */
    {
        ...
        snprintf(audit_msg, sizeof(audit_msg),
                 ...
                 g->id, strat_name, state_name, g->attempts, g->total_cost, h->duration,
                 g->confidence);
...
int talons_goal_create(talons_engine_t *t, const char *description, double priority,
...
int talons_goal_create_sub(talons_engine_t *t, int parent_id, const char *description,
                           double priority) {
    goal_t *parent = find_goal(t, parent_id);
    if (!parent)
        return -1;

    int id = talons_goal_create(t, description, priority, parent->grip, parent->strategy,
                                parent->deadline);
    if (id >= 0) {
...
bool talons_goal_capture(talons_engine_t *t, int goal_id, const char *result, double cost) {
    ...
    g->total_cost += cost;
    if (result)
        snprintf(g->last_result, sizeof(g->last_result), "%s", result);
...
bool talons_goal_escaped(talons_engine_t *t, int goal_id, const char *reason, double cost) {
    ...
    g->total_cost += cost;
    if (reason)
        snprintf(g->last_result, sizeof(g->last_result), "%s", reason);
...
bool talons_goal_depends_on(talons_engine_t *t, int goal_id, int dep_goal_id) {
    if (!t || goal_id == dep_goal_id)
        return false;
    goal_t *g = find_goal(t, goal_id);
    if (!g || !find_goal(t, dep_goal_id))
        return false;
    if (g->dep_count >= TALONS_MAX_DEPS)
        return false;
    for (int i = 0; i < g->dep_count; i++)
        if (g->deps[i] == dep_goal_id)
            return true; /* already a dep */
...
bool talons_goal_deps_met(const talons_engine_t *t, int goal_id) {
    ...
    for (int i = 0; i < g->dep_count; i++) {
        ...
        if (!dep || dep->state != GOAL_CAPTURED)
            return false;
    }
    return true;
}
...
int talons_tick(talons_engine_t *t, double now) {
    ...
    for (int i = 0; i < t->goal_count; i++) {
        goal_t *g = &t->goals[i];
        if (goal_state_is_terminal(g->state) || g->deadline <= 0)
            continue;
        ...
        if (g->created_at > 0 && g->grip < GRIP_DEATH_GRIP) {
            ...
            if (window > 0 && (now - g->created_at) / window >= 0.75) {
                if (talons_escalate_grip(t, g->id))
                    changes++;
            }
        }
    }
...
int talons_tournament_begin(talons_engine_t *t, const char *objective, double deadline,
...
static tournament_t *find_tournament(talons_engine_t *t, int tid) {
    for (int i = 0; i < t->tournament_count; i++) {
        if (t->tournaments[i].id == tid)
            return &t->tournaments[i];
    }
    return NULL;
}

int talons_tournament_add(talons_engine_t *t, int tournament_id, const char *label,
...
bool talons_tournament_result(talons_engine_t *t, int tournament_id, int competitor_id,
                              double score, double cost, const char *result) {
    tournament_t *tr = find_tournament(t, tournament_id);
...
int talons_tournament_decide(talons_engine_t *t, int tournament_id) {
    ...
    for (int i = 0; i < tr->competitor_count; i++) {
        if (!tr->competitors[i].finished)
            continue;
        if (tr->competitors[i].speed > max_speed)
            max_speed = tr->competitors[i].speed;
        if (tr->competitors[i].cost > max_cost)
            max_cost = tr->competitors[i].cost;
    }
    ...
    for (int i = 0; i < tr->competitor_count; i++) {
        ...
        double composite = tr->weight_quality * c->score + tr->weight_speed * norm_speed +
    ...
    if (g_talons_vfs && best_id >= 0) {
        vfs_log_event(g_talons_vfs, "tournament", tr->objective, tr->competitors[best_id].label);
    }
...
const tournament_t *talons_tournament_get(const talons_engine_t *t, int tournament_id) {
    for (int i = 0; i < t->tournament_count; i++) {
        if (t->tournaments[i].id == tournament_id)
            return &t->tournaments[i];
    }
    return NULL;
}
...
strategy_type_t talons_recommend_strategy(const talons_engine_t *t, double time_pressure,
                                          double resource_budget, double complexity) {
    if (!t)
    ...
    scores[STRATEGY_SIEGE] +=
        (1.0 - time_pressure) * 0.2 + complexity * 0.15 + resource_budget * 0.1;
    ...
    scores[STRATEGY_OTHISMOS] += +0.60 * (time_pressure - 0.5) * 0.4 +
                                 +0.20 * (resource_budget - 0.5) * 0.4 +
                                 -0.40 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_HAMMER_AND_ANVIL] += +0.50 * (time_pressure - 0.5) * 0.4 +
                                         +0.40 * (resource_budget - 0.5) * 0.4 +
                                         +0.30 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_PELTAST_SOFTENING] += +0.10 * (time_pressure - 0.5) * 0.4 +
                                          -0.20 * (resource_budget - 0.5) * 0.4 +
                                          +0.20 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_MANIPULAR_TACTICS] += -0.10 * (time_pressure - 0.5) * 0.4 +
                                          +0.30 * (resource_budget - 0.5) * 0.4 +
                                          +0.60 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_CIRCUMVALLATION] += -0.70 * (time_pressure - 0.5) * 0.4 +
                                        +0.80 * (resource_budget - 0.5) * 0.4 +
                                        +0.20 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_CASTRAMETATION] += -0.40 * (time_pressure - 0.5) * 0.4 +
                                       +0.50 * (resource_budget - 0.5) * 0.4 +
                                       +0.10 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_WIN_WITHOUT_FIGHTING] += -0.60 * (time_pressure - 0.5) * 0.4 +
                                             -0.30 * (resource_budget - 0.5) * 0.4 +
                                             +0.70 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_BESIEGE_WEI_TO_RESCUE_ZHAO] += +0.40 * (time_pressure - 0.5) * 0.4 +
                                                   +0.20 * (resource_budget - 0.5) * 0.4 +
                                                   +0.60 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_FEIGNED_RETREAT] += +0.50 * (time_pressure - 0.5) * 0.4 +
                                        -0.10 * (resource_budget - 0.5) * 0.4 +
                                        +0.30 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_TULUGHMA] += +0.60 * (time_pressure - 0.5) * 0.4 +
                                 +0.30 * (resource_budget - 0.5) * 0.4 +
                                 +0.40 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_CHEVAUCHEE] += +0.20 * (time_pressure - 0.5) * 0.4 +
                                   -0.10 * (resource_budget - 0.5) * 0.4 +
                                   -0.30 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_CASTELLATION] += -0.80 * (time_pressure - 0.5) * 0.4 +
                                     +0.90 * (resource_budget - 0.5) * 0.4 +
                                     +0.20 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_FLEET_IN_BEING] += -0.50 * (time_pressure - 0.5) * 0.4 +
                                       +0.10 * (resource_budget - 0.5) * 0.4 +
                                       +0.40 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_GUERRE_DE_COURSE] += -0.30 * (time_pressure - 0.5) * 0.4 +
                                         -0.40 * (resource_budget - 0.5) * 0.4 +
                                         +0.30 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_CROSSING_THE_T] += +0.70 * (time_pressure - 0.5) * 0.4 +
                                       +0.30 * (resource_budget - 0.5) * 0.4 +
                                       +0.20 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_BREAKING_THE_LINE] += +0.60 * (time_pressure - 0.5) * 0.4 +
                                          +0.20 * (resource_budget - 0.5) * 0.4 +
                                          +0.40 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_WOLFPACK] += +0.50 * (time_pressure - 0.5) * 0.4 +
                                 +0.40 * (resource_budget - 0.5) * 0.4 +
                                 +0.50 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_CENTRAL_POSITION] += +0.60 * (time_pressure - 0.5) * 0.4 +
                                         -0.10 * (resource_budget - 0.5) * 0.4 +
                                         +0.50 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_GRAND_BATTERY] += +0.30 * (time_pressure - 0.5) * 0.4 +
                                      +0.70 * (resource_budget - 0.5) * 0.4 +
                                      +0.10 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_HARD_WAR] += -0.20 * (time_pressure - 0.5) * 0.4 +
                                 +0.30 * (resource_budget - 0.5) * 0.4 +
                                 +0.20 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_ANACONDA_PLAN] += -0.80 * (time_pressure - 0.5) * 0.4 +
                                      +0.70 * (resource_budget - 0.5) * 0.4 +
                                      +0.30 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_STORMTROOPER_TACTICS] += +0.70 * (time_pressure - 0.5) * 0.4 +
                                             +0.10 * (resource_budget - 0.5) * 0.4 +
                                             +0.50 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_ELASTIC_DEFENSE] += +0.10 * (time_pressure - 0.5) * 0.4 +
                                        +0.40 * (resource_budget - 0.5) * 0.4 +
                                        +0.70 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_BITE_AND_HOLD] += -0.40 * (time_pressure - 0.5) * 0.4 +
                                      +0.60 * (resource_budget - 0.5) * 0.4 +
                                      +0.30 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_AUFTRAGSTAKTIK] += +0.80 * (time_pressure - 0.5) * 0.4 +
                                       -0.20 * (resource_budget - 0.5) * 0.4 +
                                       +0.80 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_DEEP_OPERATION] += +0.40 * (time_pressure - 0.5) * 0.4 +
                                       +0.80 * (resource_budget - 0.5) * 0.4 +
                                       +0.70 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_KESSELSCHLACHT] += +0.50 * (time_pressure - 0.5) * 0.4 +
                                       +0.60 * (resource_budget - 0.5) * 0.4 +
                                       +0.40 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_MASKIROVKA] += -0.30 * (time_pressure - 0.5) * 0.4 +
                                   +0.40 * (resource_budget - 0.5) * 0.4 +
                                   +0.90 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_DOUHET_DOCTRINE] += +0.60 * (time_pressure - 0.5) * 0.4 +
                                        +0.80 * (resource_budget - 0.5) * 0.4 +
                                        +0.30 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_MAOIST_PROTRACTED_WAR] += -0.90 * (time_pressure - 0.5) * 0.4 +
                                              -0.60 * (resource_budget - 0.5) * 0.4 +
                                              +0.50 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_CLEAR_HOLD_BUILD] += -0.70 * (time_pressure - 0.5) * 0.4 +
                                         +0.80 * (resource_budget - 0.5) * 0.4 +
                                         +0.70 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_MUTUAL_ASSURED_DESTRUCTION] += -0.90 * (time_pressure - 0.5) * 0.4 +
                                                   +0.90 * (resource_budget - 0.5) * 0.4 +
                                                   +0.40 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_FLEXIBLE_RESPONSE] += +0.20 * (time_pressure - 0.5) * 0.4 +
                                          +0.70 * (resource_budget - 0.5) * 0.4 +
                                          +0.80 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_REFLEXIVE_CONTROL] += -0.20 * (time_pressure - 0.5) * 0.4 +
                                          -0.10 * (resource_budget - 0.5) * 0.4 +
                                          +0.90 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_HYBRID_WARFARE] += +0.10 * (time_pressure - 0.5) * 0.4 +
                                       +0.60 * (resource_budget - 0.5) * 0.4 +
                                       +0.90 * (complexity - 0.5) * 0.4;
    scores[STRATEGY_NETWORK_CENTRIC_WARFARE] += +0.70 * (time_pressure - 0.5) * 0.4 +
                                                +0.90 * (resource_budget - 0.5) * 0.4 +
                                                +0.80 * (complexity - 0.5) * 0.4;
...
bool talons_should_retry(const talons_engine_t *t, int goal_id,
    ...
    if (!g)
        return false;
    if (g->state == GOAL_CAPTURED || g->state == GOAL_ABANDONED)
        return false;
    if (g->attempts >= g->max_attempts)
        return false;
...
double talons_win_rate(const talons_engine_t *t, int last_n) {
    if (!t || t->history_count == 0 || last_n <= 0)
        return 0;
...
int talons_goal_to_json(const goal_t *g, char *buf, size_t len) {
    ...
    int n = snprintf(buf, len,
                     ...
                     g->id, 80, g->description, talons_goal_state_name(g->state),
                     talons_grip_name(g->grip), talons_strategy_name(g->strategy), g->priority,
                     g->confidence, g->attempts, g->max_attempts, g->total_cost, g->parent_goal_id,
...
int talons_status_json(const talons_engine_t *t, char *buf, size_t len) {
    ...
    int n = snprintf(buf, len,
                     ...
                     "\"strategy_success\":{",
                     t->total_hunts, t->wins, t->losses, t->win_rate, t->active_goals,
                     t->avg_attempts_to_win, t->avg_cost_per_win);

    for (int i = 0; i < STRATEGY_COUNT; i++) {
        double rate = t->strategy_success[i] / (double)t->strategy_attempts[i];
        n += snprintf(buf + n, len - n, "%s\"%s\":%.3f", i ? "," : "",
                      talons_strategy_name((strategy_type_t)i), rate);
    }
...
int talons_to_json(const talons_engine_t *t, char *buf, size_t len) {
    ...
    for (int i = 0; i < t->goal_count && (size_t)n < len - 512; i++) {
        goal_state_t s = t->goals[i].state;
        if (s == GOAL_CAPTURED || s == GOAL_ESCAPED || s == GOAL_ABANDONED)
            continue;
        if (!first)
            n += snprintf(buf + n, len - n, ",");
    ...
    for (int i = 0; i < t->tournament_count && (size_t)n < len - 256; i++) {
        ...
        if (!tr->active && !tr->decided)
            continue;
        n += snprintf(buf + n, len - n,
                      "%s{\"id\":%d,\"objective\":\"%.*s\",\"competitors\":%d,"
                      "\"decided\":%s,\"winner\":%d}",
                      first ? "" : ",", tr->id, 60, tr->objective, tr->competitor_count,
