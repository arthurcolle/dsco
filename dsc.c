...
#define ANTHROPIC_BETAS                                                                            \
    "interleaved-thinking-2025-05-14,code-execution-2025-05-22,advanced-tool-use-2025-11-20"
...
static void outf(const char *fmt, ...) {
    ...
    va_end(ap);
    if (n > 0)
        out_raw(tmp, (size_t)(n < (int)sizeof(tmp) ? n : (int)sizeof(tmp) - 1));
}
...
static bool dsc_uses_claude_code_auth(const char *credential) {
    if (dsc_env_truthy(getenv("DSCO_FORCE_CLAUDE_CODE_AUTH")))
        return true;
...
static void dsc_composer_draw_bottom_rule(void) {
    ...
    fputc('\n', stdout);
    if (g_color)
        fputs("\033[2;38;5;244m  ↵ send  ·  ⌥↵ newline  ·  ↑/↓ history  ·  /help  ·  ctrl+c "
              "cancel\033[0m\n",
              stdout);
    else
...
static void dsc_composer_redraw_input(const char *buf, size_t len, size_t cur) {
    ...
    if (g_color)
        fputs("\033[1;38;5;213m>\033[0m ", stdout);
    else
        fputs("> ", stdout);
    ...
    if (ls > 0) {
        if (g_color)
            fputs("\033[38;5;240m… \033[0m", stdout);
        else
            fputs("... ", stdout);
        multi_pref = 2;
    }
...
static void dsc_composer_insert(char *buf, size_t cap, size_t *len, size_t *cur, const char *s,
                                size_t slen) {
    if (*len + slen >= cap - 1)
        return;
...
static void dsc_hist_push(const char *s) {
    if (!s || !*s)
        return;
    if (g_dsc_hist_n > 0 && strcmp(g_dsc_hist[g_dsc_hist_n - 1], s) == 0)
        return;
    ...
    while (!done) {
        ...
        if (in_paste) {
            if (c == '\033') {
                ...
                int got = 0;
                while (got < 5) {
                    if (read(STDIN_FILENO, &seq[got], 1) != 1)
                        break;
                    got++;
                }
        ...
        if (c == 0x1B) {
            ...
            unsigned char n2;
            if (dsc_read_byte_timed(STDIN_FILENO, 30, &n2) <= 0)
                continue;
            if (n2 == '2') {
                unsigned char n3, n4;
                if (dsc_read_byte_timed(STDIN_FILENO, 30, &n3) > 0 &&
                    dsc_read_byte_timed(STDIN_FILENO, 30, &n4) > 0 && n3 == '0' && n4 == '0') {
                    unsigned char tilde;
            ...
            switch (n2) {
                case 'A': /* Up */
                    if (g_dsc_hist_n > 0) {
                        ...
                        if (hist_pos >= 0 && hist_pos < g_dsc_hist_n) {
                            ...
                            size_t hl = strlen(h);
                            if (hl >= DSC_COMPOSER_CAP)
                                hl = DSC_COMPOSER_CAP - 1;
                            memcpy(buf, h, hl);
                            buf[hl] = '\0';
                            len = hl;
                            cur = hl;
                ...
                case 'B': /* Down */
                    ...
                    continue;
                case 'C':
                    dsc_cur_right(buf, len, &cur);
                    dsc_composer_redraw_input(buf, len, cur);
                    continue;
                case 'D':
                    dsc_cur_left(buf, &cur);
                    dsc_composer_redraw_input(buf, len, cur);
                    continue;
                case 'H':
                    while (cur > 0 && buf[cur - 1] != '\n')
        ...
        if (c >= 0x20 || c >= 0x80) {
            ...
            dsc_composer_insert(buf, DSC_COMPOSER_CAP, &len, &cur, (char *)utf, (size_t)(1 + want));
            dsc_composer_redraw_input(buf, len, cur);
            continue;
        }
    }
...
static void buf_grow(buf_t *b, size_t need) {
    ...
    if (!b->data) {
        fprintf(stderr, "dsc: out of memory\n");
        exit(1);
    }
}
...
static void buf_cat(buf_t *b, const char *s) {
...
static void buf_printf(buf_t *b, const char *fmt, ...) {
    ...
    va_end(ap);
    if (n > 0)
        buf_append(b, tmp, (size_t)(n < (int)sizeof(tmp) ? n : (int)sizeof(tmp) - 1));
}
...
static void dsc_build_claude_code_service_name(char *out, size_t out_len) {
    ...
    if (config_dir && config_dir[0]) {
        ...
        snprintf(dir_suffix, sizeof(dir_suffix), "-%02x%02x%02x%02x", digest[0], digest[1],
                 digest[2], digest[3]);
#endif
    }
...
static bool dsc_load_oauth_bundle(dsc_oauth_bundle_t *bundle) {
    ...
    if (dsc_load_oauth_from_keychain(bundle))
        return true;
    if (dsc_load_oauth_from_file(bundle))
        return true;
...
static char *dsc_build_refreshed_oauth_json(const dsc_oauth_bundle_t *bundle,
    ...
    char *subscription_type =
        bundle->oauth_json ? jstr(bundle->oauth_json, "subscriptionType") : NULL;
...
static bool dsc_find_json_value_span(const char *json, const char *key, const char **out_start,
                                     const char **out_end) {
...
static char *dsc_replace_oauth_json(const dsc_oauth_bundle_t *bundle, const char *new_oauth_json) {
    ...
    if (!dsc_find_json_value_span(bundle->storage_json, "claudeAiOauth", &value_start,
                                  &value_end)) {
        buf_t out;
        buf_init(&out);
...
static bool dsc_write_oauth_keychain(const dsc_oauth_bundle_t *bundle, const char *json) {
    ...
    char *q_account = dsc_shell_quote(bundle->keychain_account[0] ? bundle->keychain_account
                                                                  : "claude-code-user");
...
static bool dsc_persist_oauth_bundle(const dsc_oauth_bundle_t *bundle, const char *scope_string) {
    if (!bundle || bundle->source == DSC_OAUTH_SOURCE_ENV)
        return true;
...
static const char *dsc_resolve_claude_code_oauth_token(void) {
    ...
    if (!dsc_load_oauth_bundle(&bundle))
        return NULL;

    if (bundle.source != DSC_OAUTH_SOURCE_ENV && bundle.refresh_token[0] &&
...
static const char *TOOLS_JSON =
    "["
    "{\"name\":\"shell\","
    "\"description\":\"Execute a shell command and return stdout+stderr. Use for running programs, "
    "checking system state, fetching URLs with curl, etc.\","
    ...
    "\"input_schema\":{\"type\":\"object\",\"properties\":{\"path\":{\"type\":\"string\","
    "\"description\":\"File path\"},\"content\":{\"type\":\"string\",\"description\":\"File "
...
static char *exec_shell(const char *command) {
    ...
    if (fd < 0)
        return strdup("error: mkstemp failed");
...
static char *exec_read_file(const char *path) {
    ...
    if (!fp) {
        char e[512];
        snprintf(e, sizeof(e), "error: cannot open '%s'", path);
        return strdup(e);
    }
    buf_t b;
    buf_init(&b);
...
static char *exec_write_file(const char *path, const char *content) {
    ...
    if (!fp) {
        char e[512];
        snprintf(e, sizeof(e), "error: cannot write '%s'", path);
        return strdup(e);
    }
    fwrite(content, 1, strlen(content), fp);
    fclose(fp);
    char ok[512];
    snprintf(ok, sizeof(ok), "wrote %zu bytes to %s", strlen(content), path);
    return strdup(ok);
}
...
static char *execute_tool(const char *name, const char *input_json) {
    ...
    if (strcmp(name, "write_file") == 0) {
        ...
        if (!p || !c) {
            free(p);
            free(c);
            return strdup("error: missing path/content");
        }
        char *r = exec_write_file(p, c);
        free(p);
        free(c);
        return r;
    }
...
static void md_ansi(const char *seq) {
    if (g_color)
        out(seq);
}
...
static const mathfont_t g_mathfonts[] = {
    ...
    {"mathbf", 0x1D400, 0x1D41A, 0x1D7CE}, {"boldsymbol", 0x1D400, 0x1D41A, 0x1D7CE},
    {"mathit", 0x1D434, 0x1D44E, -1},      {"mathbi", 0x1D468, 0x1D482, -1},
    {"mathcal", 0x1D49C, -1, -1},          {"mathscr", 0x1D49C, -1, -1},
    {"mathbscr", 0x1D4D0, 0x1D4EA, -1},    {"mathfrak", 0x1D504, 0x1D51E, -1},
    {"mathbb", 0x1D538, 0x1D552, 0x1D7D8}, {"mathbfrak", 0x1D56C, 0x1D586, -1},
    {"mathsf", 0x1D5A0, 0x1D5BA, 0x1D7E2}, {"mathsfbf", 0x1D5D4, 0x1D5EE, 0x1D7EC},
    {"mathsfit", 0x1D608, 0x1D622, -1},    {"mathsfbfit", 0x1D63C, 0x1D656, -1},
    {"mathtt", 0x1D670, 0x1D68A, 0x1D7F6}, {NULL, 0, 0, 0}};
...
static const struct {
    ...
    {'0', 0x2070}, {'1', 0x00B9}, {'2', 0x00B2}, {'3', 0x00B3}, {'4', 0x2074}, {'5', 0x2075},
    {'6', 0x2076}, {'7', 0x2077}, {'8', 0x2078}, {'9', 0x2079}, {'+', 0x207A}, {'-', 0x207B},
    {'=', 0x207C}, {'(', 0x207D}, {')', 0x207E}, {'i', 0x2071}, {'n', 0x207F}, {'a', 0x1D43},
    {'b', 0x1D47}, {'c', 0x1D9C}, {'d', 0x1D48}, {'e', 0x1D49}, {'f', 0x1DA0}, {'g', 0x1D4D},
    {'h', 0x02B0}, {'j', 0x02B2}, {'k', 0x1D4F}, {'l', 0x02E1}, {'m', 0x1D50}, {'o', 0x1D52},
    {'p', 0x1D56}, {'r', 0x02B3}, {'s', 0x02E2}, {'t', 0x1D57}, {'u', 0x1D58}, {'v', 0x1D5B},
    {'w', 0x02B7}, {'x', 0x02E3}, {'y', 0x02B8}, {'z', 0x1DBB}, {'A', 0x1D2C}, {'B', 0x1D2E},
    {'D', 0x1D30}, {'E', 0x1D31}, {'G', 0x1D33}, {'H', 0x1D34}, {'I', 0x1D35}, {'J', 0x1D36},
    {'K', 0x1D37}, {'L', 0x1D38}, {'M', 0x1D39}, {'N', 0x1D3A}, {'O', 0x1D3C}, {'P', 0x1D3E},
    {'R', 0x1D3F}, {'T', 0x1D40}, {'U', 0x1D41}, {'V', 0x2C7D}, {'W', 0x1D42}, {0, 0}};

static const struct {
    char ch;
    int cp;
} g_sub[] = {{'0', 0x2080}, {'1', 0x2081}, {'2', 0x2082}, {'3', 0x2083}, {'4', 0x2084},
...
static const char *latex_lookup(const char *cmd) {
    for (int i = 0; g_latex[i].cmd; i++)
        if (strcmp(g_latex[i].cmd, cmd) == 0)
            return g_latex[i].sym;
    return NULL;
}
...
static int latex_brace_cmd(const char *cmd) {
    ...
    for (int i = 0; g_mathfonts[i].name; i++)
        if (strcmp(cmd, g_mathfonts[i].name) == 0)
            return LCMD_MATHFONT;
    /* Arbitrary Unicode: \u{XXXX} */
    if (strcmp(cmd, "u") == 0 || strcmp(cmd, "U") == 0)
        return LCMD_UNICODE;
    /* Algorithmic Unicode blocks */
    if (strcmp(cmd, "braille") == 0)
        return LCMD_BRAILLE;
    if (strcmp(cmd, "hexagram") == 0 || strcmp(cmd, "iching") == 0)
        return LCMD_HEXAGRAM;
    if (strcmp(cmd, "card") == 0)
        return LCMD_CARD;
    if (strcmp(cmd, "boxed") == 0 || strcmp(cmd, "fbox") == 0)
        return LCMD_BOXED;
    if (strcmp(cmd, "overline") == 0)
        return LCMD_OVERLINE;
    if (strcmp(cmd, "underline") == 0)
        return LCMD_UNDERLINE;
...
static int latex_brace_argc(int cmd) {
    return (cmd == LCMD_FRAC) ? 2 : 1;
}
    ...
    switch (g_md.brace_cmd) {
        ...
        case LCMD_ACCENT: {
            ...
            static const struct {
                const char *n;
                const char *u;
            } acc[] = {{"hat", "\xCC\x82"},   {"widehat", "\xCC\x82"},   {"check", "\xCC\x8C"},
            ...
            for (int i = 0; acc[i].n; i++)
                if (strcmp(acc[i].n, g_md.latex) == 0) {
                    mark = acc[i].u;
                    break;
                }
            for (int i = 0; i < g_md.ba1n; i++) {
...
static void md_resolve_latex(void) {
    ...
    if (sym)
        ...
        out("\\");
        out_raw(g_md.latex, (size_t)g_md.latexn);
    }
    g_md.latexn = 0;
}
...
static void md_resolve_stars(void) {
    ...
    if (s >= 3) {
        if (g_md.bold && g_md.italic) {
            md_ansi("\033[22;23m");
            g_md.bold = g_md.italic = false;
        } else {
            md_ansi("\033[1;3m");
            g_md.bold = g_md.italic = true;
        }
        for (int i = 3; i < s; i++)
            out("*");
    } else if (s == 2) {
    ...
    for (const unsigned char *p = (const unsigned char *)text; *p; p++) {
                ...
                if (c == '{') {
                    ...
                    if (g_md.brace_depth == 0) {
                        if (g_md.brace_which < latex_brace_argc(g_md.brace_cmd)) {
                            ...
                            md_emit_brace_result();
                        }
                        continue;
                    }
                }
                if (g_md.brace_depth > 0) {
                    if (g_md.brace_which == 1 && g_md.ba1n < 510)
                        g_md.ba1[g_md.ba1n++] = c;
                    else if (g_md.brace_which == 2 && g_md.ba2n < 510)
                        g_md.ba2[g_md.ba2n++] = c;
                }
                continue;
            }
        }
        ...
        if (g_md.sup_sub > 0) {
            if (c == '{' && !g_md.sup_sub_brace) {
                g_md.sup_sub_brace = true;
                continue;
            }
            if (g_md.sup_sub_brace) {
                if (c == '}') {
                    g_md.sup_sub = 0;
                    g_md.sup_sub_brace = false;
                    continue;
                }
        ...
        if (g_md.fence_lang_col) {
            ...
            if (g_md.fence_langn < 30)
                g_md.fence_lang[g_md.fence_langn++] = c;
            continue;
        }
        ...
        if (c == '`') {
            if (g_md.stars > 0 && !g_md.fence)
                md_resolve_stars();
            if (g_md.tildes > 0)
                md_resolve_tildes();
            if (g_md.hashes > 0) {
                for (int i = 0; i < g_md.hashes; i++)
                    out("#");
                g_md.hashes = 0;
            }
            g_md.ticks++;
        ...
        if (g_md.ticks > 0) {
            md_resolve_ticks();
            if (g_md.fence && g_md.fence_lang_col) {
                p--;
                continue;
            }
            if (g_md.code || g_md.fence) {
                out_raw(&c, 1);
                g_md.sol = (c == '\n');
                continue;
            }
        ...
        if (g_md.sol && c >= '0' && c <= '9') {
            if (g_md.sol_dign < 14)
                g_md.sol_digits[g_md.sol_dign++] = c;
            continue;
        }
        if (g_md.sol_dign > 0) {
            if (c == '.' && !g_md.sol_dot) {
            ...
            for (int i = 0; i < g_md.sol_dign; i++)
                out_raw(&g_md.sol_digits[i], 1);
            if (g_md.sol_dot)
                out(".");
            g_md.sol_dign = 0;
            g_md.sol_dot = false;
        ...
        if (g_md.sol_dashes > 0 && c != '-') {
            ...
            for (int i = 0; i < g_md.sol_dashes; i++)
                out("-");
            g_md.sol_dashes = 0;
        }
...
static void md_end_block(void) {
    ...
    if (g_md.brace_cmd != LCMD_NONE)
        g_md.brace_cmd = LCMD_NONE;
    if (g_md.stars > 0) {
        for (int i = 0; i < g_md.stars; i++)
            out("*");
        g_md.stars = 0;
    }
    if (g_md.tildes > 0) {
        for (int i = 0; i < g_md.tildes; i++)
            out("~");
        g_md.tildes = 0;
    }
    if (g_md.ticks > 0) {
        for (int i = 0; i < g_md.ticks; i++)
            out("`");
        g_md.ticks = 0;
    }
    if (g_md.hashes > 0) {
    ...
    g_md.sup_sub = 0;
    g_md.sup_sub_brace = false;
    if (g_md.bold || g_md.italic || g_md.code || g_md.fence || g_md.header || g_md.strike ||
        g_md.math)
        md_ansi("\033[0m");
...
static void sse_handle_event(sse_t *s, const char *data) {
    ...
    if (strcmp(type, "message_start") == 0) {
        ...
        if (cb) {
            ...
            if (s->cur_type && strcmp(s->cur_type, "tool_use") == 0) {
                free(s->cur_tool_name);
                s->cur_tool_name = jstr(cb, "name");
                free(s->cur_tool_id);
                s->cur_tool_id = jstr(cb, "id");
                buf_reset(&s->input_buf);
                if (s->cur_tool_name)
                    outf("\n\033[1;33m  ⚡ %s\033[0m\n", s->cur_tool_name);
...
static const char *g_system = "You are a helpful coding assistant with access to shell, read_file, "
                              "and write_file tools. Use them proactively. Be concise and direct.";
...
static bool api_stream(const char *api_key, sse_t *state) {
    ...
    char ver[128];
    snprintf(ver, sizeof(ver), "anthropic-version: %s", API_VERSION);
...
__attribute__((noreturn)) static void dsc_exec_dsco_new(int argc, char **argv, const char *reason);
        ...
        if (state.got_error) {
            ...
            if (dsc_err_is_credit(state.error_msg)) {
                ...
                if (has_other && g_main_argv) {
                    sse_free(&state);
                    dsc_exec_dsco_new(g_main_argc, g_main_argv, "anthropic credit exhausted");
                    /* noreturn */
                }
...
static bool dsc_model_is_anthropic(const char *m) {
    ...
    if (strchr(m, '/'))
        return false;
    if (strstr(m, "claude") || strstr(m, "opus") || strstr(m, "sonnet") || strstr(m, "haiku"))
        return true;
    return false;
}

static char *dsc_sibling_path(const char *argv0, const char *name, char *out, size_t out_len) {
    if (!out || out_len == 0)
        return NULL;
    out[0] = '\0';
    if (!argv0 || !*argv0) {
...
int main(int argc, char **argv) {
    ...
    if (!api_key || !*api_key) {
        ...
        if (getenv("OPENROUTER_API_KEY") || getenv("OPENAI_API_KEY") || getenv("XAI_API_KEY") ||
            getenv("GROK_API_KEY") || getenv("GOOGLE_API_KEY") || getenv("GEMINI_API_KEY")) {
            dsc_exec_dsco_new(argc, argv, "no Anthropic credential");
        }
        fprintf(stderr, "dsc: sign in with Claude Code or set ANTHROPIC_API_KEY\n"
                        "  \033[2mhint: or set OPENROUTER_API_KEY / XAI_API_KEY / OPENAI_API_KEY "
                        "and dsc will hand off to dsco-new.\033[0m\n");
        return 1;
    }
    ...
    for (int i = 1; i < argc; i++) {
        if ((strcmp(argv[i], "--model") == 0 || strcmp(argv[i], "-m") == 0) && i + 1 < argc)
            ...
            if (i + 1 < argc && argv[i + 1][0] >= '0' && argv[i + 1][0] <= '9') {
                ...
                if (g_thinking_budget <= 0)
                    g_thinking_budget = 10000;
            }
    ...
    if (g_color) {
        ...
        if (g_thinking)
            outf(" · \033[33mthinking:%d\033[0m", g_thinking_budget);
        out("\n");
        out("  \033[2m/help for commands");
        if (g_iterm2)
            out(" · iTerm2 streaming");
