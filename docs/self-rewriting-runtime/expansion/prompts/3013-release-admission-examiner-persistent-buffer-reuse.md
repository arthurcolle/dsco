# 3013 — Release admission examiner: Learned persistent-buffer reuse

Implement the learned persistent-buffer reuse feature for the Release admission examiner RLM in `/Users/arthurcolle/Dsco/dsco-cli` or its assigned worktree. Decide whether a tested code/model pair satisfies release obligations without confusing recorded assertions with verified evidence.
...
Domain challenge: A process retaining its PID after exec must not count as uninterrupted in-memory code replacement. Change one dependency while leaving the buffer's human-readable label unchanged. The policy must recompute affected results and retain unaffected ones; stale reuse or unnecessary replay of completed effects fails.
