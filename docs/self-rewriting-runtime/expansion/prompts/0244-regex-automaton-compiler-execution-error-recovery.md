# 244 — Bounded regex automaton compiler: Learned execution-error recovery

Implement the learned execution-error recovery feature for the Bounded regex automaton compiler RLM in `/Users/arthurcolle/Dsco/dsco-cli` or its assigned worktree. Learn native matcher generation for a declared regular-expression subset using bounded automata instead of uncontrolled backtracking.
...
Domain challenge: A nested ambiguous quantifier triggers exponential work or silently changes leftmost matching on an overlapping substring. Inject an undefined buffer and a completed effect before the failure. Recovery must resolve the buffer without repeating the effect; inventing a final answer cannot count as repair.
