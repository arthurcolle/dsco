# 701 — Native hashmap collision policy designer: Coverage-directed recursive traversal

Implement the coverage-directed recursive traversal feature for the Native hashmap collision policy designer RLM in `/Users/arthurcolle/Dsco/dsco-cli` or its assigned worktree. Learn native hashmap lookup, insertion, deletion, and resizing procedures that preserve key identity under adversarial collisions.
...
Domain challenge: Deleting one colliding key breaks the probe chain and makes a later still-present key appear absent. Drop one required region while returning a plausible aggregate. The completeness check must reject it; duplicate completion messages cannot compensate for a missing node or inflate coverage.
