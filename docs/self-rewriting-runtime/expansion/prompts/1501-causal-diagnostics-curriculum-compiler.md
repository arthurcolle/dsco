# 1501 — Causal diagnostic investigator: Executable curriculum compiler

Implement the executable curriculum compiler feature for the Causal diagnostic investigator RLM in `/Users/arthurcolle/Dsco/dsco-cli` or its assigned worktree. Train a diagnostic RLM to discriminate competing runtime fault mechanisms using controlled interventions and preserved counterevidence.
...
Domain challenge: The last function in a crash trace must not be blamed when earlier ownership transfer caused corruption. Reject a generator that changes the answer without updating its reference or produces duplicate families under different seeds. Measure valid-case coverage and trained-root success separately.
