# 3017 — Release admission examiner: Learned typed role handoffs

Implement the learned typed role handoffs feature for the Release admission examiner RLM in `/Users/arthurcolle/Dsco/dsco-cli` or its assigned worktree. Decide whether a tested code/model pair satisfies release obligations without confusing recorded assertions with verified evidence.
...
Domain challenge: A process retaining its PID after exec must not count as uninterrupted in-memory code replacement. A partner omitting required evidence or returning a result from another task must be rejected. Matching prose labels cannot replace schema and identity checks, and a repaired handoff must preserve its original authority.
