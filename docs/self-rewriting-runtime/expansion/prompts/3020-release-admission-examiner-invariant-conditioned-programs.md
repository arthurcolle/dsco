# 3020 — Release admission examiner: Invariant-conditioned program generation

Implement the invariant-conditioned program generation feature for the Release admission examiner RLM in `/Users/arthurcolle/Dsco/dsco-cli` or its assigned worktree. Decide whether a tested code/model pair satisfies release obligations without confusing recorded assertions with verified evidence.
...
Domain challenge: A process retaining its PID after exec must not count as uninterrupted in-memory code replacement. A candidate that satisfies visible examples while violating a required invariant on a boundary case must fail. Removing the check or replacing it with the root's assurance must not change the verdict.
