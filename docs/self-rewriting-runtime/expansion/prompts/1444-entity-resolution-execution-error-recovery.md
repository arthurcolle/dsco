# 1444 — Entity identity resolver: Learned execution-error recovery

Implement the learned execution-error recovery feature for the Entity identity resolver RLM in `/Users/arthurcolle/Dsco/dsco-cli` or its assigned worktree. Train an entity RLM to resolve identities across traces and documents without collapsing similar names.
...
Domain challenge: Two functions with the same name in different libraries must not share behavioral evidence automatically. Inject an undefined buffer and a completed effect before the failure. Recovery must resolve the buffer without repeating the effect; inventing a final answer cannot count as repair.
