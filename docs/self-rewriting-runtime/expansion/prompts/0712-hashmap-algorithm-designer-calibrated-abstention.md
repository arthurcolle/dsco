# 712 — Native hashmap collision policy designer: Calibrated role abstention

Implement the calibrated role abstention feature for the Native hashmap collision policy designer RLM in `/Users/arthurcolle/Dsco/dsco-cli` or its assigned worktree. Learn native hashmap lookup, insertion, deletion, and resizing procedures that preserve key identity under adversarial collisions.
...
Domain challenge: Deleting one colliding key breaks the probe chain and makes a later still-present key appear absent. A timeout or unavailable oracle must remain unknown. Report the retained-task denominator and coverage; a high-confidence wrong answer and an always-abstain policy must both fail their predeclared acceptance conditions.
